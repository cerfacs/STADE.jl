...
# All shapes below are Dict{Symbol,Any}, keyed by the field names shown.
#
# kernel        :: Dict(:sig=>kernel_sig, :body=>statement_list)
# kernel_sig    :: Dict(:name=>Symbol, :args=>Vector{Symbol},
...
# statement     :: one of
#     Dict(:kind=>:assign, :lhs=>.., :rhs=>..)                    # lhs/rhs :: Expr|Symbol|Number
#     Dict(:kind=>:for, :var=>Symbol, :lo=>.., :hi=>.., :step=>.., :sequential=>Bool, :body=>..)  # body :: statement_list
#     Dict(:kind=>:if, :cond=>.., :then=>statement_list, :els=>statement_list)
...
function parse_kernel(expr::Expr)
    ...
    kinds = shape_infer(Dict{Symbol,Any}(:name => name, :args => args), body)
    indep, dep = parse_infer_indep_dep(args, kinds)
    sig = Dict{Symbol,Any}(:name => name, :args => args, :kinds => kinds,
                            :independents => indep, :dependents => dep)
    return Dict{Symbol,Any}(:sig => sig, :body => body)
end
...
function parse_override_indep_dep(kernel, independents, dependents)
    ...
    sig = kernel[:sig]
    new_sig = Dict{Symbol,Any}(:name => sig[:name], :args => sig[:args], :kinds => sig[:kinds],
                                :independents => independents === nothing ? sig[:independents] : independents,
                                :dependents => dependents === nothing ? sig[:dependents] : dependents)
    return Dict{Symbol,Any}(:sig => new_sig, :body => kernel[:body])
end
...
function parse_statements(stmts::Vector)
    return Dict{Symbol,Any}[parse_statement(s) for s in stmts]
end
...
function parse_assign(stmt::Expr)
    ...
    return Dict{Symbol,Any}(:kind => :assign, :lhs => lhs, :rhs => rhs)
end
...
function parse_for(stmt::Expr)
    ...
    return Dict{Symbol,Any}(:kind => :for, :var => var, :lo => lo, :hi => hi, :step => step, :sequential => sequential, :body => body)
end
...
function parse_if(stmt::Expr)
    ...
    return Dict{Symbol,Any}(:kind => :if, :cond => cond, :then => then_stmts, :els => els_stmts)
end
...
function parse_check_local_names(body::Vector{Dict{Symbol,Any}})
    for stmt in body
        if stmt[:kind] == :assign
...
function shape_collect_vars!(body::Vector{Dict{Symbol,Any}}, vars::Set{Symbol})
    for stmt in body
        if stmt[:kind] == :assign
...
function shape_mark_arrays!(body::Vector{Dict{Symbol,Any}}, is_array::Dict{Symbol,Bool})
    for stmt in body
        if stmt[:kind] == :assign
...
function shape_mark_int_direct!(body::Vector{Dict{Symbol,Any}}, is_int::Dict{Symbol,Bool})
    for stmt in body
        if stmt[:kind] == :assign
...
function shape_propagate_int!(body::Vector{Dict{Symbol,Any}}, is_int::Dict{Symbol,Bool})
    changed = false
    for stmt in body
...
function der_table()
    return Dict{Symbol,Dict{Symbol,Any}}(
        :+     => Dict{Symbol,Any}(:tangent => der_tangent_add, :adjoint => der_adjoint_add),
        :-     => Dict{Symbol,Any}(:tangent => der_tangent_sub, :adjoint => der_adjoint_sub),
        :*     => Dict{Symbol,Any}(:tangent => der_tangent_mul, :adjoint => der_adjoint_mul),
        :/     => Dict{Symbol,Any}(:tangent => der_tangent_divide, :adjoint => der_adjoint_divide),
        :^     => Dict{Symbol,Any}(:tangent => der_tangent_pow, :adjoint => der_adjoint_pow),
        :abs   => Dict{Symbol,Any}(:tangent => der_tangent_abs, :adjoint => der_adjoint_abs),
        :sqrt  => Dict{Symbol,Any}(:tangent => der_tangent_sqrt, :adjoint => der_adjoint_sqrt),
        :exp   => Dict{Symbol,Any}(:tangent => der_tangent_exp, :adjoint => der_adjoint_exp),
        :log   => Dict{Symbol,Any}(:tangent => der_tangent_log, :adjoint => der_adjoint_log),
        :log10 => Dict{Symbol,Any}(:tangent => der_tangent_log10, :adjoint => der_adjoint_log10),
        :sin   => Dict{Symbol,Any}(:tangent => der_tangent_sin, :adjoint => der_adjoint_sin),
        :cos   => Dict{Symbol,Any}(:tangent => der_tangent_cos, :adjoint => der_adjoint_cos),
        :tan   => Dict{Symbol,Any}(:tangent => der_tangent_tan, :adjoint => der_adjoint_tan),
        :asin  => Dict{Symbol,Any}(:tangent => der_tangent_asin, :adjoint => der_adjoint_asin),
        :acos  => Dict{Symbol,Any}(:tangent => der_tangent_acos, :adjoint => der_adjoint_acos),
        :atan  => Dict{Symbol,Any}(:tangent => der_tangent_atan, :adjoint => der_adjoint_atan),
        :sinh  => Dict{Symbol,Any}(:tangent => der_tangent_sinh, :adjoint => der_adjoint_sinh),
        :cosh  => Dict{Symbol,Any}(:tangent => der_tangent_cosh, :adjoint => der_adjoint_cosh),
        :tanh  => Dict{Symbol,Any}(:tangent => der_tangent_tanh, :adjoint => der_adjoint_tanh),
        :mod   => Dict{Symbol,Any}(:tangent => der_tangent_mod, :adjoint => der_adjoint_mod),
        :div   => Dict{Symbol,Any}(:tangent => der_tangent_intdiv, :adjoint => der_adjoint_intdiv),
        :max   => Dict{Symbol,Any}(:tangent => der_tangent_max, :adjoint => der_adjoint_max),
        :min   => Dict{Symbol,Any}(:tangent => der_tangent_min, :adjoint => der_adjoint_min),
        :sign  => Dict{Symbol,Any}(:tangent => der_tangent_sign, :adjoint => der_adjoint_sign),
        :floor => Dict{Symbol,Any}(:tangent => der_tangent_floor, :adjoint => der_adjoint_floor),
        :ceil  => Dict{Symbol,Any}(:tangent => der_tangent_ceil, :adjoint => der_adjoint_ceil),
        :trunc => Dict{Symbol,Any}(:tangent => der_tangent_trunc, :adjoint => der_adjoint_trunc),
    )
end
...
# spirit, and round-tripping parse_if's els = Dict{Symbol,Any}[] for a
...
# pass
function act_propagate!(body::Vector{Dict{Symbol,Any}}, active_map::Dict{Symbol,Bool}, kinds::Dict{Symbol,Symbol})
    changed = false
    for stmt in body
        if stmt[:kind] == :assign
...
function act_propagate_assign!(stmt::Dict, active_map::Dict{Symbol,Bool}, kinds::Dict{Symbol,Symbol})
    var = stmt[:lhs] isa Symbol ? stmt[:lhs] : stmt[:lhs].args[1]
...
function snap_walk!(body, active_map, kinds, seq, reassigned, read_anywhere, sites, counter)
    for stmt in body
        ...
        elseif stmt[:kind] == :if
            counter[] = counter[] + 1
            push!(sites, Dict{Symbol,Any}(:kind => :branch, :array => :cond, :at => counter[]))
...
function snap_check_assign!(stmt, active_map, kinds, seq, read_anywhere, sites, counter)
    ...
    counter[] = counter[] + 1
    push!(sites, Dict{Symbol,Any}(:kind => kind, :array => var, :at => counter[]))
    return nothing
end
...
function snap_check_tripcount!(stmt, reassigned, sites, counter)
    ...
    for bv in bound_vars
        if bv in reassigned
            counter[] = counter[] + 1
            push!(sites, Dict{Symbol,Any}(:kind => :tripcount, :array => bv, :at => counter[]))
        end
    end
    return nothing
end
...
#     Dict(:kind=>:assign, :lhs=>.., :active=>Bool, :tree=>lin_node)
#     Dict(:kind=>:for, :var=>.., :lo=>.., :hi=>.., :step=>.., :sequential=>.., :body=>lin_plan)
#     Dict(:kind=>:if, :cond=>.., :then=>lin_plan, :els=>lin_plan)
...
function lin_build_stmt(stmt, active_map)
    if stmt[:kind] == :assign
        ...
        return Dict{Symbol,Any}(:kind => :assign, :lhs => stmt[:lhs], :active => tree[:active], :tree => tree)
    elseif stmt[:kind] == :for
        return Dict{Symbol,Any}(:kind => :for, :var => stmt[:var], :lo => stmt[:lo], :hi => stmt[:hi], :step => stmt[:step],
                :sequential => stmt[:sequential], :body => lin_build_body(stmt[:body], active_map))
    elseif stmt[:kind] == :if
        return Dict{Symbol,Any}(:kind => :if, :cond => stmt[:cond],
...
function lin_build_expr(expr, active_map)
    if expr isa Symbol
        return Dict{Symbol,Any}(:kind => :leaf, :expr => expr, :op => :leaf, :args => [], :children => Dict{Symbol,Any}[],
    ...
    elseif expr isa Expr && expr.head == :call
        ...
        return Dict{Symbol,Any}(:kind => :op, :expr => rebuilt, :op => op, :args => args, :children => children,
...
function agen_emit(kernel, lin_plan, snapshot_plan)
    ...
    return Dict{Symbol,Any}(:adjoint => adjoint_expr, :initstacks => initstacks_expr)
end
...
function hvp_emit(kernel, active_map, lin_plan, sites)
    ...
    # the exact statement lists agen_ itself builds -- this stage
    # differentiates that output, it never regenerates it
...
function hvp_double_stmt(e::Expr, shadow_of)
    ...
    return Any[e]   # anything else (shouldn't occur in agen_'s own output) passes through unchanged
end
...
function val_finite_diff_check(f_eval::Function, f_grad::Function, x0::Vector{Float64};
    ...
    for t in 1:trials
        ...
        push!(results, Dict{Symbol,Any}(:direction => d, :finite_diff => fd, :adjoint_derivative => ad, :rel_err => rel_err))
    end
    return Dict{Symbol,Any}(:ok => worst_rel_err <= rtol, :max_rel_err => worst_rel_err, :trials => results)
end
...
# Hessian-vector product via forward-over-reverse: a second forward-
# mode pass over the adjoint kernel's own generated code (hvp_*).
# The returned :hvp function reuses the SAME stacks foo_b itself
# needs (declared as ordinary parameters, exactly like foo_b's own
# signature) -- :initstacks here is that same companion, not a new
# one, so one initstacks_foo_b() call serves both the adjoint and the
# hvp kernel.
