...
function metal_kernel_two_field_loss_hv_1!(i_n, p, pd, u, ud)
...
function metal_kernel_two_field_loss_hv_2!(i_n, q, qd, v, vd)
    __tid = (thread_position_in_grid()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function metal_kernel_two_field_loss_hv_3!(i_n, qb, qbd, v, vb, vbd, vd)
    __tid = (thread_position_in_grid()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function metal_kernel_two_field_loss_hv_4!(i_n, pb, pbd, u, ub, ubd, ud)
    __tid = (thread_position_in_grid()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function two_field_loss_hv_metal(loss, lossb, u, ub, v, vb, p, pb, q, qb, i_n, lossd, lossbd, ud, ubd, vd, vbd, pd, pbd, qd, qbd)
    nthread_per_block = 256
    @metal threads = nthread_per_block groups = cld(div(i_n - 1, 1) + 1, nthread_per_block) metal_kernel_two_field_loss_hv_1!(i_n, p, pd, u, ud)
    @metal threads = nthread_per_block groups = cld(div(i_n - 1, 1) + 1, nthread_per_block) metal_kernel_two_field_loss_hv_2!(i_n, q, qd, v, vd)
    for i_seq_x = 1:i_n
    ...
    @metal threads = nthread_per_block groups = cld(div(i_n - 1, 1) + 1, nthread_per_block) metal_kernel_two_field_loss_hv_3!(i_n, qb, qbd, v, vb, vbd, vd)
    @metal threads = nthread_per_block groups = cld(div(i_n - 1, 1) + 1, nthread_per_block) metal_kernel_two_field_loss_hv_4!(i_n, pb, pbd, u, ub, ubd, ud)
    return nothing
end
