...
function initstacks_cond_loop_choice_b_metal()
    branch_stack = Vector{Int64}()
    return branch_stack
end

function cond_loop_choice_hv_metal(loss, lossb, u, ub, v, vb, i_branch, i_n, lossd, lossbd, ud, ubd, vd, vbd, branch_stack)
    if i_branch == 1
        push!(branch_stack, 1)
        for i_seq_x = 1:i_n
