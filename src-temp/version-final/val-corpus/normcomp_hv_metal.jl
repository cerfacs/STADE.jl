...
function metal_kernel_normcomp_hv_1!(i_n, u, ud, v, vd, w, wd)
...
function metal_kernel_normcomp_hv_2!(i_n, ub, ubd, vb, vbd, wb, wbd)
    __tid = (thread_position_in_grid()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function initstacks_normcomp_b_metal()
    return nothing
end

function normcomp_hv_metal(loss, lossb, u, ub, v, vb, w, wb, i_n, lossd, lossbd, ud, ubd, vd, vbd, wd, wbd)
    nthread_per_block = 256
    @metal threads = nthread_per_block groups = cld(div(i_n - 1, 1) + 1, nthread_per_block) metal_kernel_normcomp_hv_1!(i_n, u, ud, v, vd, w, wd)
    for i_seq_x = 1:i_n
    ...
    @metal threads = nthread_per_block groups = cld(div(i_n - 1, 1) + 1, nthread_per_block) metal_kernel_normcomp_hv_2!(i_n, ub, ubd, vb, vbd, wb, wbd)
    return nothing
end
