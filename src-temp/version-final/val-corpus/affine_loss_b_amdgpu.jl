...
function amdgpu_kernel_affine_loss_b_1!(a, b, i_n, u, v)
...
function amdgpu_kernel_affine_loss_b_2!(a, ab, bb, i_n, u, ub, vb)
    __tid = (workitemIdx()).x + ((workgroupIdx()).x - 1) * (workgroupDim()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function affine_loss_b_amdgpu(loss, lossb, u, ub, a, ab, b, bb, v, vb, i_n)
    nthread_per_block = 256
    @roc groupsize = nthread_per_block gridsize = cld(div(i_n - 1, 1) + 1, nthread_per_block) amdgpu_kernel_affine_loss_b_1!(a, b, i_n, u, v)
    for i_seq_x = 1:i_n
    ...
    @roc groupsize = nthread_per_block gridsize = cld(div(i_n - 1, 1) + 1, nthread_per_block) amdgpu_kernel_affine_loss_b_2!(a, ab, bb, i_n, u, ub, vb)
    return nothing
end
