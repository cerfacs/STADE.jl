...
function cuda_kernel_stencil_loss_b_2!(i_n, ub, wb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div((i_n - 1) - 2, 1) + 1
        return nothing
    end
...
function stencil_loss_b_cuda(loss, lossb, u, ub, w, wb, i_n)
    ...
    @cuda threads = nthread_per_block blocks = cld(div((i_n - 1) - 2, 1) + 1, nthread_per_block) cuda_kernel_stencil_loss_b_2!(i_n, ub, wb)
    return nothing
end
