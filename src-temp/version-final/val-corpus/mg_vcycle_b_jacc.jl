...
function jacc_kernel_mg_vcycle_b_1!(__jacc_i, i_seq_level, nc, u)
    j = 1 + (__jacc_i - 1)
    u[j, i_seq_level + 1] = 0.0
    return nothing
end

function jacc_kernel_mg_vcycle_b_2!(__jacc_i, i_seq_level, nc, ub)
    j = 1 + (__jacc_i - 1)
...
function mg_vcycle_b_jacc(u, ub, f, fb, r, rb, nfine, num_levels, h1, h1b, nu1, nu2, n, hl_stack, hl2_stack, tripcount_stack, left_stack, branch_stack, right_stack, u_stack, r_stack, f_stack, cl_stack, cr_stack)
    cl = 0.0
    cr = 0.0
    hl = 0.0
    hl2 = 0.0
    ...
    for i_seq_level = 1:num_levels - 1
        ...
        push!(tripcount_stack, nc)
        JACC.parallel_for(div(nc - 1, 1) + 1, jacc_kernel_mg_vcycle_b_1!, i_seq_level, nc, u)
        nl = ncg
        push!(hl_stack, hl)
        hl = hl * 2.0
    end
    ...
    for i_seq_level = num_levels - 1:-1:1
        ...
        hlb = 2.0hlb
        nc = pop!(tripcount_stack)
        JACC.parallel_for(div(nc - 1, 1) + 1, jacc_kernel_mg_vcycle_b_2!, i_seq_level, nc, ub)
        nc = pop!(tripcount_stack)
        for j = nc:-1:1
            jf = j * 2
