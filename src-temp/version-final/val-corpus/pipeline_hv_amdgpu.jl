...
function amdgpu_kernel_pipeline_hv_1!(i_n, u, ud, v, vd)
...
function amdgpu_kernel_pipeline_hv_2!(i_n, u, ud, v, vd, w, wd)
    __tid = (workitemIdx()).x + ((workgroupIdx()).x - 1) * (workgroupDim()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function amdgpu_kernel_pipeline_hv_3!(i_n, u, ub, ubd, ud, v, vb, vbd, vd, wb, wbd)
    __tid = (workitemIdx()).x + ((workgroupIdx()).x - 1) * (workgroupDim()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function amdgpu_kernel_pipeline_hv_4!(i_n, u, ub, ubd, ud, vb, vbd)
    __tid = (workitemIdx()).x + ((workgroupIdx()).x - 1) * (workgroupDim()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function pipeline_hv_amdgpu(loss, lossb, u, ub, v, vb, w, wb, i_n, lossd, lossbd, ud, ubd, vd, vbd, wd, wbd)
    nthread_per_block = 256
    @roc groupsize = nthread_per_block gridsize = cld(div(i_n - 1, 1) + 1, nthread_per_block) amdgpu_kernel_pipeline_hv_1!(i_n, u, ud, v, vd)
    @roc groupsize = nthread_per_block gridsize = cld(div(i_n - 1, 1) + 1, nthread_per_block) amdgpu_kernel_pipeline_hv_2!(i_n, u, ud, v, vd, w, wd)
    for i_seq_x = 1:i_n
    ...
    @roc groupsize = nthread_per_block gridsize = cld(div(i_n - 1, 1) + 1, nthread_per_block) amdgpu_kernel_pipeline_hv_3!(i_n, u, ub, ubd, ud, v, vb, vbd, vd, wb, wbd)
    @roc groupsize = nthread_per_block gridsize = cld(div(i_n - 1, 1) + 1, nthread_per_block) amdgpu_kernel_pipeline_hv_4!(i_n, u, ub, ubd, ud, vb, vbd)
    return nothing
end
