...
function jacc_kernel_normcomp_b_2!(__jacc_i, i_n, ub, vb, wb)
    i_x = 1 + (__jacc_i - 1)
...
function normcomp_b_jacc(loss, lossb, u, ub, v, vb, w, wb, i_n)
    JACC.parallel_for(div(i_n - 1, 1) + 1, jacc_kernel_normcomp_b_1!, i_n, u, v, w)
    for i_seq_x = 1:i_n
    ...
    JACC.parallel_for(div(i_n - 1, 1) + 1, jacc_kernel_normcomp_b_2!, i_n, ub, vb, wb)
    return nothing
end
