...
function amdgpu_kernel_normcomp_b_1!(i_n, u, v, w)
...
function amdgpu_kernel_normcomp_b_2!(i_n, ub, vb, wb)
    __tid = (workitemIdx()).x + ((workgroupIdx()).x - 1) * (workgroupDim()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function normcomp_b_amdgpu(loss, lossb, u, ub, v, vb, w, wb, i_n)
    nthread_per_block = 256
    @roc groupsize = nthread_per_block gridsize = cld(div(i_n - 1, 1) + 1, nthread_per_block) amdgpu_kernel_normcomp_b_1!(i_n, u, v, w)
    for i_seq_x = 1:i_n
    ...
    @roc groupsize = nthread_per_block gridsize = cld(div(i_n - 1, 1) + 1, nthread_per_block) amdgpu_kernel_normcomp_b_2!(i_n, ub, vb, wb)
    return nothing
end
