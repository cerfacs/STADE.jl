...
function metal_kernel_affine_loss_hv_1!(a, ad, b, bd, i_n, u, ud, v, vd)
...
function metal_kernel_affine_loss_hv_2!(a, ab, abd, ad, bb, bbd, i_n, u, ub, ubd, ud, vb, vbd)
    __tid = (thread_position_in_grid()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function affine_loss_hv_metal(loss, lossb, u, ub, a, ab, b, bb, v, vb, i_n, lossd, lossbd, ud, ubd, ad, abd, bd, bbd, vd, vbd)
    nthread_per_block = 256
    @metal threads = nthread_per_block groups = cld(div(i_n - 1, 1) + 1, nthread_per_block) metal_kernel_affine_loss_hv_1!(a, ad, b, bd, i_n, u, ud, v, vd)
    for i_seq_x = 1:i_n
    ...
    @metal threads = nthread_per_block groups = cld(div(i_n - 1, 1) + 1, nthread_per_block) metal_kernel_affine_loss_hv_2!(a, ab, abd, ad, bb, bbd, i_n, u, ub, ubd, ud, vb, vbd)
    return nothing
end
