...
function cuda_kernel_cond_field_choice_b_3!(i_n, u, ub, wb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_cond_field_choice_b_4!(i_n, v, vb, wb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function cond_field_choice_b_cuda(loss, lossb, u, ub, v, vb, w, wb, i_branch, i_n, branch_stack)
    ...
    if __branch == 1
        @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_cond_field_choice_b_3!(i_n, u, ub, wb)
    else
        @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_cond_field_choice_b_4!(i_n, v, vb, wb)
    end
    return nothing
end
