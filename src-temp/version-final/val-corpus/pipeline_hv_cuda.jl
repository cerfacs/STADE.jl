...
function cuda_kernel_pipeline_hv_3!(i_n, u, ub, ubd, ud, v, vb, vbd, vd, wb, wbd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_pipeline_hv_4!(i_n, u, ub, ubd, ud, vb, vbd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function pipeline_hv_cuda(loss, lossb, u, ub, v, vb, w, wb, i_n, lossd, lossbd, ud, ubd, vd, vbd, wd, wbd)
    ...
    @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_pipeline_hv_3!(i_n, u, ub, ubd, ud, v, vb, vbd, vd, wb, wbd)
    @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_pipeline_hv_4!(i_n, u, ub, ubd, ud, vb, vbd)
    return nothing
end
