...
function cuda_kernel_normcomp_b_2!(i_n, ub, vb, wb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function normcomp_b_cuda(loss, lossb, u, ub, v, vb, w, wb, i_n)
    ...
    @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_normcomp_b_2!(i_n, ub, vb, wb)
    return nothing
end
