...
function metal_kernel_mg_vcycle_d_1!(f, fd, hl2, hl2d, i_seq_level, n, r, rd, u, ud)
...
function metal_kernel_mg_vcycle_d_2!(f, fd, i_seq_level, nc, r, rd)
    __tid = (thread_position_in_grid()).x
...
function metal_kernel_mg_vcycle_d_3!(i_seq_level, nc, u, ud)
    __tid = (thread_position_in_grid()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
...
function metal_kernel_mg_vcycle_d_4!(i_seq_level, nc, u, ud)
    __tid = (thread_position_in_grid()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
    ...
    jfd = 0.0f0
    jf = j * 2
    Metal.@atomic ud[jf, i_seq_level] += ud[j, i_seq_level + 1]
    Metal.@atomic u[jf, i_seq_level] += u[j, i_seq_level + 1]
    return nothing
end

function metal_kernel_mg_vcycle_d_5!(i_seq_level, nc, u, ud)
    __tid = (thread_position_in_grid()).x
    ...
    Metal.@atomic ud[jf, i_seq_level] += 0.5f0 * (cld + crd)
...
function mg_vcycle_d_metal(u, ud, f, fd, r, rd, nfine, num_levels, h1, h1d, nu1, nu2, n)
    ...
    for i_seq_level = 1:num_levels - 1
        ...
        @metal threads = nthread_per_block groups = cld(div(n - 1, 1) + 1, nthread_per_block) metal_kernel_mg_vcycle_d_1!(f, fd, hl2, hl2d, i_seq_level, n, r, rd, u, ud)
        ncgd = 0.0f0
        ncg = div(nl, 2)
        ncd = 0.0f0
        nc = ncg - 1
        @metal threads = nthread_per_block groups = cld(div(nc - 1, 1) + 1, nthread_per_block) metal_kernel_mg_vcycle_d_2!(f, fd, i_seq_level, nc, r, rd)
        @metal threads = nthread_per_block groups = cld(div(nc - 1, 1) + 1, nthread_per_block) metal_kernel_mg_vcycle_d_3!(i_seq_level, nc, u, ud)
        nld = 0.0f0
        nl = ncg
        hld = 2.0f0hld
        hl = hl * 2.0f0
    ...
    for i_seq_level = num_levels - 1:-1:1
        ...
        hl2d = hl * hld + hl * hld
        hl2 = hl * hl
        @metal threads = nthread_per_block groups = cld(div(nc - 1, 1) + 1, nthread_per_block) metal_kernel_mg_vcycle_d_4!(i_seq_level, nc, u, ud)
        @metal threads = nthread_per_block groups = cld(div((nc + 1) - 1, 1) + 1, nthread_per_block) metal_kernel_mg_vcycle_d_5!(i_seq_level, nc, u, ud)
        for i_seq_k = 1:nu2
            for i_seq_j = 1:n
                leftd = 0.0f0
                left = 0.0f0
