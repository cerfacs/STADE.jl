...
function ttgc_b(u, ub, uref, urefb, i_cell_to_node, cell_vol, cell_volb, node_vol, skx, skxb, sky, skyb, skz, skzb, i_ncell, i_nnode, c, cb, dt, dtb, beta, betab, gamma, gammab, i_njac, res, resb, res2, res2b, up, upb, mup, npernode_half, resperio, i_node_perio, loss, lossb)
    for i_seq_node = i_nnode:-1:1
        tempb = 2 * ((u[i_seq_node] + up[i_seq_node]) - uref[i_seq_node]) * lossb[1]
        ub[i_seq_node] = ub[i_seq_node] + tempb
        upb[i_seq_node] = upb[i_seq_node] + tempb
        urefb[i_seq_node] = urefb[i_seq_node] - tempb
    end
    for i_cell = 1:i_ncell
        ...
        for i_loc = 1:4
            ...
            for i_k = 1:4
                ...
                factor = dt ^ div(2, 3) * aeresk
                i_k_node = i_cell_to_node[i_k, i_cell]
                auxres2b = res2b[i_k_node]
                auxresb = resb[i_k_node]
                factorb = gamma[i_cell] * auxres2b + beta[i_cell] * auxresb
                gammab[i_cell] = gammab[i_cell] + factor * auxres2b
                resib = resib + auxresb
                betab[i_cell] = betab[i_cell] + factor * auxresb
                dtb = dtb + (2 * dt * aeresk * factorb) / 3
                aereskb = (dt ^ 2 * factorb) / 3
            ...
            reb = cavgz * aerezb + cavgy * aereyb + cavgx * aerexb
        ...
        for i_loc = 1:4
            i_node = i_cell_to_node[i_loc, i_cell]
            cb[3, i_node] = cb[3, i_node] + cavgzb / 4
            cb[2, i_node] = cb[2, i_node] + cavgyb / 4
