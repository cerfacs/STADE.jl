...
function initstacks_transformer_b_cuda(d, dff, dk, h, n, n_layers)
...
function transformer_hv_cuda(x, xb, wq, wqb, bq, bqb, wk, wkb, bk, bkb, wv, wvb, bv, bvb, wo, wob, bo, bob, ln1_gain, ln1_gainb, ln1_bias, ln1_biasb, w1, w1b, b1, b1b, w2, w2b, b2, b2b, ln2_gain, ln2_gainb, ln2_bias, ln2_biasb, q, qb, k, kb, v, vb, scores, scoresb, probs, probsb, ctx, ctxb, attn_out, attn_outb, resid1, resid1b, normed1, normed1b, ff_hidden, ff_hiddenb, ff_out, ff_outb, resid2, resid2b, x_next, x_nextb, n, d, dk, h, dff, n_layers, eps, epsb, xd, xbd, wqd, wqbd, bqd, bqbd, wkd, wkbd, bkd, bkbd, wvd, wvbd, bvd, bvbd, wod, wobd, bod, bobd, ln1_gaind, ln1_gainbd, ln1_biasd, ln1_biasbd, w1d, w1bd, b1d, b1bd, w2d, w2bd, b2d, b2bd, ln2_gaind, ln2_gainbd, ln2_biasd, ln2_biasbd, qd, qbd, kd, kbd, vd, vbd, scoresd, scoresbd, probsd, probsbd, ctxd, ctxbd, attn_outd, attn_outbd, resid1d, resid1bd, normed1d, normed1bd, ff_hiddend, ff_hiddenbd, ff_outd, ff_outbd, resid2d, resid2bd, x_nextd, x_nextbd, epsd, epsbd, s_stack, q_stack, k_stack, v_stack, scores_stack, row_max_stack, probs_stack, row_sum_stack, ctx_stack, resid1_stack, row_mean_stack, s2_stack, diff_stack, row_var_stack, denom_stack, normed1_stack, ff_hidden_stack, resid2_stack, x_stack)
    ...
    s_stack_d = CuArray{Float64}(undef, length(s_stack))
    q_stack_d = CuArray{Float64}(undef, length(q_stack))
    k_stack_d = CuArray{Float64}(undef, length(k_stack))
    v_stack_d = CuArray{Float64}(undef, length(v_stack))
    scores_stack_d = CuArray{Float64}(undef, length(scores_stack))
    row_max_stack_d = CuArray{Float64}(undef, length(row_max_stack))
    probs_stack_d = CuArray{Float64}(undef, length(probs_stack))
    row_sum_stack_d = CuArray{Float64}(undef, length(row_sum_stack))
    ctx_stack_d = CuArray{Float64}(undef, length(ctx_stack))
    resid1_stack_d = CuArray{Float64}(undef, length(resid1_stack))
    row_mean_stack_d = CuArray{Float64}(undef, length(row_mean_stack))
    s2_stack_d = CuArray{Float64}(undef, length(s2_stack))
    diff_stack_d = CuArray{Float64}(undef, length(diff_stack))
    row_var_stack_d = CuArray{Float64}(undef, length(row_var_stack))
    denom_stack_d = CuArray{Float64}(undef, length(denom_stack))
    normed1_stack_d = CuArray{Float64}(undef, length(normed1_stack))
    ff_hidden_stack_d = CuArray{Float64}(undef, length(ff_hidden_stack))
    resid2_stack_d = CuArray{Float64}(undef, length(resid2_stack))
    x_stack_d = CuArray{Float64}(undef, length(x_stack))
    denom = 0.0
    diff = 0.0
    row_max = 0.0
    row_mean = 0.0
