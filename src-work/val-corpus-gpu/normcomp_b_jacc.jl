...
function jacc_kernel_normcomp_b_2!(__jacc_i, loss, __jgen_redval)
    Atomix.@atomic loss[1] += __jgen_redval[1]
    return nothing
end

function jacc_kernel_normcomp_b_3!(__jacc_i, i_n, lossb, w, wb)
    i_seq_x = i_n + (__jacc_i - 1) * -1
    wb[i_seq_x] = wb[i_seq_x] + (2 * w[i_seq_x]) * lossb[1]
    return nothing
end

function jacc_kernel_normcomp_b_4!(__jacc_i, i_n, ub, vb, wb)
    i_x = 1 + (__jacc_i - 1)
...
function normcomp_b_jacc(loss, lossb, u, ub, v, vb, w, wb, i_n)
    ...
    JACC.@parallel_for range = 1 jacc_kernel_normcomp_b_2!(loss, __jgen_redval_2)
    JACC.@parallel_for range = div(1 - i_n, -1) + 1 jacc_kernel_normcomp_b_3!(i_n, lossb, w, wb)
    JACC.@parallel_for range = div(i_n - 1, 1) + 1 jacc_kernel_normcomp_b_4!(i_n, ub, vb, wb)
    return nothing
end
