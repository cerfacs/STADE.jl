...
function bnd_nested_only_hv(x, xb, y, yb, i_n, i_m, out, outb, acc, accb, xd, xbd, yd, ybd, outd, outbd, accd, accbd, v_stack, out_stack)
    ...
    for i_i = 2:i_n
        for i_j = 1:i_m
            v_stack_d[((i_i - 2) * (div(i_m - 1, 1) + 1) + (i_j - 1)) + 1] = vd
            v_stack[((i_i - 2) * (div(i_m - 1, 1) + 1) + (i_j - 1)) + 1] = v
            vd = y[i_i] * xd[i_j] + x[i_j] * yd[i_i]
            v = x[i_j] * y[i_i]
            out_stack_d[((i_i - 2) * (div(i_m - 1, 1) + 1) + (i_j - 1)) + 1] = outd[i_i]
    ...
    for i_i = i_n:-1:2
        ...
        for i_j = i_m:-1:1
            outd[i_i] = out_stack_d[((i_i - 2) * (div(i_m - 1, 1) + 1) + (i_j - 1)) + 1]
            out[i_i] = out_stack[((i_i - 2) * (div(i_m - 1, 1) + 1) + (i_j - 1)) + 1]
            vbd = vbd + (outb[i_i] * vd + v * outbd[i_i])
            vb = vb + v * outb[i_i]
            vbd = vbd + (outb[i_i] * vd + v * outbd[i_i])
            vb = vb + v * outb[i_i]
            vd = v_stack_d[((i_i - 2) * (div(i_m - 1, 1) + 1) + (i_j - 1)) + 1]
