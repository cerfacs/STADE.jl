function initstacks_unet_b(c1, c2, c3, h, w)
    two = 2
    h2 = div(h, two)
    w2 = div(w, two)
    hw2 = h2 * w2
    four = 4
    h4 = div(h, four)
    w4 = div(w, four)
    hw4 = h4 * w4
    n_b_mid = c3 * hw4
    n_b_out = c3 * hw4
    hw = h * w
    n_d1_mid = c1 * hw
    n_d1_out = c1 * hw
    n_d2_mid = c2 * hw2
    n_d2_out = c2 * hw2
    n_e1_mid = c1 * hw
    n_e1_out = c1 * hw
    n_e2_mid = c2 * hw2
    n_e2_out = c2 * hw2
    t_e1_stack = Vector{Float64}(undef, max(0, div(n_e1_mid - 1, 1) + 1))
    skip1_stack = Vector{Float64}(undef, max(0, div(n_e1_out - 1, 1) + 1))
    t_e2_stack = Vector{Float64}(undef, max(0, div(n_e2_mid - 1, 1) + 1))
    skip2_stack = Vector{Float64}(undef, max(0, div(n_e2_out - 1, 1) + 1))
    t_b_stack = Vector{Float64}(undef, max(0, div(n_b_mid - 1, 1) + 1))
    bott_stack = Vector{Float64}(undef, max(0, div(n_b_out - 1, 1) + 1))
    t_d2_stack = Vector{Float64}(undef, max(0, div(n_d2_mid - 1, 1) + 1))
    dec2out_stack = Vector{Float64}(undef, max(0, div(n_d2_out - 1, 1) + 1))
    t_d1_stack = Vector{Float64}(undef, max(0, div(n_d1_mid - 1, 1) + 1))
    dec1out_stack = Vector{Float64}(undef, max(0, div(n_d1_out - 1, 1) + 1))
    return (t_e1_stack, skip1_stack, t_e2_stack, skip2_stack, t_b_stack, bott_stack, t_d2_stack, dec2out_stack, t_d1_stack, dec1out_stack)
end

function unet_hv(x, xb, h, w, c_in, c1, c2, c3, c_out, w_e1a, w_e1ab, b_e1a, b_e1ab, w_e1b, w_e1bb, b_e1b, b_e1bb, w_e2a, w_e2ab, b_e2a, b_e2ab, w_e2b, w_e2bb, b_e2b, b_e2bb, w_ba, w_bab, b_ba, b_bab, w_bb, w_bbb, b_bb, b_bbb, w_d2a, w_d2ab, b_d2a, b_d2ab, w_d2b, w_d2bb, b_d2b, b_d2bb, w_d1a, w_d1ab, b_d1a, b_d1ab, w_d1b, w_d1bb, b_d1b, b_d1bb, w_out, w_outb, b_out, b_outb, xpad0, xpad0b, t_e1, t_e1b, t_e1pad, t_e1padb, skip1, skip1b, p1, p1b, p1pad, p1padb, t_e2, t_e2b, t_e2pad, t_e2padb, skip2, skip2b, p2, p2b, p2pad, p2padb, t_b, t_bb, t_bpad, t_bpadb, bott, bottb, u2, u2b, cat2, cat2b, cat2pad, cat2padb, t_d2, t_d2b, t_d2pad, t_d2padb, dec2out, dec2outb, u1, u1b, cat1, cat1b, cat1pad, cat1padb, t_d1, t_d1b, t_d1pad, t_d1padb, dec1out, dec1outb, y, yb, xd, xbd, w_e1ad, w_e1abd, b_e1ad, b_e1abd, w_e1bd, w_e1bbd, b_e1bd, b_e1bbd, w_e2ad, w_e2abd, b_e2ad, b_e2abd, w_e2bd, w_e2bbd, b_e2bd, b_e2bbd, w_bad, w_babd, b_bad, b_babd, w_bbd, w_bbbd, b_bbd, b_bbbd, w_d2ad, w_d2abd, b_d2ad, b_d2abd, w_d2bd, w_d2bbd, b_d2bd, b_d2bbd, w_d1ad, w_d1abd, b_d1ad, b_d1abd, w_d1bd, w_d1bbd, b_d1bd, b_d1bbd, w_outd, w_outbd, b_outd, b_outbd, xpad0d, xpad0bd, t_e1d, t_e1bd, t_e1padd, t_e1padbd, skip1d, skip1bd, p1d, p1bd, p1padd, p1padbd, t_e2d, t_e2bd, t_e2padd, t_e2padbd, skip2d, skip2bd, p2d, p2bd, p2padd, p2padbd, t_bd, t_bbd, t_bpadd, t_bpadbd, bottd, bottbd, u2d, u2bd, cat2d, cat2bd, cat2padd, cat2padbd, t_d2d, t_d2bd, t_d2padd, t_d2padbd, dec2outd, dec2outbd, u1d, u1bd, cat1d, cat1bd, cat1padd, cat1padbd, t_d1d, t_d1bd, t_d1padd, t_d1padbd, dec1outd, dec1outbd, yd, ybd, t_e1_stack, skip1_stack, t_e2_stack, skip2_stack, t_b_stack, bott_stack, t_d2_stack, dec2out_stack, t_d1_stack, dec1out_stack)
    t_e1_stack_d = Vector{Float64}(undef, length(t_e1_stack))
    skip1_stack_d = Vector{Float64}(undef, length(skip1_stack))
    t_e2_stack_d = Vector{Float64}(undef, length(t_e2_stack))
    skip2_stack_d = Vector{Float64}(undef, length(skip2_stack))
    t_b_stack_d = Vector{Float64}(undef, length(t_b_stack))
    bott_stack_d = Vector{Float64}(undef, length(bott_stack))
    t_d2_stack_d = Vector{Float64}(undef, length(t_d2_stack))
    dec2out_stack_d = Vector{Float64}(undef, length(dec2out_stack))
    t_d1_stack_d = Vector{Float64}(undef, length(t_d1_stack))
    dec1out_stack_d = Vector{Float64}(undef, length(dec1out_stack))
    a11 = 0.0
    a12 = 0.0
    a21 = 0.0
    a22 = 0.0
    m1 = 0.0
    m2 = 0.0
    s = 0.0
    a11b = 0.0
    a12b = 0.0
    a21b = 0.0
    a22b = 0.0
    m1b = 0.0
    m2b = 0.0
    sb = 0.0
    a11d = 0.0
    a11bd = 0.0
    a12d = 0.0
    a12bd = 0.0
    a21d = 0.0
    a21bd = 0.0
    a22d = 0.0
    a22bd = 0.0
    m1d = 0.0
    m1bd = 0.0
    m2d = 0.0
    m2bd = 0.0
    sd = 0.0
    sbd = 0.0
    zero_vald = 0.0
    zero_valbd = 0.0
    two = 2
    four = 4
    h2 = div(h, two)
    w2 = div(w, two)
    h4 = div(h, four)
    w4 = div(w, four)
    hp1 = h + 2
    wp1 = w + 2
    hp2 = h2 + 2
    wp2 = w2 + 2
    hp4 = h4 + 2
    wp4 = w4 + 2
    pad = 1
    kh = 3
    kw = 3
    khkw = kh * kw
    kh_out = 1
    kw_out = 1
    khkw_out = kh_out * kw_out
    scale = 2
    zero_vald = 0.0
    zero_val = 0.0
    c32 = c3 + c2
    c21 = c2 + c1
    hw = h * w
    hw2 = h2 * w2
    hw4 = h4 * w4
    n_xpad0 = c_in * hp1 * wp1
    __icse_0 = c1 * hw
    n_e1_mid = __icse_0
    __icse_1 = c1 * hp1 * wp1
    n_e1_midpad = __icse_1
    n_e1_out = __icse_0
    n_p1pad = c1 * hp2 * wp2
    __icse_2 = c2 * hw2
    n_e2_mid = __icse_2
    __icse_3 = c2 * hp2 * wp2
    n_e2_midpad = __icse_3
    n_e2_out = __icse_2
    n_p2pad = c2 * hp4 * wp4
    __icse_4 = c3 * hw4
    n_b_mid = __icse_4
    n_b_midpad = c3 * hp4 * wp4
    n_b_out = __icse_4
    n_cat2pad = c32 * hp2 * wp2
    n_d2_mid = __icse_2
    n_d2_midpad = __icse_3
    n_d2_out = __icse_2
    n_cat1pad = c21 * hp1 * wp1
    n_d1_mid = __icse_0
    n_d1_midpad = __icse_1
    n_d1_out = __icse_0
    for i = 1:n_xpad0
        xpad0d[i] = zero_vald
        xpad0[i] = zero_val
    end
    for idx = 1:c_in * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        xpad0d[yi] = xd[idx]
        xpad0[yi] = x[idx]
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c_in * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_5 = ci - 1
            xi = __icse_5 * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c_in + __icse_5) * kh + (ki - 1)) * kw + kj
            __hcse_0 = w_e1a[wi]
            __hcse_1 = xpad0[xi]
            sd = sd + (__hcse_0 * xpad0d[xi] + __hcse_1 * w_e1ad[wi])
            s = s + __hcse_1 * __hcse_0
        end
        t_e1d[idx] = sd + b_e1ad[co]
        t_e1[idx] = s + b_e1a[co]
    end
    for i = 1:n_e1_mid
        __idx_t_e1_stack_0 = (i - 1) + 1
        __cse_6d = t_e1d[i]
        __cse_6 = t_e1[i]
        t_e1_stack_d[__idx_t_e1_stack_0] = __cse_6d
        t_e1_stack[__idx_t_e1_stack_0] = __cse_6
        t_e1d[i] = (0.5 * (1.0 + sign(__cse_6 - zero_val))) * __cse_6d + (0.5 * (1.0 + sign(zero_val - __cse_6))) * zero_vald
        t_e1[i] = max(__cse_6, zero_val)
    end
    for i = 1:n_e1_midpad
        t_e1padd[i] = zero_vald
        t_e1pad[i] = zero_val
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        t_e1padd[yi] = t_e1d[idx]
        t_e1pad[yi] = t_e1[idx]
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c1 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_7 = ci - 1
            xi = __icse_7 * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c1 + __icse_7) * kh + (ki - 1)) * kw + kj
            __hcse_2 = w_e1b[wi]
            __hcse_3 = t_e1pad[xi]
            sd = sd + (__hcse_2 * t_e1padd[xi] + __hcse_3 * w_e1bd[wi])
            s = s + __hcse_3 * __hcse_2
        end
        skip1d[idx] = sd + b_e1bd[co]
        skip1[idx] = s + b_e1b[co]
    end
    for i = 1:n_e1_out
        __idx_skip1_stack_0 = (i - 1) + 1
        __cse_8d = skip1d[i]
        __cse_8 = skip1[i]
        skip1_stack_d[__idx_skip1_stack_0] = __cse_8d
        skip1_stack[__idx_skip1_stack_0] = __cse_8
        skip1d[i] = (0.5 * (1.0 + sign(__cse_8 - zero_val))) * __cse_8d + (0.5 * (1.0 + sign(zero_val - __cse_8))) * zero_vald
        skip1[i] = max(__cse_8, zero_val)
    end
    for idx = 1:c1 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        oi = div(rem, w2) + 1
        oj = mod(rem, w2) + 1
        i0 = 2oi - 1
        j0 = 2oj - 1
        a11d = skip1d[(ci - 1) * hw + (i0 - 1) * w + j0]
        a11 = skip1[(ci - 1) * hw + (i0 - 1) * w + j0]
        a12d = skip1d[(ci - 1) * hw + (i0 - 1) * w + j0 + 1]
        a12 = skip1[(ci - 1) * hw + (i0 - 1) * w + j0 + 1]
        a21d = skip1d[(ci - 1) * hw + i0 * w + j0]
        a21 = skip1[(ci - 1) * hw + i0 * w + j0]
        a22d = skip1d[(ci - 1) * hw + i0 * w + j0 + 1]
        a22 = skip1[(ci - 1) * hw + i0 * w + j0 + 1]
        m1d = (0.5 * (1.0 + sign(a11 - a12))) * a11d + (0.5 * (1.0 + sign(a12 - a11))) * a12d
        m1 = max(a11, a12)
        m2d = (0.5 * (1.0 + sign(a21 - a22))) * a21d + (0.5 * (1.0 + sign(a22 - a21))) * a22d
        m2 = max(a21, a22)
        p1d[idx] = (0.5 * (1.0 + sign(m1 - m2))) * m1d + (0.5 * (1.0 + sign(m2 - m1))) * m2d
        p1[idx] = max(m1, m2)
    end
    for i = 1:n_p1pad
        p1padd[i] = zero_vald
        p1pad[i] = zero_val
    end
    for idx = 1:c1 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        p1padd[yi] = p1d[idx]
        p1pad[yi] = p1[idx]
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c1 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_9 = ci - 1
            xi = __icse_9 * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c1 + __icse_9) * kh + (ki - 1)) * kw + kj
            __hcse_4 = w_e2a[wi]
            __hcse_5 = p1pad[xi]
            sd = sd + (__hcse_4 * p1padd[xi] + __hcse_5 * w_e2ad[wi])
            s = s + __hcse_5 * __hcse_4
        end
        t_e2d[idx] = sd + b_e2ad[co]
        t_e2[idx] = s + b_e2a[co]
    end
    for i = 1:n_e2_mid
        __idx_t_e2_stack_0 = (i - 1) + 1
        __cse_10d = t_e2d[i]
        __cse_10 = t_e2[i]
        t_e2_stack_d[__idx_t_e2_stack_0] = __cse_10d
        t_e2_stack[__idx_t_e2_stack_0] = __cse_10
        t_e2d[i] = (0.5 * (1.0 + sign(__cse_10 - zero_val))) * __cse_10d + (0.5 * (1.0 + sign(zero_val - __cse_10))) * zero_vald
        t_e2[i] = max(__cse_10, zero_val)
    end
    for i = 1:n_e2_midpad
        t_e2padd[i] = zero_vald
        t_e2pad[i] = zero_val
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        t_e2padd[yi] = t_e2d[idx]
        t_e2pad[yi] = t_e2[idx]
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c2 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_11 = ci - 1
            xi = __icse_11 * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c2 + __icse_11) * kh + (ki - 1)) * kw + kj
            __hcse_6 = w_e2b[wi]
            __hcse_7 = t_e2pad[xi]
            sd = sd + (__hcse_6 * t_e2padd[xi] + __hcse_7 * w_e2bd[wi])
            s = s + __hcse_7 * __hcse_6
        end
        skip2d[idx] = sd + b_e2bd[co]
        skip2[idx] = s + b_e2b[co]
    end
    for i = 1:n_e2_out
        __idx_skip2_stack_0 = (i - 1) + 1
        __cse_12d = skip2d[i]
        __cse_12 = skip2[i]
        skip2_stack_d[__idx_skip2_stack_0] = __cse_12d
        skip2_stack[__idx_skip2_stack_0] = __cse_12
        skip2d[i] = (0.5 * (1.0 + sign(__cse_12 - zero_val))) * __cse_12d + (0.5 * (1.0 + sign(zero_val - __cse_12))) * zero_vald
        skip2[i] = max(__cse_12, zero_val)
    end
    for idx = 1:c2 * hw4
        idxm1 = idx - 1
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        oi = div(rem, w4) + 1
        oj = mod(rem, w4) + 1
        i0 = 2oi - 1
        j0 = 2oj - 1
        a11d = skip2d[(ci - 1) * hw2 + (i0 - 1) * w2 + j0]
        a11 = skip2[(ci - 1) * hw2 + (i0 - 1) * w2 + j0]
        a12d = skip2d[(ci - 1) * hw2 + (i0 - 1) * w2 + j0 + 1]
        a12 = skip2[(ci - 1) * hw2 + (i0 - 1) * w2 + j0 + 1]
        a21d = skip2d[(ci - 1) * hw2 + i0 * w2 + j0]
        a21 = skip2[(ci - 1) * hw2 + i0 * w2 + j0]
        a22d = skip2d[(ci - 1) * hw2 + i0 * w2 + j0 + 1]
        a22 = skip2[(ci - 1) * hw2 + i0 * w2 + j0 + 1]
        m1d = (0.5 * (1.0 + sign(a11 - a12))) * a11d + (0.5 * (1.0 + sign(a12 - a11))) * a12d
        m1 = max(a11, a12)
        m2d = (0.5 * (1.0 + sign(a21 - a22))) * a21d + (0.5 * (1.0 + sign(a22 - a21))) * a22d
        m2 = max(a21, a22)
        p2d[idx] = (0.5 * (1.0 + sign(m1 - m2))) * m1d + (0.5 * (1.0 + sign(m2 - m1))) * m2d
        p2[idx] = max(m1, m2)
    end
    for i = 1:n_p2pad
        p2padd[i] = zero_vald
        p2pad[i] = zero_val
    end
    for idx = 1:c2 * hw4
        idxm1 = idx - 1
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        yi = (ci - 1) * hp4 * wp4 + ((i + pad) - 1) * wp4 + (j + pad)
        p2padd[yi] = p2d[idx]
        p2pad[yi] = p2[idx]
    end
    for idx = 1:c3 * hw4
        idxm1 = idx - 1
        co = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c2 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_13 = ci - 1
            xi = __icse_13 * hp4 * wp4 + (row - 1) * wp4 + col
            wi = (((co - 1) * c2 + __icse_13) * kh + (ki - 1)) * kw + kj
            __hcse_8 = w_ba[wi]
            __hcse_9 = p2pad[xi]
            sd = sd + (__hcse_8 * p2padd[xi] + __hcse_9 * w_bad[wi])
            s = s + __hcse_9 * __hcse_8
        end
        t_bd[idx] = sd + b_bad[co]
        t_b[idx] = s + b_ba[co]
    end
    for i = 1:n_b_mid
        __idx_t_b_stack_0 = (i - 1) + 1
        __cse_14d = t_bd[i]
        __cse_14 = t_b[i]
        t_b_stack_d[__idx_t_b_stack_0] = __cse_14d
        t_b_stack[__idx_t_b_stack_0] = __cse_14
        t_bd[i] = (0.5 * (1.0 + sign(__cse_14 - zero_val))) * __cse_14d + (0.5 * (1.0 + sign(zero_val - __cse_14))) * zero_vald
        t_b[i] = max(__cse_14, zero_val)
    end
    for i = 1:n_b_midpad
        t_bpadd[i] = zero_vald
        t_bpad[i] = zero_val
    end
    for idx = 1:c3 * hw4
        idxm1 = idx - 1
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        yi = (ci - 1) * hp4 * wp4 + ((i + pad) - 1) * wp4 + (j + pad)
        t_bpadd[yi] = t_bd[idx]
        t_bpad[yi] = t_b[idx]
    end
    for idx = 1:c3 * hw4
        idxm1 = idx - 1
        co = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c3 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_15 = ci - 1
            xi = __icse_15 * hp4 * wp4 + (row - 1) * wp4 + col
            wi = (((co - 1) * c3 + __icse_15) * kh + (ki - 1)) * kw + kj
            __hcse_10 = w_bb[wi]
            __hcse_11 = t_bpad[xi]
            sd = sd + (__hcse_10 * t_bpadd[xi] + __hcse_11 * w_bbd[wi])
            s = s + __hcse_11 * __hcse_10
        end
        bottd[idx] = sd + b_bbd[co]
        bott[idx] = s + b_bb[co]
    end
    for i = 1:n_b_out
        __idx_bott_stack_0 = (i - 1) + 1
        __cse_16d = bottd[i]
        __cse_16 = bott[i]
        bott_stack_d[__idx_bott_stack_0] = __cse_16d
        bott_stack[__idx_bott_stack_0] = __cse_16
        bottd[i] = (0.5 * (1.0 + sign(__cse_16 - zero_val))) * __cse_16d + (0.5 * (1.0 + sign(zero_val - __cse_16))) * zero_vald
        bott[i] = max(__cse_16, zero_val)
    end
    for idx = 1:c3 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        oi = div(rem, w2) + 1
        oj = mod(rem, w2) + 1
        oim1 = oi - 1
        ojm1 = oj - 1
        i = div(oim1, scale) + 1
        j = div(ojm1, scale) + 1
        xi = (ci - 1) * hw4 + (i - 1) * w4 + j
        u2d[idx] = bottd[xi]
        u2[idx] = bott[xi]
    end
    for idx = 1:c3 * hw2
        cat2d[idx] = u2d[idx]
        cat2[idx] = u2[idx]
    end
    for idx = 1:c2 * hw2
        cat2d[c3 * hw2 + idx] = skip2d[idx]
        cat2[c3 * hw2 + idx] = skip2[idx]
    end
    for i = 1:n_cat2pad
        cat2padd[i] = zero_vald
        cat2pad[i] = zero_val
    end
    for idx = 1:c32 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        cat2padd[yi] = cat2d[idx]
        cat2pad[yi] = cat2[idx]
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c32 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_17 = ci - 1
            xi = __icse_17 * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c32 + __icse_17) * kh + (ki - 1)) * kw + kj
            __hcse_12 = w_d2a[wi]
            __hcse_13 = cat2pad[xi]
            sd = sd + (__hcse_12 * cat2padd[xi] + __hcse_13 * w_d2ad[wi])
            s = s + __hcse_13 * __hcse_12
        end
        t_d2d[idx] = sd + b_d2ad[co]
        t_d2[idx] = s + b_d2a[co]
    end
    for i = 1:n_d2_mid
        __idx_t_d2_stack_0 = (i - 1) + 1
        __cse_18d = t_d2d[i]
        __cse_18 = t_d2[i]
        t_d2_stack_d[__idx_t_d2_stack_0] = __cse_18d
        t_d2_stack[__idx_t_d2_stack_0] = __cse_18
        t_d2d[i] = (0.5 * (1.0 + sign(__cse_18 - zero_val))) * __cse_18d + (0.5 * (1.0 + sign(zero_val - __cse_18))) * zero_vald
        t_d2[i] = max(__cse_18, zero_val)
    end
    for i = 1:n_d2_midpad
        t_d2padd[i] = zero_vald
        t_d2pad[i] = zero_val
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        t_d2padd[yi] = t_d2d[idx]
        t_d2pad[yi] = t_d2[idx]
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c2 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_19 = ci - 1
            xi = __icse_19 * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c2 + __icse_19) * kh + (ki - 1)) * kw + kj
            __hcse_14 = w_d2b[wi]
            __hcse_15 = t_d2pad[xi]
            sd = sd + (__hcse_14 * t_d2padd[xi] + __hcse_15 * w_d2bd[wi])
            s = s + __hcse_15 * __hcse_14
        end
        dec2outd[idx] = sd + b_d2bd[co]
        dec2out[idx] = s + b_d2b[co]
    end
    for i = 1:n_d2_out
        __idx_dec2out_stack_0 = (i - 1) + 1
        __cse_20d = dec2outd[i]
        __cse_20 = dec2out[i]
        dec2out_stack_d[__idx_dec2out_stack_0] = __cse_20d
        dec2out_stack[__idx_dec2out_stack_0] = __cse_20
        dec2outd[i] = (0.5 * (1.0 + sign(__cse_20 - zero_val))) * __cse_20d + (0.5 * (1.0 + sign(zero_val - __cse_20))) * zero_vald
        dec2out[i] = max(__cse_20, zero_val)
    end
    for idx = 1:c2 * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        oi = div(rem, w) + 1
        oj = mod(rem, w) + 1
        oim1 = oi - 1
        ojm1 = oj - 1
        i = div(oim1, scale) + 1
        j = div(ojm1, scale) + 1
        xi = (ci - 1) * hw2 + (i - 1) * w2 + j
        u1d[idx] = dec2outd[xi]
        u1[idx] = dec2out[xi]
    end
    for idx = 1:c2 * hw
        cat1d[idx] = u1d[idx]
        cat1[idx] = u1[idx]
    end
    for idx = 1:c1 * hw
        cat1d[c2 * hw + idx] = skip1d[idx]
        cat1[c2 * hw + idx] = skip1[idx]
    end
    for i = 1:n_cat1pad
        cat1padd[i] = zero_vald
        cat1pad[i] = zero_val
    end
    for idx = 1:c21 * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        cat1padd[yi] = cat1d[idx]
        cat1pad[yi] = cat1[idx]
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c21 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_21 = ci - 1
            xi = __icse_21 * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c21 + __icse_21) * kh + (ki - 1)) * kw + kj
            __hcse_16 = w_d1a[wi]
            __hcse_17 = cat1pad[xi]
            sd = sd + (__hcse_16 * cat1padd[xi] + __hcse_17 * w_d1ad[wi])
            s = s + __hcse_17 * __hcse_16
        end
        t_d1d[idx] = sd + b_d1ad[co]
        t_d1[idx] = s + b_d1a[co]
    end
    for i = 1:n_d1_mid
        __idx_t_d1_stack_0 = (i - 1) + 1
        __cse_22d = t_d1d[i]
        __cse_22 = t_d1[i]
        t_d1_stack_d[__idx_t_d1_stack_0] = __cse_22d
        t_d1_stack[__idx_t_d1_stack_0] = __cse_22
        t_d1d[i] = (0.5 * (1.0 + sign(__cse_22 - zero_val))) * __cse_22d + (0.5 * (1.0 + sign(zero_val - __cse_22))) * zero_vald
        t_d1[i] = max(__cse_22, zero_val)
    end
    for i = 1:n_d1_midpad
        t_d1padd[i] = zero_vald
        t_d1pad[i] = zero_val
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        t_d1padd[yi] = t_d1d[idx]
        t_d1pad[yi] = t_d1[idx]
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c1 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_23 = ci - 1
            xi = __icse_23 * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c1 + __icse_23) * kh + (ki - 1)) * kw + kj
            __hcse_18 = w_d1b[wi]
            __hcse_19 = t_d1pad[xi]
            sd = sd + (__hcse_18 * t_d1padd[xi] + __hcse_19 * w_d1bd[wi])
            s = s + __hcse_19 * __hcse_18
        end
        dec1outd[idx] = sd + b_d1bd[co]
        dec1out[idx] = s + b_d1b[co]
    end
    for i = 1:n_d1_out
        __idx_dec1out_stack_0 = (i - 1) + 1
        __cse_24d = dec1outd[i]
        __cse_24 = dec1out[i]
        dec1out_stack_d[__idx_dec1out_stack_0] = __cse_24d
        dec1out_stack[__idx_dec1out_stack_0] = __cse_24
        dec1outd[i] = (0.5 * (1.0 + sign(__cse_24 - zero_val))) * __cse_24d + (0.5 * (1.0 + sign(zero_val - __cse_24))) * zero_vald
        dec1out[i] = max(__cse_24, zero_val)
    end
    for idx = 1:c_out * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        sd = 0.0
        s = 0.0
        for i_k = 1:c1 * khkw_out
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw_out) + 1
            rem2 = mod(kseqm1, khkw_out)
            ki = div(rem2, kw_out) + 1
            kj = mod(rem2, kw_out) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_25 = ci - 1
            xi = __icse_25 * hw + (row - 1) * w + col
            wi = (((co - 1) * c1 + __icse_25) * kh_out + (ki - 1)) * kw_out + kj
            __hcse_20 = w_out[wi]
            __hcse_21 = dec1out[xi]
            sd = sd + (__hcse_20 * dec1outd[xi] + __hcse_21 * w_outd[wi])
            s = s + __hcse_21 * __hcse_20
        end
        yd[idx] = sd + b_outd[co]
        y[idx] = s + b_out[co]
    end
    two = 2
    four = 4
    h2 = div(h, two)
    w2 = div(w, two)
    h4 = div(h, four)
    w4 = div(w, four)
    hp1 = h + 2
    wp1 = w + 2
    hp2 = h2 + 2
    wp2 = w2 + 2
    hp4 = h4 + 2
    wp4 = w4 + 2
    pad = 1
    kh = 3
    kw = 3
    khkw = kh * kw
    kh_out = 1
    kw_out = 1
    khkw_out = kh_out * kw_out
    scale = 2
    c32 = c3 + c2
    c21 = c2 + c1
    hw = h * w
    hw2 = h2 * w2
    hw4 = h4 * w4
    n_xpad0 = c_in * hp1 * wp1
    __icse_26 = c1 * hw
    n_e1_mid = __icse_26
    __icse_27 = c1 * hp1 * wp1
    n_e1_midpad = __icse_27
    n_e1_out = __icse_26
    n_p1pad = c1 * hp2 * wp2
    __icse_28 = c2 * hw2
    n_e2_mid = __icse_28
    __icse_29 = c2 * hp2 * wp2
    n_e2_midpad = __icse_29
    n_e2_out = __icse_28
    n_p2pad = c2 * hp4 * wp4
    __icse_30 = c3 * hw4
    n_b_mid = __icse_30
    n_b_midpad = c3 * hp4 * wp4
    n_b_out = __icse_30
    n_cat2pad = c32 * hp2 * wp2
    n_d2_mid = __icse_28
    n_d2_midpad = __icse_29
    n_d2_out = __icse_28
    n_cat1pad = c21 * hp1 * wp1
    n_d1_mid = __icse_26
    n_d1_midpad = __icse_27
    n_d1_out = __icse_26
    for idx = c_out * hw:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        __oldb_0d = ybd[idx]
        __oldb_0 = yb[idx]
        ybd[idx] = 0.0
        yb[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_outbd[co] = b_outbd[co] + __oldb_0d
        b_outb[co] = b_outb[co] + __oldb_0
        for i_k = c1 * khkw_out:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw_out) + 1
            rem2 = mod(kseqm1, khkw_out)
            ki = div(rem2, kw_out) + 1
            kj = mod(rem2, kw_out) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_31 = ci - 1
            xi = __icse_31 * hw + (row - 1) * w + col
            wi = (((co - 1) * c1 + __icse_31) * kh_out + (ki - 1)) * kw_out + kj
            __hcse_22 = w_out[wi]
            dec1outbd[xi] = dec1outbd[xi] + (sb * w_outd[wi] + __hcse_22 * sbd)
            dec1outb[xi] = dec1outb[xi] + __hcse_22 * sb
            __hcse_23 = dec1out[xi]
            w_outbd[wi] = w_outbd[wi] + (sb * dec1outd[xi] + __hcse_23 * sbd)
            w_outb[wi] = w_outb[wi] + __hcse_23 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for i = n_d1_out:-1:1
        __idx_dec1out_stack_0 = (i - 1) + 1
        dec1outd[i] = dec1out_stack_d[__idx_dec1out_stack_0]
        dec1out[i] = dec1out_stack[__idx_dec1out_stack_0]
        __oldb_2d = dec1outbd[i]
        __oldb_2 = dec1outb[i]
        dec1outbd[i] = 0.0
        dec1outb[i] = 0.0
        __hcse_24 = 0.5 * (1.0 + sign(dec1out[i] - zero_val))
        dec1outbd[i] = dec1outbd[i] + __hcse_24 * __oldb_2d
        dec1outb[i] = dec1outb[i] + __hcse_24 * __oldb_2
    end
    for idx = c1 * hw:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        __oldb_0d = dec1outbd[idx]
        __oldb_0 = dec1outb[idx]
        dec1outbd[idx] = 0.0
        dec1outb[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_d1bbd[co] = b_d1bbd[co] + __oldb_0d
        b_d1bb[co] = b_d1bb[co] + __oldb_0
        for i_k = c1 * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_32 = ci - 1
            xi = __icse_32 * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c1 + __icse_32) * kh + (ki - 1)) * kw + kj
            __hcse_25 = w_d1b[wi]
            t_d1padbd[xi] = t_d1padbd[xi] + (sb * w_d1bd[wi] + __hcse_25 * sbd)
            t_d1padb[xi] = t_d1padb[xi] + __hcse_25 * sb
            __hcse_26 = t_d1pad[xi]
            w_d1bbd[wi] = w_d1bbd[wi] + (sb * t_d1padd[xi] + __hcse_26 * sbd)
            w_d1bb[wi] = w_d1bb[wi] + __hcse_26 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c1 * hw:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        __oldb_0d = t_d1padbd[yi]
        __oldb_0 = t_d1padb[yi]
        t_d1padbd[yi] = 0.0
        t_d1padb[yi] = 0.0
        t_d1bd[idx] = t_d1bd[idx] + __oldb_0d
        t_d1b[idx] = t_d1b[idx] + __oldb_0
    end
    for i = n_d1_midpad:-1:1
        t_d1padbd[i] = 0.0
        t_d1padb[i] = 0.0
    end
    for i = n_d1_mid:-1:1
        __idx_t_d1_stack_0 = (i - 1) + 1
        t_d1d[i] = t_d1_stack_d[__idx_t_d1_stack_0]
        t_d1[i] = t_d1_stack[__idx_t_d1_stack_0]
        __oldb_2d = t_d1bd[i]
        __oldb_2 = t_d1b[i]
        t_d1bd[i] = 0.0
        t_d1b[i] = 0.0
        __hcse_27 = 0.5 * (1.0 + sign(t_d1[i] - zero_val))
        t_d1bd[i] = t_d1bd[i] + __hcse_27 * __oldb_2d
        t_d1b[i] = t_d1b[i] + __hcse_27 * __oldb_2
    end
    for idx = c1 * hw:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        __oldb_0d = t_d1bd[idx]
        __oldb_0 = t_d1b[idx]
        t_d1bd[idx] = 0.0
        t_d1b[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_d1abd[co] = b_d1abd[co] + __oldb_0d
        b_d1ab[co] = b_d1ab[co] + __oldb_0
        for i_k = c21 * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_33 = ci - 1
            xi = __icse_33 * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c21 + __icse_33) * kh + (ki - 1)) * kw + kj
            __hcse_28 = w_d1a[wi]
            cat1padbd[xi] = cat1padbd[xi] + (sb * w_d1ad[wi] + __hcse_28 * sbd)
            cat1padb[xi] = cat1padb[xi] + __hcse_28 * sb
            __hcse_29 = cat1pad[xi]
            w_d1abd[wi] = w_d1abd[wi] + (sb * cat1padd[xi] + __hcse_29 * sbd)
            w_d1ab[wi] = w_d1ab[wi] + __hcse_29 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c21 * hw:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        __oldb_0d = cat1padbd[yi]
        __oldb_0 = cat1padb[yi]
        cat1padbd[yi] = 0.0
        cat1padb[yi] = 0.0
        cat1bd[idx] = cat1bd[idx] + __oldb_0d
        cat1b[idx] = cat1b[idx] + __oldb_0
    end
    for i = n_cat1pad:-1:1
        cat1padbd[i] = 0.0
        cat1padb[i] = 0.0
    end
    for idx = c1 * hw:-1:1
        __oldb_0d = cat1bd[c2 * hw + idx]
        __oldb_0 = cat1b[c2 * hw + idx]
        cat1bd[c2 * hw + idx] = 0.0
        cat1b[c2 * hw + idx] = 0.0
        skip1bd[idx] = skip1bd[idx] + __oldb_0d
        skip1b[idx] = skip1b[idx] + __oldb_0
    end
    for idx = c2 * hw:-1:1
        __oldb_0d = cat1bd[idx]
        __oldb_0 = cat1b[idx]
        cat1bd[idx] = 0.0
        cat1b[idx] = 0.0
        u1bd[idx] = u1bd[idx] + __oldb_0d
        u1b[idx] = u1b[idx] + __oldb_0
    end
    for idx = c2 * hw:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        oi = div(rem, w) + 1
        oj = mod(rem, w) + 1
        oim1 = oi - 1
        ojm1 = oj - 1
        i = div(oim1, scale) + 1
        j = div(ojm1, scale) + 1
        xi = (ci - 1) * hw2 + (i - 1) * w2 + j
        __oldb_0d = u1bd[idx]
        __oldb_0 = u1b[idx]
        u1bd[idx] = 0.0
        u1b[idx] = 0.0
        dec2outbd[xi] = dec2outbd[xi] + __oldb_0d
        dec2outb[xi] = dec2outb[xi] + __oldb_0
    end
    for i = n_d2_out:-1:1
        __idx_dec2out_stack_0 = (i - 1) + 1
        dec2outd[i] = dec2out_stack_d[__idx_dec2out_stack_0]
        dec2out[i] = dec2out_stack[__idx_dec2out_stack_0]
        __oldb_2d = dec2outbd[i]
        __oldb_2 = dec2outb[i]
        dec2outbd[i] = 0.0
        dec2outb[i] = 0.0
        __hcse_30 = 0.5 * (1.0 + sign(dec2out[i] - zero_val))
        dec2outbd[i] = dec2outbd[i] + __hcse_30 * __oldb_2d
        dec2outb[i] = dec2outb[i] + __hcse_30 * __oldb_2
    end
    for idx = c2 * hw2:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        __oldb_0d = dec2outbd[idx]
        __oldb_0 = dec2outb[idx]
        dec2outbd[idx] = 0.0
        dec2outb[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_d2bbd[co] = b_d2bbd[co] + __oldb_0d
        b_d2bb[co] = b_d2bb[co] + __oldb_0
        for i_k = c2 * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_34 = ci - 1
            xi = __icse_34 * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c2 + __icse_34) * kh + (ki - 1)) * kw + kj
            __hcse_31 = w_d2b[wi]
            t_d2padbd[xi] = t_d2padbd[xi] + (sb * w_d2bd[wi] + __hcse_31 * sbd)
            t_d2padb[xi] = t_d2padb[xi] + __hcse_31 * sb
            __hcse_32 = t_d2pad[xi]
            w_d2bbd[wi] = w_d2bbd[wi] + (sb * t_d2padd[xi] + __hcse_32 * sbd)
            w_d2bb[wi] = w_d2bb[wi] + __hcse_32 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c2 * hw2:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        __oldb_0d = t_d2padbd[yi]
        __oldb_0 = t_d2padb[yi]
        t_d2padbd[yi] = 0.0
        t_d2padb[yi] = 0.0
        t_d2bd[idx] = t_d2bd[idx] + __oldb_0d
        t_d2b[idx] = t_d2b[idx] + __oldb_0
    end
    for i = n_d2_midpad:-1:1
        t_d2padbd[i] = 0.0
        t_d2padb[i] = 0.0
    end
    for i = n_d2_mid:-1:1
        __idx_t_d2_stack_0 = (i - 1) + 1
        t_d2d[i] = t_d2_stack_d[__idx_t_d2_stack_0]
        t_d2[i] = t_d2_stack[__idx_t_d2_stack_0]
        __oldb_2d = t_d2bd[i]
        __oldb_2 = t_d2b[i]
        t_d2bd[i] = 0.0
        t_d2b[i] = 0.0
        __hcse_33 = 0.5 * (1.0 + sign(t_d2[i] - zero_val))
        t_d2bd[i] = t_d2bd[i] + __hcse_33 * __oldb_2d
        t_d2b[i] = t_d2b[i] + __hcse_33 * __oldb_2
    end
    for idx = c2 * hw2:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        __oldb_0d = t_d2bd[idx]
        __oldb_0 = t_d2b[idx]
        t_d2bd[idx] = 0.0
        t_d2b[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_d2abd[co] = b_d2abd[co] + __oldb_0d
        b_d2ab[co] = b_d2ab[co] + __oldb_0
        for i_k = c32 * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_35 = ci - 1
            xi = __icse_35 * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c32 + __icse_35) * kh + (ki - 1)) * kw + kj
            __hcse_34 = w_d2a[wi]
            cat2padbd[xi] = cat2padbd[xi] + (sb * w_d2ad[wi] + __hcse_34 * sbd)
            cat2padb[xi] = cat2padb[xi] + __hcse_34 * sb
            __hcse_35 = cat2pad[xi]
            w_d2abd[wi] = w_d2abd[wi] + (sb * cat2padd[xi] + __hcse_35 * sbd)
            w_d2ab[wi] = w_d2ab[wi] + __hcse_35 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c32 * hw2:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        __oldb_0d = cat2padbd[yi]
        __oldb_0 = cat2padb[yi]
        cat2padbd[yi] = 0.0
        cat2padb[yi] = 0.0
        cat2bd[idx] = cat2bd[idx] + __oldb_0d
        cat2b[idx] = cat2b[idx] + __oldb_0
    end
    for i = n_cat2pad:-1:1
        cat2padbd[i] = 0.0
        cat2padb[i] = 0.0
    end
    for idx = c2 * hw2:-1:1
        __oldb_0d = cat2bd[c3 * hw2 + idx]
        __oldb_0 = cat2b[c3 * hw2 + idx]
        cat2bd[c3 * hw2 + idx] = 0.0
        cat2b[c3 * hw2 + idx] = 0.0
        skip2bd[idx] = skip2bd[idx] + __oldb_0d
        skip2b[idx] = skip2b[idx] + __oldb_0
    end
    for idx = c3 * hw2:-1:1
        __oldb_0d = cat2bd[idx]
        __oldb_0 = cat2b[idx]
        cat2bd[idx] = 0.0
        cat2b[idx] = 0.0
        u2bd[idx] = u2bd[idx] + __oldb_0d
        u2b[idx] = u2b[idx] + __oldb_0
    end
    for idx = c3 * hw2:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        oi = div(rem, w2) + 1
        oj = mod(rem, w2) + 1
        oim1 = oi - 1
        ojm1 = oj - 1
        i = div(oim1, scale) + 1
        j = div(ojm1, scale) + 1
        xi = (ci - 1) * hw4 + (i - 1) * w4 + j
        __oldb_0d = u2bd[idx]
        __oldb_0 = u2b[idx]
        u2bd[idx] = 0.0
        u2b[idx] = 0.0
        bottbd[xi] = bottbd[xi] + __oldb_0d
        bottb[xi] = bottb[xi] + __oldb_0
    end
    for i = n_b_out:-1:1
        __idx_bott_stack_0 = (i - 1) + 1
        bottd[i] = bott_stack_d[__idx_bott_stack_0]
        bott[i] = bott_stack[__idx_bott_stack_0]
        __oldb_2d = bottbd[i]
        __oldb_2 = bottb[i]
        bottbd[i] = 0.0
        bottb[i] = 0.0
        __hcse_36 = 0.5 * (1.0 + sign(bott[i] - zero_val))
        bottbd[i] = bottbd[i] + __hcse_36 * __oldb_2d
        bottb[i] = bottb[i] + __hcse_36 * __oldb_2
    end
    for idx = c3 * hw4:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        __oldb_0d = bottbd[idx]
        __oldb_0 = bottb[idx]
        bottbd[idx] = 0.0
        bottb[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_bbbd[co] = b_bbbd[co] + __oldb_0d
        b_bbb[co] = b_bbb[co] + __oldb_0
        for i_k = c3 * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_36 = ci - 1
            xi = __icse_36 * hp4 * wp4 + (row - 1) * wp4 + col
            wi = (((co - 1) * c3 + __icse_36) * kh + (ki - 1)) * kw + kj
            __hcse_37 = w_bb[wi]
            t_bpadbd[xi] = t_bpadbd[xi] + (sb * w_bbd[wi] + __hcse_37 * sbd)
            t_bpadb[xi] = t_bpadb[xi] + __hcse_37 * sb
            __hcse_38 = t_bpad[xi]
            w_bbbd[wi] = w_bbbd[wi] + (sb * t_bpadd[xi] + __hcse_38 * sbd)
            w_bbb[wi] = w_bbb[wi] + __hcse_38 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c3 * hw4:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        yi = (ci - 1) * hp4 * wp4 + ((i + pad) - 1) * wp4 + (j + pad)
        __oldb_0d = t_bpadbd[yi]
        __oldb_0 = t_bpadb[yi]
        t_bpadbd[yi] = 0.0
        t_bpadb[yi] = 0.0
        t_bbd[idx] = t_bbd[idx] + __oldb_0d
        t_bb[idx] = t_bb[idx] + __oldb_0
    end
    for i = n_b_midpad:-1:1
        t_bpadbd[i] = 0.0
        t_bpadb[i] = 0.0
    end
    for i = n_b_mid:-1:1
        __idx_t_b_stack_0 = (i - 1) + 1
        t_bd[i] = t_b_stack_d[__idx_t_b_stack_0]
        t_b[i] = t_b_stack[__idx_t_b_stack_0]
        __oldb_2d = t_bbd[i]
        __oldb_2 = t_bb[i]
        t_bbd[i] = 0.0
        t_bb[i] = 0.0
        __hcse_39 = 0.5 * (1.0 + sign(t_b[i] - zero_val))
        t_bbd[i] = t_bbd[i] + __hcse_39 * __oldb_2d
        t_bb[i] = t_bb[i] + __hcse_39 * __oldb_2
    end
    for idx = c3 * hw4:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        __oldb_0d = t_bbd[idx]
        __oldb_0 = t_bb[idx]
        t_bbd[idx] = 0.0
        t_bb[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_babd[co] = b_babd[co] + __oldb_0d
        b_bab[co] = b_bab[co] + __oldb_0
        for i_k = c2 * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_37 = ci - 1
            xi = __icse_37 * hp4 * wp4 + (row - 1) * wp4 + col
            wi = (((co - 1) * c2 + __icse_37) * kh + (ki - 1)) * kw + kj
            __hcse_40 = w_ba[wi]
            p2padbd[xi] = p2padbd[xi] + (sb * w_bad[wi] + __hcse_40 * sbd)
            p2padb[xi] = p2padb[xi] + __hcse_40 * sb
            __hcse_41 = p2pad[xi]
            w_babd[wi] = w_babd[wi] + (sb * p2padd[xi] + __hcse_41 * sbd)
            w_bab[wi] = w_bab[wi] + __hcse_41 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c2 * hw4:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        yi = (ci - 1) * hp4 * wp4 + ((i + pad) - 1) * wp4 + (j + pad)
        __oldb_0d = p2padbd[yi]
        __oldb_0 = p2padb[yi]
        p2padbd[yi] = 0.0
        p2padb[yi] = 0.0
        p2bd[idx] = p2bd[idx] + __oldb_0d
        p2b[idx] = p2b[idx] + __oldb_0
    end
    for i = n_p2pad:-1:1
        p2padbd[i] = 0.0
        p2padb[i] = 0.0
    end
    for idx = 1:c2 * hw4
        __icse_38 = idx - 1
        idxm1 = __icse_38
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        oi = div(rem, w4) + 1
        oj = mod(rem, w4) + 1
        i0 = 2oi - 1
        j0 = 2oj - 1
        a11d = skip2d[(ci - 1) * hw2 + (i0 - 1) * w2 + j0]
        a11 = skip2[(ci - 1) * hw2 + (i0 - 1) * w2 + j0]
        a12d = skip2d[(ci - 1) * hw2 + (i0 - 1) * w2 + j0 + 1]
        a12 = skip2[(ci - 1) * hw2 + (i0 - 1) * w2 + j0 + 1]
        a21d = skip2d[(ci - 1) * hw2 + i0 * w2 + j0]
        a21 = skip2[(ci - 1) * hw2 + i0 * w2 + j0]
        a22d = skip2d[(ci - 1) * hw2 + i0 * w2 + j0 + 1]
        a22 = skip2[(ci - 1) * hw2 + i0 * w2 + j0 + 1]
        __hcse_42 = 0.5 * (1.0 + sign(a11 - a12))
        __hcse_43 = 0.5 * (1.0 + sign(a12 - a11))
        m1d = __hcse_42 * a11d + __hcse_43 * a12d
        m1 = max(a11, a12)
        __hcse_44 = 0.5 * (1.0 + sign(a21 - a22))
        __hcse_45 = 0.5 * (1.0 + sign(a22 - a21))
        m2d = __hcse_44 * a21d + __hcse_45 * a22d
        m2 = max(a21, a22)
        idxm1 = __icse_38
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        oi = div(rem, w4) + 1
        oj = mod(rem, w4) + 1
        i0 = 2oi - 1
        j0 = 2oj - 1
        __oldb_0d = p2bd[idx]
        __oldb_0 = p2b[idx]
        p2bd[idx] = 0.0
        p2b[idx] = 0.0
        __hcse_46 = 0.5 * (1.0 + sign(m1 - m2))
        m1bd = m1bd + __hcse_46 * __oldb_0d
        m1b = m1b + __hcse_46 * __oldb_0
        __hcse_47 = 0.5 * (1.0 + sign(m2 - m1))
        m2bd = m2bd + __hcse_47 * __oldb_0d
        m2b = m2b + __hcse_47 * __oldb_0
        __oldb_0d = m2bd
        __oldb_0 = m2b
        m2bd = 0.0
        m2b = 0.0
        a21bd = a21bd + __hcse_44 * __oldb_0d
        a21b = a21b + __hcse_44 * __oldb_0
        a22bd = a22bd + __hcse_45 * __oldb_0d
        a22b = a22b + __hcse_45 * __oldb_0
        __oldb_0d = m1bd
        __oldb_0 = m1b
        m1bd = 0.0
        m1b = 0.0
        a11bd = a11bd + __hcse_42 * __oldb_0d
        a11b = a11b + __hcse_42 * __oldb_0
        a12bd = a12bd + __hcse_43 * __oldb_0d
        a12b = a12b + __hcse_43 * __oldb_0
        __oldb_0d = a22bd
        __oldb_0 = a22b
        a22bd = 0.0
        a22b = 0.0
        skip2bd[(ci - 1) * hw2 + i0 * w2 + j0 + 1] = skip2bd[(ci - 1) * hw2 + i0 * w2 + j0 + 1] + __oldb_0d
        skip2b[(ci - 1) * hw2 + i0 * w2 + j0 + 1] = skip2b[(ci - 1) * hw2 + i0 * w2 + j0 + 1] + __oldb_0
        __oldb_0d = a21bd
        __oldb_0 = a21b
        a21bd = 0.0
        a21b = 0.0
        skip2bd[(ci - 1) * hw2 + i0 * w2 + j0] = skip2bd[(ci - 1) * hw2 + i0 * w2 + j0] + __oldb_0d
        skip2b[(ci - 1) * hw2 + i0 * w2 + j0] = skip2b[(ci - 1) * hw2 + i0 * w2 + j0] + __oldb_0
        __oldb_0d = a12bd
        __oldb_0 = a12b
        a12bd = 0.0
        a12b = 0.0
        skip2bd[(ci - 1) * hw2 + (i0 - 1) * w2 + j0 + 1] = skip2bd[(ci - 1) * hw2 + (i0 - 1) * w2 + j0 + 1] + __oldb_0d
        skip2b[(ci - 1) * hw2 + (i0 - 1) * w2 + j0 + 1] = skip2b[(ci - 1) * hw2 + (i0 - 1) * w2 + j0 + 1] + __oldb_0
        __oldb_0d = a11bd
        __oldb_0 = a11b
        a11bd = 0.0
        a11b = 0.0
        skip2bd[(ci - 1) * hw2 + (i0 - 1) * w2 + j0] = skip2bd[(ci - 1) * hw2 + (i0 - 1) * w2 + j0] + __oldb_0d
        skip2b[(ci - 1) * hw2 + (i0 - 1) * w2 + j0] = skip2b[(ci - 1) * hw2 + (i0 - 1) * w2 + j0] + __oldb_0
    end
    for i = n_e2_out:-1:1
        __idx_skip2_stack_0 = (i - 1) + 1
        skip2d[i] = skip2_stack_d[__idx_skip2_stack_0]
        skip2[i] = skip2_stack[__idx_skip2_stack_0]
        __oldb_2d = skip2bd[i]
        __oldb_2 = skip2b[i]
        skip2bd[i] = 0.0
        skip2b[i] = 0.0
        __hcse_48 = 0.5 * (1.0 + sign(skip2[i] - zero_val))
        skip2bd[i] = skip2bd[i] + __hcse_48 * __oldb_2d
        skip2b[i] = skip2b[i] + __hcse_48 * __oldb_2
    end
    for idx = c2 * hw2:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        __oldb_0d = skip2bd[idx]
        __oldb_0 = skip2b[idx]
        skip2bd[idx] = 0.0
        skip2b[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_e2bbd[co] = b_e2bbd[co] + __oldb_0d
        b_e2bb[co] = b_e2bb[co] + __oldb_0
        for i_k = c2 * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_39 = ci - 1
            xi = __icse_39 * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c2 + __icse_39) * kh + (ki - 1)) * kw + kj
            __hcse_49 = w_e2b[wi]
            t_e2padbd[xi] = t_e2padbd[xi] + (sb * w_e2bd[wi] + __hcse_49 * sbd)
            t_e2padb[xi] = t_e2padb[xi] + __hcse_49 * sb
            __hcse_50 = t_e2pad[xi]
            w_e2bbd[wi] = w_e2bbd[wi] + (sb * t_e2padd[xi] + __hcse_50 * sbd)
            w_e2bb[wi] = w_e2bb[wi] + __hcse_50 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c2 * hw2:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        __oldb_0d = t_e2padbd[yi]
        __oldb_0 = t_e2padb[yi]
        t_e2padbd[yi] = 0.0
        t_e2padb[yi] = 0.0
        t_e2bd[idx] = t_e2bd[idx] + __oldb_0d
        t_e2b[idx] = t_e2b[idx] + __oldb_0
    end
    for i = n_e2_midpad:-1:1
        t_e2padbd[i] = 0.0
        t_e2padb[i] = 0.0
    end
    for i = n_e2_mid:-1:1
        __idx_t_e2_stack_0 = (i - 1) + 1
        t_e2d[i] = t_e2_stack_d[__idx_t_e2_stack_0]
        t_e2[i] = t_e2_stack[__idx_t_e2_stack_0]
        __oldb_2d = t_e2bd[i]
        __oldb_2 = t_e2b[i]
        t_e2bd[i] = 0.0
        t_e2b[i] = 0.0
        __hcse_51 = 0.5 * (1.0 + sign(t_e2[i] - zero_val))
        t_e2bd[i] = t_e2bd[i] + __hcse_51 * __oldb_2d
        t_e2b[i] = t_e2b[i] + __hcse_51 * __oldb_2
    end
    for idx = c2 * hw2:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        __oldb_0d = t_e2bd[idx]
        __oldb_0 = t_e2b[idx]
        t_e2bd[idx] = 0.0
        t_e2b[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_e2abd[co] = b_e2abd[co] + __oldb_0d
        b_e2ab[co] = b_e2ab[co] + __oldb_0
        for i_k = c1 * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_40 = ci - 1
            xi = __icse_40 * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c1 + __icse_40) * kh + (ki - 1)) * kw + kj
            __hcse_52 = w_e2a[wi]
            p1padbd[xi] = p1padbd[xi] + (sb * w_e2ad[wi] + __hcse_52 * sbd)
            p1padb[xi] = p1padb[xi] + __hcse_52 * sb
            __hcse_53 = p1pad[xi]
            w_e2abd[wi] = w_e2abd[wi] + (sb * p1padd[xi] + __hcse_53 * sbd)
            w_e2ab[wi] = w_e2ab[wi] + __hcse_53 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c1 * hw2:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        __oldb_0d = p1padbd[yi]
        __oldb_0 = p1padb[yi]
        p1padbd[yi] = 0.0
        p1padb[yi] = 0.0
        p1bd[idx] = p1bd[idx] + __oldb_0d
        p1b[idx] = p1b[idx] + __oldb_0
    end
    for i = n_p1pad:-1:1
        p1padbd[i] = 0.0
        p1padb[i] = 0.0
    end
    for idx = 1:c1 * hw2
        __icse_41 = idx - 1
        idxm1 = __icse_41
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        oi = div(rem, w2) + 1
        oj = mod(rem, w2) + 1
        i0 = 2oi - 1
        j0 = 2oj - 1
        a11d = skip1d[(ci - 1) * hw + (i0 - 1) * w + j0]
        a11 = skip1[(ci - 1) * hw + (i0 - 1) * w + j0]
        a12d = skip1d[(ci - 1) * hw + (i0 - 1) * w + j0 + 1]
        a12 = skip1[(ci - 1) * hw + (i0 - 1) * w + j0 + 1]
        a21d = skip1d[(ci - 1) * hw + i0 * w + j0]
        a21 = skip1[(ci - 1) * hw + i0 * w + j0]
        a22d = skip1d[(ci - 1) * hw + i0 * w + j0 + 1]
        a22 = skip1[(ci - 1) * hw + i0 * w + j0 + 1]
        __hcse_54 = 0.5 * (1.0 + sign(a11 - a12))
        __hcse_55 = 0.5 * (1.0 + sign(a12 - a11))
        m1d = __hcse_54 * a11d + __hcse_55 * a12d
        m1 = max(a11, a12)
        __hcse_56 = 0.5 * (1.0 + sign(a21 - a22))
        __hcse_57 = 0.5 * (1.0 + sign(a22 - a21))
        m2d = __hcse_56 * a21d + __hcse_57 * a22d
        m2 = max(a21, a22)
        idxm1 = __icse_41
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        oi = div(rem, w2) + 1
        oj = mod(rem, w2) + 1
        i0 = 2oi - 1
        j0 = 2oj - 1
        __oldb_0d = p1bd[idx]
        __oldb_0 = p1b[idx]
        p1bd[idx] = 0.0
        p1b[idx] = 0.0
        __hcse_58 = 0.5 * (1.0 + sign(m1 - m2))
        m1bd = m1bd + __hcse_58 * __oldb_0d
        m1b = m1b + __hcse_58 * __oldb_0
        __hcse_59 = 0.5 * (1.0 + sign(m2 - m1))
        m2bd = m2bd + __hcse_59 * __oldb_0d
        m2b = m2b + __hcse_59 * __oldb_0
        __oldb_0d = m2bd
        __oldb_0 = m2b
        m2bd = 0.0
        m2b = 0.0
        a21bd = a21bd + __hcse_56 * __oldb_0d
        a21b = a21b + __hcse_56 * __oldb_0
        a22bd = a22bd + __hcse_57 * __oldb_0d
        a22b = a22b + __hcse_57 * __oldb_0
        __oldb_0d = m1bd
        __oldb_0 = m1b
        m1bd = 0.0
        m1b = 0.0
        a11bd = a11bd + __hcse_54 * __oldb_0d
        a11b = a11b + __hcse_54 * __oldb_0
        a12bd = a12bd + __hcse_55 * __oldb_0d
        a12b = a12b + __hcse_55 * __oldb_0
        __oldb_0d = a22bd
        __oldb_0 = a22b
        a22bd = 0.0
        a22b = 0.0
        skip1bd[(ci - 1) * hw + i0 * w + j0 + 1] = skip1bd[(ci - 1) * hw + i0 * w + j0 + 1] + __oldb_0d
        skip1b[(ci - 1) * hw + i0 * w + j0 + 1] = skip1b[(ci - 1) * hw + i0 * w + j0 + 1] + __oldb_0
        __oldb_0d = a21bd
        __oldb_0 = a21b
        a21bd = 0.0
        a21b = 0.0
        skip1bd[(ci - 1) * hw + i0 * w + j0] = skip1bd[(ci - 1) * hw + i0 * w + j0] + __oldb_0d
        skip1b[(ci - 1) * hw + i0 * w + j0] = skip1b[(ci - 1) * hw + i0 * w + j0] + __oldb_0
        __oldb_0d = a12bd
        __oldb_0 = a12b
        a12bd = 0.0
        a12b = 0.0
        skip1bd[(ci - 1) * hw + (i0 - 1) * w + j0 + 1] = skip1bd[(ci - 1) * hw + (i0 - 1) * w + j0 + 1] + __oldb_0d
        skip1b[(ci - 1) * hw + (i0 - 1) * w + j0 + 1] = skip1b[(ci - 1) * hw + (i0 - 1) * w + j0 + 1] + __oldb_0
        __oldb_0d = a11bd
        __oldb_0 = a11b
        a11bd = 0.0
        a11b = 0.0
        skip1bd[(ci - 1) * hw + (i0 - 1) * w + j0] = skip1bd[(ci - 1) * hw + (i0 - 1) * w + j0] + __oldb_0d
        skip1b[(ci - 1) * hw + (i0 - 1) * w + j0] = skip1b[(ci - 1) * hw + (i0 - 1) * w + j0] + __oldb_0
    end
    for i = n_e1_out:-1:1
        __idx_skip1_stack_0 = (i - 1) + 1
        skip1d[i] = skip1_stack_d[__idx_skip1_stack_0]
        skip1[i] = skip1_stack[__idx_skip1_stack_0]
        __oldb_2d = skip1bd[i]
        __oldb_2 = skip1b[i]
        skip1bd[i] = 0.0
        skip1b[i] = 0.0
        __hcse_60 = 0.5 * (1.0 + sign(skip1[i] - zero_val))
        skip1bd[i] = skip1bd[i] + __hcse_60 * __oldb_2d
        skip1b[i] = skip1b[i] + __hcse_60 * __oldb_2
    end
    for idx = c1 * hw:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        __oldb_0d = skip1bd[idx]
        __oldb_0 = skip1b[idx]
        skip1bd[idx] = 0.0
        skip1b[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_e1bbd[co] = b_e1bbd[co] + __oldb_0d
        b_e1bb[co] = b_e1bb[co] + __oldb_0
        for i_k = c1 * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_42 = ci - 1
            xi = __icse_42 * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c1 + __icse_42) * kh + (ki - 1)) * kw + kj
            __hcse_61 = w_e1b[wi]
            t_e1padbd[xi] = t_e1padbd[xi] + (sb * w_e1bd[wi] + __hcse_61 * sbd)
            t_e1padb[xi] = t_e1padb[xi] + __hcse_61 * sb
            __hcse_62 = t_e1pad[xi]
            w_e1bbd[wi] = w_e1bbd[wi] + (sb * t_e1padd[xi] + __hcse_62 * sbd)
            w_e1bb[wi] = w_e1bb[wi] + __hcse_62 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c1 * hw:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        __oldb_0d = t_e1padbd[yi]
        __oldb_0 = t_e1padb[yi]
        t_e1padbd[yi] = 0.0
        t_e1padb[yi] = 0.0
        t_e1bd[idx] = t_e1bd[idx] + __oldb_0d
        t_e1b[idx] = t_e1b[idx] + __oldb_0
    end
    for i = n_e1_midpad:-1:1
        t_e1padbd[i] = 0.0
        t_e1padb[i] = 0.0
    end
    for i = n_e1_mid:-1:1
        __idx_t_e1_stack_0 = (i - 1) + 1
        t_e1d[i] = t_e1_stack_d[__idx_t_e1_stack_0]
        t_e1[i] = t_e1_stack[__idx_t_e1_stack_0]
        __oldb_2d = t_e1bd[i]
        __oldb_2 = t_e1b[i]
        t_e1bd[i] = 0.0
        t_e1b[i] = 0.0
        __hcse_63 = 0.5 * (1.0 + sign(t_e1[i] - zero_val))
        t_e1bd[i] = t_e1bd[i] + __hcse_63 * __oldb_2d
        t_e1b[i] = t_e1b[i] + __hcse_63 * __oldb_2
    end
    for idx = c1 * hw:-1:1
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        __oldb_0d = t_e1bd[idx]
        __oldb_0 = t_e1b[idx]
        t_e1bd[idx] = 0.0
        t_e1b[idx] = 0.0
        sbd = sbd + __oldb_0d
        sb = sb + __oldb_0
        b_e1abd[co] = b_e1abd[co] + __oldb_0d
        b_e1ab[co] = b_e1ab[co] + __oldb_0
        for i_k = c_in * khkw:-1:1
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            __icse_43 = ci - 1
            xi = __icse_43 * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c_in + __icse_43) * kh + (ki - 1)) * kw + kj
            __hcse_64 = w_e1a[wi]
            xpad0bd[xi] = xpad0bd[xi] + (sb * w_e1ad[wi] + __hcse_64 * sbd)
            xpad0b[xi] = xpad0b[xi] + __hcse_64 * sb
            __hcse_65 = xpad0[xi]
            w_e1abd[wi] = w_e1abd[wi] + (sb * xpad0d[xi] + __hcse_65 * sbd)
            w_e1ab[wi] = w_e1ab[wi] + __hcse_65 * sb
        end
        sbd = 0.0
        sb = 0.0
    end
    for idx = c_in * hw:-1:1
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        __oldb_0d = xpad0bd[yi]
        __oldb_0 = xpad0b[yi]
        xpad0bd[yi] = 0.0
        xpad0b[yi] = 0.0
        xbd[idx] = xbd[idx] + __oldb_0d
        xb[idx] = xb[idx] + __oldb_0
    end
    for i = n_xpad0:-1:1
        xpad0bd[i] = 0.0
        xpad0b[i] = 0.0
    end
    zero_valbd = 0.0
    zero_valb = 0.0
    return nothing
end

function unet(x, h, w, c_in, c1, c2, c3, c_out, w_e1a, b_e1a, w_e1b, b_e1b, w_e2a, b_e2a, w_e2b, b_e2b, w_ba, b_ba, w_bb, b_bb, w_d2a, b_d2a, w_d2b, b_d2b, w_d1a, b_d1a, w_d1b, b_d1b, w_out, b_out, xpad0, t_e1, t_e1pad, skip1, p1, p1pad, t_e2, t_e2pad, skip2, p2, p2pad, t_b, t_bpad, bott, u2, cat2, cat2pad, t_d2, t_d2pad, dec2out, u1, cat1, cat1pad, t_d1, t_d1pad, dec1out, y)
    two = 2
    four = 4
    h2 = div(h, two)
    w2 = div(w, two)
    h4 = div(h, four)
    w4 = div(w, four)
    hp1 = h + 2
    wp1 = w + 2
    hp2 = h2 + 2
    wp2 = w2 + 2
    hp4 = h4 + 2
    wp4 = w4 + 2
    pad = 1
    kh = 3
    kw = 3
    khkw = kh * kw
    kh_out = 1
    kw_out = 1
    khkw_out = kh_out * kw_out
    scale = 2
    zero_val = 0.0
    c32 = c3 + c2
    c21 = c2 + c1
    hw = h * w
    hw2 = h2 * w2
    hw4 = h4 * w4
    n_xpad0 = c_in * hp1 * wp1
    n_e1_mid = c1 * hw
    n_e1_midpad = c1 * hp1 * wp1
    n_e1_out = c1 * hw
    n_p1pad = c1 * hp2 * wp2
    n_e2_mid = c2 * hw2
    n_e2_midpad = c2 * hp2 * wp2
    n_e2_out = c2 * hw2
    n_p2pad = c2 * hp4 * wp4
    n_b_mid = c3 * hw4
    n_b_midpad = c3 * hp4 * wp4
    n_b_out = c3 * hw4
    n_cat2pad = c32 * hp2 * wp2
    n_d2_mid = c2 * hw2
    n_d2_midpad = c2 * hp2 * wp2
    n_d2_out = c2 * hw2
    n_cat1pad = c21 * hp1 * wp1
    n_d1_mid = c1 * hw
    n_d1_midpad = c1 * hp1 * wp1
    n_d1_out = c1 * hw
    for i = 1:n_xpad0
        xpad0[i] = zero_val
    end
    for idx = 1:c_in * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        xpad0[yi] = x[idx]
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        s = 0.0
        for i_k = 1:c_in * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c_in + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + xpad0[xi] * w_e1a[wi]
        end
        t_e1[idx] = s + b_e1a[co]
    end
    for i = 1:n_e1_mid
        t_e1[i] = max(t_e1[i], zero_val)
    end
    for i = 1:n_e1_midpad
        t_e1pad[i] = zero_val
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        t_e1pad[yi] = t_e1[idx]
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        s = 0.0
        for i_k = 1:c1 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c1 + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + t_e1pad[xi] * w_e1b[wi]
        end
        skip1[idx] = s + b_e1b[co]
    end
    for i = 1:n_e1_out
        skip1[i] = max(skip1[i], zero_val)
    end
    for idx = 1:c1 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        oi = div(rem, w2) + 1
        oj = mod(rem, w2) + 1
        i0 = 2oi - 1
        j0 = 2oj - 1
        a11 = skip1[(ci - 1) * hw + (i0 - 1) * w + j0]
        a12 = skip1[(ci - 1) * hw + (i0 - 1) * w + j0 + 1]
        a21 = skip1[(ci - 1) * hw + i0 * w + j0]
        a22 = skip1[(ci - 1) * hw + i0 * w + j0 + 1]
        m1 = max(a11, a12)
        m2 = max(a21, a22)
        p1[idx] = max(m1, m2)
    end
    for i = 1:n_p1pad
        p1pad[i] = zero_val
    end
    for idx = 1:c1 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        p1pad[yi] = p1[idx]
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        s = 0.0
        for i_k = 1:c1 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c1 + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + p1pad[xi] * w_e2a[wi]
        end
        t_e2[idx] = s + b_e2a[co]
    end
    for i = 1:n_e2_mid
        t_e2[i] = max(t_e2[i], zero_val)
    end
    for i = 1:n_e2_midpad
        t_e2pad[i] = zero_val
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        t_e2pad[yi] = t_e2[idx]
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        s = 0.0
        for i_k = 1:c2 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c2 + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + t_e2pad[xi] * w_e2b[wi]
        end
        skip2[idx] = s + b_e2b[co]
    end
    for i = 1:n_e2_out
        skip2[i] = max(skip2[i], zero_val)
    end
    for idx = 1:c2 * hw4
        idxm1 = idx - 1
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        oi = div(rem, w4) + 1
        oj = mod(rem, w4) + 1
        i0 = 2oi - 1
        j0 = 2oj - 1
        a11 = skip2[(ci - 1) * hw2 + (i0 - 1) * w2 + j0]
        a12 = skip2[(ci - 1) * hw2 + (i0 - 1) * w2 + j0 + 1]
        a21 = skip2[(ci - 1) * hw2 + i0 * w2 + j0]
        a22 = skip2[(ci - 1) * hw2 + i0 * w2 + j0 + 1]
        m1 = max(a11, a12)
        m2 = max(a21, a22)
        p2[idx] = max(m1, m2)
    end
    for i = 1:n_p2pad
        p2pad[i] = zero_val
    end
    for idx = 1:c2 * hw4
        idxm1 = idx - 1
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        yi = (ci - 1) * hp4 * wp4 + ((i + pad) - 1) * wp4 + (j + pad)
        p2pad[yi] = p2[idx]
    end
    for idx = 1:c3 * hw4
        idxm1 = idx - 1
        co = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        s = 0.0
        for i_k = 1:c2 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp4 * wp4 + (row - 1) * wp4 + col
            wi = (((co - 1) * c2 + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + p2pad[xi] * w_ba[wi]
        end
        t_b[idx] = s + b_ba[co]
    end
    for i = 1:n_b_mid
        t_b[i] = max(t_b[i], zero_val)
    end
    for i = 1:n_b_midpad
        t_bpad[i] = zero_val
    end
    for idx = 1:c3 * hw4
        idxm1 = idx - 1
        ci = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        yi = (ci - 1) * hp4 * wp4 + ((i + pad) - 1) * wp4 + (j + pad)
        t_bpad[yi] = t_b[idx]
    end
    for idx = 1:c3 * hw4
        idxm1 = idx - 1
        co = div(idxm1, hw4) + 1
        rem = mod(idxm1, hw4)
        i = div(rem, w4) + 1
        j = mod(rem, w4) + 1
        s = 0.0
        for i_k = 1:c3 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp4 * wp4 + (row - 1) * wp4 + col
            wi = (((co - 1) * c3 + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + t_bpad[xi] * w_bb[wi]
        end
        bott[idx] = s + b_bb[co]
    end
    for i = 1:n_b_out
        bott[i] = max(bott[i], zero_val)
    end
    for idx = 1:c3 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        oi = div(rem, w2) + 1
        oj = mod(rem, w2) + 1
        oim1 = oi - 1
        ojm1 = oj - 1
        i = div(oim1, scale) + 1
        j = div(ojm1, scale) + 1
        xi = (ci - 1) * hw4 + (i - 1) * w4 + j
        u2[idx] = bott[xi]
    end
    for idx = 1:c3 * hw2
        cat2[idx] = u2[idx]
    end
    for idx = 1:c2 * hw2
        cat2[c3 * hw2 + idx] = skip2[idx]
    end
    for i = 1:n_cat2pad
        cat2pad[i] = zero_val
    end
    for idx = 1:c32 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        cat2pad[yi] = cat2[idx]
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        s = 0.0
        for i_k = 1:c32 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c32 + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + cat2pad[xi] * w_d2a[wi]
        end
        t_d2[idx] = s + b_d2a[co]
    end
    for i = 1:n_d2_mid
        t_d2[i] = max(t_d2[i], zero_val)
    end
    for i = 1:n_d2_midpad
        t_d2pad[i] = zero_val
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        ci = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        yi = (ci - 1) * hp2 * wp2 + ((i + pad) - 1) * wp2 + (j + pad)
        t_d2pad[yi] = t_d2[idx]
    end
    for idx = 1:c2 * hw2
        idxm1 = idx - 1
        co = div(idxm1, hw2) + 1
        rem = mod(idxm1, hw2)
        i = div(rem, w2) + 1
        j = mod(rem, w2) + 1
        s = 0.0
        for i_k = 1:c2 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp2 * wp2 + (row - 1) * wp2 + col
            wi = (((co - 1) * c2 + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + t_d2pad[xi] * w_d2b[wi]
        end
        dec2out[idx] = s + b_d2b[co]
    end
    for i = 1:n_d2_out
        dec2out[i] = max(dec2out[i], zero_val)
    end
    for idx = 1:c2 * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        oi = div(rem, w) + 1
        oj = mod(rem, w) + 1
        oim1 = oi - 1
        ojm1 = oj - 1
        i = div(oim1, scale) + 1
        j = div(ojm1, scale) + 1
        xi = (ci - 1) * hw2 + (i - 1) * w2 + j
        u1[idx] = dec2out[xi]
    end
    for idx = 1:c2 * hw
        cat1[idx] = u1[idx]
    end
    for idx = 1:c1 * hw
        cat1[c2 * hw + idx] = skip1[idx]
    end
    for i = 1:n_cat1pad
        cat1pad[i] = zero_val
    end
    for idx = 1:c21 * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        cat1pad[yi] = cat1[idx]
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        s = 0.0
        for i_k = 1:c21 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c21 + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + cat1pad[xi] * w_d1a[wi]
        end
        t_d1[idx] = s + b_d1a[co]
    end
    for i = 1:n_d1_mid
        t_d1[i] = max(t_d1[i], zero_val)
    end
    for i = 1:n_d1_midpad
        t_d1pad[i] = zero_val
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        ci = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        yi = (ci - 1) * hp1 * wp1 + ((i + pad) - 1) * wp1 + (j + pad)
        t_d1pad[yi] = t_d1[idx]
    end
    for idx = 1:c1 * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        s = 0.0
        for i_k = 1:c1 * khkw
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw) + 1
            rem2 = mod(kseqm1, khkw)
            ki = div(rem2, kw) + 1
            kj = mod(rem2, kw) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hp1 * wp1 + (row - 1) * wp1 + col
            wi = (((co - 1) * c1 + (ci - 1)) * kh + (ki - 1)) * kw + kj
            s = s + t_d1pad[xi] * w_d1b[wi]
        end
        dec1out[idx] = s + b_d1b[co]
    end
    for i = 1:n_d1_out
        dec1out[i] = max(dec1out[i], zero_val)
    end
    for idx = 1:c_out * hw
        idxm1 = idx - 1
        co = div(idxm1, hw) + 1
        rem = mod(idxm1, hw)
        i = div(rem, w) + 1
        j = mod(rem, w) + 1
        s = 0.0
        for i_k = 1:c1 * khkw_out
            kseqm1 = i_k - 1
            ci = div(kseqm1, khkw_out) + 1
            rem2 = mod(kseqm1, khkw_out)
            ki = div(rem2, kw_out) + 1
            kj = mod(rem2, kw_out) + 1
            row = (i + ki) - 1
            col = (j + kj) - 1
            xi = (ci - 1) * hw + (row - 1) * w + col
            wi = (((co - 1) * c1 + (ci - 1)) * kh_out + (ki - 1)) * kw_out + kj
            s = s + dec1out[xi] * w_out[wi]
        end
        y[idx] = s + b_out[co]
    end
end
