...
function cuda_kernel_mpnn_b_2!(b_msg, dst, edge_feat, messages, msg_input, msg_input_stack, msg_scratch, msg_scratch_stack, n_edge_feat, n_edges, n_in_msg, n_msg_feat, n_node_feat, node_feat, src, w_msg)
    ...
    for k = 1:n_node_feat
        __idx_msg_input_stack_0 = max(0, div(n_edges - 1, 1) + 1) * max(0, div(n_node_feat - 1, 1) + 1) + (((e - 1) * (div(n_node_feat - 1, 1) + 1) + (k - 1)) + 1)
        msg_input_stack[__idx_msg_input_stack_0] = msg_input[in_off + n_node_feat + k]
        msg_input[in_off + n_node_feat + k] = node_feat[dst_off + k]
    end
    for k = 1:n_edge_feat
        __idx_msg_input_stack_0 = (max(0, div(n_edges - 1, 1) + 1) * max(0, div(n_node_feat - 1, 1) + 1) + max(0, div(n_edges - 1, 1) + 1) * max(0, div(n_node_feat - 1, 1) + 1)) + (((e - 1) * (div(n_edge_feat - 1, 1) + 1) + (k - 1)) + 1)
...
function cuda_kernel_mpnn_b_5!(aggb, b_updb, n_in_upd, n_msg_feat, n_node_feat, n_nodes, node_feat_outb, node_featb, upd_input, upd_input_stack, upd_inputb, upd_scratch, upd_scratch_stack, upd_scratchb, w_upd, w_updb)
    ...
    for o = n_node_feat:-1:1
        ...
        for i_i = n_in_upd:-1:1
            widx = (o - 1) * n_in_upd + i_i
            CUDA.@atomic w_updb[widx] += upd_input[uin_off + i_i] * sb
...
function cuda_kernel_mpnn_b_7!(b_msgb, dst, edge_featb, messagesb, msg_input, msg_input_stack, msg_inputb, msg_scratch, msg_scratch_stack, msg_scratchb, n_edge_feat, n_edges, n_in_msg, n_msg_feat, n_node_feat, node_featb, src, w_msg, w_msgb)
    ...
    for o = n_msg_feat:-1:1
        ...
        for i_i = n_in_msg:-1:1
            widx = (o - 1) * n_in_msg + i_i
            CUDA.@atomic w_msgb[widx] += msg_input[in_off + i_i] * sb
...
function mpnn_b_cuda(node_feat, node_featb, edge_feat, edge_featb, src, dst, w_msg, w_msgb, b_msg, b_msgb, w_upd, w_updb, b_upd, b_updb, n_nodes, n_edges, n_node_feat, n_edge_feat, n_msg_feat, msg_input, msg_inputb, msg_scratch, msg_scratchb, messages, messagesb, agg, aggb, upd_input, upd_inputb, upd_scratch, upd_scratchb, node_feat_out, node_feat_outb, msg_input_stack, msg_scratch_stack, upd_input_stack, upd_scratch_stack)
    ...
    @cuda threads = nthread_per_block blocks = max(1, Base.cld(div(n_agg - 1, 1) + 1, nthread_per_block)) cuda_kernel_mpnn_b_1!(agg, n_agg)
    @cuda threads = nthread_per_block blocks = max(1, Base.cld(div(n_edges - 1, 1) + 1, nthread_per_block)) cuda_kernel_mpnn_b_2!(b_msg, dst, edge_feat, messages, msg_input, msg_input_stack, msg_scratch, msg_scratch_stack, n_edge_feat, n_edges, n_in_msg, n_msg_feat, n_node_feat, node_feat, src, w_msg)
    @cuda threads = nthread_per_block blocks = max(1, Base.cld(div(n_edges - 1, 1) + 1, nthread_per_block)) cuda_kernel_mpnn_b_3!(agg, dst, messages, n_edges, n_msg_feat)
    @cuda threads = nthread_per_block blocks = max(1, Base.cld(div(n_nodes - 1, 1) + 1, nthread_per_block)) cuda_kernel_mpnn_b_4!(agg, b_upd, n_in_upd, n_msg_feat, n_node_feat, n_nodes, node_feat, node_feat_out, upd_input, upd_input_stack, upd_scratch, upd_scratch_stack, w_upd)
    n_in_msg = 2n_node_feat + n_edge_feat
    n_in_upd = n_node_feat + n_msg_feat
    n_agg = n_nodes * n_msg_feat
    @cuda threads = nthread_per_block blocks = max(1, Base.cld(div(1 - n_nodes, -1) + 1, nthread_per_block)) cuda_kernel_mpnn_b_5!(aggb, b_updb, n_in_upd, n_msg_feat, n_node_feat, n_nodes, node_feat_outb, node_featb, upd_input, upd_input_stack, upd_inputb, upd_scratch, upd_scratch_stack, upd_scratchb, w_upd, w_updb)
    @cuda threads = nthread_per_block blocks = max(1, Base.cld(div(1 - n_edges, -1) + 1, nthread_per_block)) cuda_kernel_mpnn_b_6!(aggb, dst, messagesb, n_edges, n_msg_feat)
    @cuda threads = nthread_per_block blocks = max(1, Base.cld(div(1 - n_edges, -1) + 1, nthread_per_block)) cuda_kernel_mpnn_b_7!(b_msgb, dst, edge_featb, messagesb, msg_input, msg_input_stack, msg_inputb, msg_scratch, msg_scratch_stack, msg_scratchb, n_edge_feat, n_edges, n_in_msg, n_msg_feat, n_node_feat, node_featb, src, w_msg, w_msgb)
    @cuda threads = nthread_per_block blocks = max(1, Base.cld(div(1 - n_agg, -1) + 1, nthread_per_block)) cuda_kernel_mpnn_b_8!(aggb, n_agg)
    return nothing
end
