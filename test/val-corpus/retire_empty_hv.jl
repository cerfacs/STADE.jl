...
function retire_empty_hv(u, ub, x, xb, i_npass, i_w0, ud, ubd, xd, xbd, tripcount_stack, u_stack, prefix_tripcount_stack_1, prefix_u_stack_1, __tot_tripcount_stack_1, __tot_u_stack_1, val_w_1)
    ...
    for i_p = 1:i_npass
        ...
        for i_j = 1:w
            ...
            __hcse_0 = x[i_j]
            ud[i_j] = __cse_0d + (__cse_0 * xd[i_j] + __hcse_0 * __cse_0d)
            u[i_j] = __cse_0 + __hcse_0 * __cse_0
        end
        w = w - 3
    end
    for i_p = i_npass:-1:1
        ...
        for i_j = w:-1:1
            ...
            ubd[i_j] = 0.0
            ub[i_j] = 0.0
            ubd[i_j] = ubd[i_j] + __oldb_2d
            ub[i_j] = ub[i_j] + __oldb_2
            __hcse_1 = u[i_j]
            xbd[i_j] = xbd[i_j] + (__oldb_2 * ud[i_j] + __hcse_1 * __oldb_2d)
            xb[i_j] = xb[i_j] + __hcse_1 * __oldb_2
            __hcse_2 = x[i_j]
            ubd[i_j] = ubd[i_j] + (__oldb_2 * xd[i_j] + __hcse_2 * __oldb_2d)
            ub[i_j] = ub[i_j] + __hcse_2 * __oldb_2
        end
    end
    return nothing
end
