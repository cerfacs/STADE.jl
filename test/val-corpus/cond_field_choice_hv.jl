...
function cond_field_choice_hv(loss, lossb, u, ub, v, vb, w, wb, i_branch, i_n, lossd, lossbd, ud, ubd, vd, vbd, wd, wbd, branch_stack)
    ...
    if __branch == 1
        for i_x = i_n:-1:1
            ...
            wb[i_x] = 0.0
            __hcse_2 = 2 * u[i_x]
            ubd[i_x] = ubd[i_x] + (__oldb_0 * (2 * ud[i_x]) + __hcse_2 * __oldb_0d)
            ub[i_x] = ub[i_x] + __hcse_2 * __oldb_0
        end
    else
        for i_x = i_n:-1:1
            __oldb_0d = wbd[i_x]
            ...
            wb[i_x] = 0.0
            __hcse_3 = 2 * v[i_x]
            vbd[i_x] = vbd[i_x] + (__oldb_0 * (2 * vd[i_x]) + __hcse_3 * __oldb_0d)
