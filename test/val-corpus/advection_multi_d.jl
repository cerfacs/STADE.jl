function advection_multi_d(u, ud, du, dud, c, cd, dx, dxd, dt, dtd, i_nstep, i_nnode)
    for i_ = 1:i_nstep
        ...
        for i_x_advection_update_c2 = 2:i_nnode
            ...
            ud[i_x_advection_update_c2] = ud[i_x_advection_update_c2] + -(((1.0 / dx) * (((dt * __cse_0) * cd + (c * __cse_0) * dtd) + (c * dt) * dud[i_x_advection_update_c2]) + -(__cse_1 / dx ^ 2) * dxd))
            u[i_x_advection_update_c2] = u[i_x_advection_update_c2] - __cse_1 / dx
        end
    end
    return nothing
end
