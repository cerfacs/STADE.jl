...
function cuda_kernel_stencil_loss_hv_2!(i_n, ub, ubd, wb, wbd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div((i_n - 1) - 2, 1) + 1
        return nothing
    end
...
function stencil_loss_hv_cuda(loss, lossb, u, ub, w, wb, i_n, lossd, lossbd, ud, ubd, wd, wbd)
    ...
    @cuda threads = nthread_per_block blocks = cld(div((i_n - 1) - 2, 1) + 1, nthread_per_block) cuda_kernel_stencil_loss_hv_2!(i_n, ub, ubd, wb, wbd)
    return nothing
end
