...
function amdgpu_kernel_mg_vcycle_d_1!(f, fd, hl2, hl2d, i_seq_level, n, r, rd, u, ud)
...
function amdgpu_kernel_mg_vcycle_d_2!(f, fd, i_seq_level, nc, r, rd)
    __tid = (workitemIdx()).x + ((workgroupIdx()).x - 1) * (workgroupDim()).x
...
function amdgpu_kernel_mg_vcycle_d_3!(i_seq_level, nc, u, ud)
    __tid = (workitemIdx()).x + ((workgroupIdx()).x - 1) * (workgroupDim()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
...
function amdgpu_kernel_mg_vcycle_d_4!(i_seq_level, nc, u, ud)
    __tid = (workitemIdx()).x + ((workgroupIdx()).x - 1) * (workgroupDim()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
    ...
    jfd = 0.0
    jf = j * 2
    AMDGPU.@atomic ud[jf, i_seq_level] += ud[j, i_seq_level + 1]
    AMDGPU.@atomic u[jf, i_seq_level] += u[j, i_seq_level + 1]
    return nothing
end

function amdgpu_kernel_mg_vcycle_d_5!(i_seq_level, nc, u, ud)
    __tid = (workitemIdx()).x + ((workgroupIdx()).x - 1) * (workgroupDim()).x
    ...
    AMDGPU.@atomic ud[jf, i_seq_level] += 0.5 * (cld + crd)
...
function mg_vcycle_d_amdgpu(u, ud, f, fd, r, rd, nfine, num_levels, h1, h1d, nu1, nu2, n)
    ...
    for i_seq_level = 1:num_levels - 1
        ...
        @roc groupsize = nthread_per_block gridsize = cld(div(n - 1, 1) + 1, nthread_per_block) amdgpu_kernel_mg_vcycle_d_1!(f, fd, hl2, hl2d, i_seq_level, n, r, rd, u, ud)
        ncgd = 0.0
        ncg = div(nl, 2)
        ncd = 0.0
        nc = ncg - 1
        @roc groupsize = nthread_per_block gridsize = cld(div(nc - 1, 1) + 1, nthread_per_block) amdgpu_kernel_mg_vcycle_d_2!(f, fd, i_seq_level, nc, r, rd)
        @roc groupsize = nthread_per_block gridsize = cld(div(nc - 1, 1) + 1, nthread_per_block) amdgpu_kernel_mg_vcycle_d_3!(i_seq_level, nc, u, ud)
        nld = 0.0
        nl = ncg
        hld = 2.0hld
        hl = hl * 2.0
    ...
    for i_seq_level = num_levels - 1:-1:1
        ...
        hl2d = hl * hld + hl * hld
        hl2 = hl * hl
        @roc groupsize = nthread_per_block gridsize = cld(div(nc - 1, 1) + 1, nthread_per_block) amdgpu_kernel_mg_vcycle_d_4!(i_seq_level, nc, u, ud)
        @roc groupsize = nthread_per_block gridsize = cld(div((nc + 1) - 1, 1) + 1, nthread_per_block) amdgpu_kernel_mg_vcycle_d_5!(i_seq_level, nc, u, ud)
        for i_seq_k = 1:nu2
            for i_seq_j = 1:n
                leftd = 0.0
                left = 0.0
