...
function metal_kernel_mg_vcycle_b_1!(i_seq_level, nc, u)
...
function metal_kernel_mg_vcycle_b_2!(i_seq_level, nc, ub)
    __tid = (thread_position_in_grid()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
...
function mg_vcycle_b_metal(u, ub, f, fb, r, rb, nfine, num_levels, h1, h1b, nu1, nu2, n, hl_stack, hl2_stack, tripcount_stack, left_stack, branch_stack, right_stack, u_stack, r_stack, f_stack, cl_stack, cr_stack)
    ...
    for i_seq_level = 1:num_levels - 1
        ...
        push!(tripcount_stack, nc)
        @metal threads = nthread_per_block groups = cld(div(nc - 1, 1) + 1, nthread_per_block) metal_kernel_mg_vcycle_b_1!(i_seq_level, nc, u)
        nl = ncg
        push!(hl_stack, hl)
        hl = hl * 2.0f0
    end
    ...
    for i_seq_level = num_levels - 1:-1:1
        ...
        hlb = 2.0f0hlb
        nc = pop!(tripcount_stack)
        @metal threads = nthread_per_block groups = cld(div(nc - 1, 1) + 1, nthread_per_block) metal_kernel_mg_vcycle_b_2!(i_seq_level, nc, ub)
        nc = pop!(tripcount_stack)
        for j = nc:-1:1
            jf = j * 2
