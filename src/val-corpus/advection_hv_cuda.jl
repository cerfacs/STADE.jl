...
function cuda_kernel_advection_hv_1!(c, cd, dt, dtd, du, dud, dx, dxd, i_nnode, u, ud)
...
function cuda_kernel_advection_hv_2!(c, cd, dt, dtd, du, dub, dubd, dud, dx, dxd, i_nnode, ub, ubd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_nnode - 2, 1) + 1
        return nothing
    end
...
function advection_hv_cuda(u, ub, du, dub, c, cb, dx, dxb, dt, dtb, i_nstep, i_nnode, ud, ubd, dud, dubd, cd, cbd, dxd, dxbd, dtd, dtbd, du_stack)
    ...
    for i_seq_ = 1:i_nstep
        ...
        @cuda threads = nthread_per_block blocks = cld(div(i_nnode - 2, 1) + 1, nthread_per_block) cuda_kernel_advection_hv_1!(c, cd, dt, dtd, du, dud, dx, dxd, i_nnode, u, ud)
    end
    for i_seq_ = i_nstep:-1:1
        @cuda threads = nthread_per_block blocks = cld(div(i_nnode - 2, 1) + 1, nthread_per_block) cuda_kernel_advection_hv_2!(c, cd, dt, dtd, du, dub, dubd, dud, dx, dxd, i_nnode, ub, ubd)
        for i_x = i_nnode:-1:2
