...
function cuda_kernel_mg_vcycle_hv_1!(i_seq_level, nc, u, ud)
...
function cuda_kernel_mg_vcycle_hv_2!(i_seq_level, nc, ub, ubd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
...
function initstacks_mg_vcycle_b_cuda()
...
function mg_vcycle_hv_cuda(u, ub, f, fb, r, rb, nfine, num_levels, h1, h1b, nu1, nu2, n, ud, ubd, fd, fbd, rd, rbd, h1d, h1bd, hl_stack, hl2_stack, tripcount_stack, left_stack, branch_stack, right_stack, u_stack, r_stack, f_stack, cl_stack, cr_stack)
    ...
    for i_seq_level = 1:num_levels - 1
        ...
        push!(tripcount_stack, nc)
        @cuda threads = nthread_per_block blocks = cld(div(nc - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_hv_1!(i_seq_level, nc, u, ud)
        nl = ncg
        push!(hl_stack_d, hld)
        push!(hl_stack, hl)
        hld = 2.0hld
    ...
    for i_seq_level = num_levels - 1:-1:1
        ...
        hlb = 2.0hlb
        nc = pop!(tripcount_stack)
        @cuda threads = nthread_per_block blocks = cld(div(nc - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_hv_2!(i_seq_level, nc, ub, ubd)
        nc = pop!(tripcount_stack)
        for j = nc:-1:1
            jf = j * 2
