...
function jacc_kernel_transformer_b_1!(__jacc_i, b_offset, bq, d, i_seq_l, n_d, n_layers, q, q_stack, s_stack, w_offset, wq, x)
    idx = 1 + (__jacc_i - 1)
    i = div(idx - 1, d) + 1
    j = mod(idx - 1, d) + 1
...
function jacc_kernel_transformer_b_2!(__jacc_i, b_offset, bk, d, i_seq_l, k, k_stack, n_d, n_layers, s_stack, w_offset, wk, x)
    idx = 1 + (__jacc_i - 1)
    i = div(idx - 1, d) + 1
    j = mod(idx - 1, d) + 1
...
function jacc_kernel_transformer_b_3!(__jacc_i, b_offset, bv, d, i_seq_l, n_d, n_layers, s_stack, v, v_stack, w_offset, wv, x)
    idx = 1 + (__jacc_i - 1)
    i = div(idx - 1, d) + 1
    j = mod(idx - 1, d) + 1
...
function jacc_kernel_transformer_b_4!(__jacc_i, ctx, ctx_stack, d, dk, h, i_seq_l, inv_sqrt_dk, k, n, n_d, n_layers, probs, probs_stack, q, row_max_stack, row_sum_stack, s_stack, scores, scores_stack, v)
    hh = 1 + (__jacc_i - 1)
    head_offset = (hh - 1) * dk
    score_off = (hh - 1) * n * n
    for idx2 = 1:n * n
...
function jacc_kernel_transformer_b_5!(__jacc_i, attn_out, b_offset, bo, ctx, d, dk, h, i_seq_l, n, n_d, n_layers, s_stack, w_offset, wo)
    idx = 1 + (__jacc_i - 1)
    i = div(idx - 1, d) + 1
    j = mod(idx - 1, d) + 1
...
function jacc_kernel_transformer_b_6!(__jacc_i, attn_out, i_seq_l, n_d, resid1, resid1_stack, x)
    idx = 1 + (__jacc_i - 1)
...
function jacc_kernel_transformer_b_7!(__jacc_i, d, denom_stack, diff_stack, dk, eps, h, i_seq_l, ln1_bias, ln1_gain, ln_offset, n, n_d, n_layers, normed1, normed1_stack, resid1, row_mean_stack, row_var_stack, s2_stack, s_stack)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_transformer_b_8!(__jacc_i, b1, b1_offset, d, dff, dk, ff_hidden, ff_hidden_stack, h, i_seq_l, n, n_d, n_dff, n_layers, normed1, s_stack, w1, w1_offset)
    idx = 1 + (__jacc_i - 1)
    i = div(idx - 1, dff) + 1
    j = mod(idx - 1, dff) + 1
...
function jacc_kernel_transformer_b_9!(__jacc_i, b2, b2_offset, d, dff, dk, ff_hidden, ff_out, h, i_seq_l, n, n_d, n_dff, n_layers, s_stack, w2, w2_offset)
    idx = 1 + (__jacc_i - 1)
    i = div(idx - 1, d) + 1
    j = mod(idx - 1, d) + 1
...
function jacc_kernel_transformer_b_10!(__jacc_i, ff_out, i_seq_l, n_d, normed1, resid2, resid2_stack)
    idx = 1 + (__jacc_i - 1)
...
function jacc_kernel_transformer_b_11!(__jacc_i, d, denom_stack, dff, diff_stack, dk, eps, h, i_seq_l, ln2_bias, ln2_gain, ln2_offset, n, n_d, n_dff, n_layers, resid2, row_mean_stack, row_var_stack, s2_stack, s_stack, x_next)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_transformer_b_12!(__jacc_i, i_seq_l, n_d, x, x_next, x_stack)
    idx = 1 + (__jacc_i - 1)
...
function jacc_kernel_transformer_b_13!(__jacc_i, i_seq_l, n_d, x, x_nextb, x_stack, xb)
    idx = n_d + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_b_14!(__jacc_i, d, denom_stack, dff, diff_stack, dk, eps, epsb, h, i_seq_l, ln2_biasb, ln2_gain, ln2_gainb, ln2_offset, n, n_d, n_dff, n_layers, resid2, resid2b, row_mean_stack, row_var_stack, s2_stack, s_stack, x_nextb)
    i = n + (__jacc_i - 1) * -1
    ...
    for j = 1:d
        ...
        Atomix.@atomic ln2_gainb[ln2_offset + j] += ((resid2[kk] - row_mean) / denom) * x_nextb[kk]
        Atomix.@atomic ln2_biasb[ln2_offset + j] += x_nextb[kk]
        x_nextb[kk] = 0.0
    end
    ...
    Atomix.@atomic epsb[1] += (1.0 / (2.0 * sqrt(row_var + eps))) * denomb
    denomb = 0.0
...
function jacc_kernel_transformer_b_15!(__jacc_i, ff_outb, i_seq_l, n_d, normed1b, resid2, resid2_stack, resid2b)
    idx = n_d + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_b_16!(__jacc_i, b2_offset, b2b, d, dff, dk, ff_hidden, ff_hiddenb, ff_outb, h, i_seq_l, n, n_d, n_dff, n_layers, s_stack, w2, w2_offset, w2b)
    idx = n_d + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_b_17!(__jacc_i, b1, b1_offset, b1b, d, dff, dk, ff_hidden, ff_hidden_stack, ff_hiddenb, h, i_seq_l, n, n_d, n_dff, n_layers, normed1, normed1b, s_stack, w1, w1_offset, w1b)
    idx = n_dff + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_b_18!(__jacc_i, d, denom_stack, diff_stack, dk, eps, epsb, h, i_seq_l, ln1_biasb, ln1_gain, ln1_gainb, ln_offset, n, n_d, n_layers, normed1, normed1_stack, normed1b, resid1, resid1b, row_mean_stack, row_var_stack, s2_stack, s_stack)
    i = n + (__jacc_i - 1) * -1
    ...
    for j = d:-1:1
        ...
        Atomix.@atomic ln1_gainb[ln_offset + j] += ((resid1[kk] - row_mean) / denom) * normed1b[kk]
        Atomix.@atomic ln1_biasb[ln_offset + j] += normed1b[kk]
        normed1b[kk] = 0.0
    end
...
function jacc_kernel_transformer_b_19!(__jacc_i, attn_outb, i_seq_l, n_d, resid1, resid1_stack, resid1b, xb)
    idx = n_d + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_b_20!(__jacc_i, attn_outb, b_offset, bob, ctx, ctxb, d, dk, h, i_seq_l, n, n_d, n_layers, s_stack, w_offset, wo, wob)
    idx = n_d + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_b_21!(__jacc_i, ctx, ctx_stack, ctxb, d, dk, h, i_seq_l, inv_sqrt_dk, k, kb, n, n_d, n_layers, probs, probs_stack, probsb, q, qb, row_max_stack, row_sum_stack, s_stack, scores, scores_stack, scoresb, v, vb)
    hh = h + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_b_22!(__jacc_i, b_offset, bvb, d, i_seq_l, n_d, n_layers, s_stack, v, v_stack, vb, w_offset, wv, wvb, x, xb)
    idx = n_d + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_b_23!(__jacc_i, b_offset, bkb, d, i_seq_l, k, k_stack, kb, n_d, n_layers, s_stack, w_offset, wk, wkb, x, xb)
    idx = n_d + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_b_24!(__jacc_i, b_offset, bqb, d, i_seq_l, n_d, n_layers, q, q_stack, qb, s_stack, w_offset, wq, wqb, x, xb)
    idx = n_d + (__jacc_i - 1) * -1
...
function jacc_kernel_transformer_4!(__jacc_i, ctx, d, dk, h, inv_sqrt_dk, k, n, probs, q, scores, v)
    hh = 1 + (__jacc_i - 1)
    head_offset = (hh - 1) * dk
    score_off = (hh - 1) * n * n
    for idx2 = 1:n * n
...
function jacc_kernel_transformer_5!(__jacc_i, attn_out, b_offset, bo, ctx, d, n_d, w_offset, wo)
    idx = 1 + (__jacc_i - 1)
    i = div(idx - 1, d) + 1
    j = mod(idx - 1, d) + 1
    s = 0.0
...
function jacc_kernel_transformer_6!(__jacc_i, attn_out, n_d, resid1, x)
    idx = 1 + (__jacc_i - 1)
    resid1[idx] = x[idx] + attn_out[idx]
    return nothing
end

function jacc_kernel_transformer_7!(__jacc_i, d, eps, ln1_bias, ln1_gain, ln_offset, n, normed1, resid1)
    i = 1 + (__jacc_i - 1)
    s = 0.0
    for i_seq_j = 1:d
        s = s + resid1[(i - 1) * d + i_seq_j]
...
function jacc_kernel_transformer_8!(__jacc_i, b1, b1_offset, d, dff, ff_hidden, n_dff, normed1, w1, w1_offset)
    idx = 1 + (__jacc_i - 1)
    i = div(idx - 1, dff) + 1
    j = mod(idx - 1, dff) + 1
    s = 0.0
...
function jacc_kernel_transformer_9!(__jacc_i, b2, b2_offset, d, dff, ff_hidden, ff_out, n_d, w2, w2_offset)
    idx = 1 + (__jacc_i - 1)
    i = div(idx - 1, d) + 1
    j = mod(idx - 1, d) + 1
    s = 0.0
...
function jacc_kernel_transformer_10!(__jacc_i, ff_out, n_d, normed1, resid2)
    idx = 1 + (__jacc_i - 1)
    resid2[idx] = normed1[idx] + ff_out[idx]
    return nothing
end

function jacc_kernel_transformer_11!(__jacc_i, d, eps, ln2_bias, ln2_gain, ln2_offset, n, resid2, x_next)
    i = 1 + (__jacc_i - 1)
    s = 0.0
    for i_seq_j = 1:d
        s = s + resid2[(i - 1) * d + i_seq_j]
...
function jacc_kernel_transformer_12!(__jacc_i, n_d, x, x_next)
    idx = 1 + (__jacc_i - 1)
    x[idx] = x_next[idx]
    return nothing
end

function initstacks_transformer_b_jacc(d, dff, dk, h, n, n_d, n_dff, n_layers)
    s_stack = JACC.zeros(Float64, ((((((((((((((((((((((((((((((((div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n * n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n * n - 1, 1) + 1) * (div(dk - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n * n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n * dk - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n * dk - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n * dk - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_dff - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_dff - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_dff - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1) * (div(dff - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1)) + 1)
    q_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1))
    k_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1))
    v_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1))
    scores_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n * n - 1, 1) + 1))
    row_max_stack = JACC.zeros(Float64, (((((div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n - 1, 1) + 1) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(n - 2, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1)) + 1)
    probs_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(n - 1, 1) + 1) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(n - 1, 1) + 1))
    row_sum_stack = JACC.zeros(Float64, (((((div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n - 1, 1) + 1) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1)) + 1)
    ctx_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(h - 1, 1) + 1) * (div(n * dk - 1, 1) + 1))
    resid1_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1))
    row_mean_stack = JACC.zeros(Float64, (((div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1)) + 1)
    s2_stack = JACC.zeros(Float64, (((((((div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1)) + 1)
    diff_stack = JACC.zeros(Float64, (((((div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(d - 1, 1) + 1) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(d - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1)) + 1)
    row_var_stack = JACC.zeros(Float64, (((div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1)) + 1)
    denom_stack = JACC.zeros(Float64, (((div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) + (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1)) + (div(n_layers - 1, 1) + 1)) + 1)
    normed1_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(n - 1, 1) + 1) * (div(d - 1, 1) + 1))
    ff_hidden_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(n_dff - 1, 1) + 1))
    resid2_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1))
    x_stack = JACC.zeros(Float64, (div(n_layers - 1, 1) + 1) * (div(n_d - 1, 1) + 1))
...
function transformer_b_jacc(x, xb, wq, wqb, bq, bqb, wk, wkb, bk, bkb, wv, wvb, bv, bvb, wo, wob, bo, bob, ln1_gain, ln1_gainb, ln1_bias, ln1_biasb, w1, w1b, b1, b1b, w2, w2b, b2, b2b, ln2_gain, ln2_gainb, ln2_bias, ln2_biasb, q, qb, k, kb, v, vb, scores, scoresb, probs, probsb, ctx, ctxb, attn_out, attn_outb, resid1, resid1b, normed1, normed1b, ff_hidden, ff_hiddenb, ff_out, ff_outb, resid2, resid2b, x_next, x_nextb, n, d, dk, h, dff, n_layers, eps, epsb, s_stack, q_stack, k_stack, v_stack, scores_stack, row_max_stack, probs_stack, row_sum_stack, ctx_stack, resid1_stack, row_mean_stack, s2_stack, diff_stack, row_var_stack, denom_stack, normed1_stack, ff_hidden_stack, resid2_stack, x_stack)
    ...
    for i_seq_l = 1:n_layers
        ...
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_b_1!(b_offset, bq, d, i_seq_l, n_d, n_layers, q, q_stack, s_stack, w_offset, wq, x)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_b_2!(b_offset, bk, d, i_seq_l, k, k_stack, n_d, n_layers, s_stack, w_offset, wk, x)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_b_3!(b_offset, bv, d, i_seq_l, n_d, n_layers, s_stack, v, v_stack, w_offset, wv, x)
        JACC.@parallel_for range = div(h - 1, 1) + 1 jacc_kernel_transformer_b_4!(ctx, ctx_stack, d, dk, h, i_seq_l, inv_sqrt_dk, k, n, n_d, n_layers, probs, probs_stack, q, row_max_stack, row_sum_stack, s_stack, scores, scores_stack, v)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_b_5!(attn_out, b_offset, bo, ctx, d, dk, h, i_seq_l, n, n_d, n_layers, s_stack, w_offset, wo)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_b_6!(attn_out, i_seq_l, n_d, resid1, resid1_stack, x)
        JACC.@parallel_for range = div(n - 1, 1) + 1 jacc_kernel_transformer_b_7!(d, denom_stack, diff_stack, dk, eps, h, i_seq_l, ln1_bias, ln1_gain, ln_offset, n, n_d, n_layers, normed1, normed1_stack, resid1, row_mean_stack, row_var_stack, s2_stack, s_stack)
        JACC.@parallel_for range = div(n_dff - 1, 1) + 1 jacc_kernel_transformer_b_8!(b1, b1_offset, d, dff, dk, ff_hidden, ff_hidden_stack, h, i_seq_l, n, n_d, n_dff, n_layers, normed1, s_stack, w1, w1_offset)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_b_9!(b2, b2_offset, d, dff, dk, ff_hidden, ff_out, h, i_seq_l, n, n_d, n_dff, n_layers, s_stack, w2, w2_offset)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_b_10!(ff_out, i_seq_l, n_d, normed1, resid2, resid2_stack)
        JACC.@parallel_for range = div(n - 1, 1) + 1 jacc_kernel_transformer_b_11!(d, denom_stack, dff, diff_stack, dk, eps, h, i_seq_l, ln2_bias, ln2_gain, ln2_offset, n, n_d, n_dff, n_layers, resid2, row_mean_stack, row_var_stack, s2_stack, s_stack, x_next)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_b_12!(i_seq_l, n_d, x, x_next, x_stack)
    ...
    for i_seq_l = n_layers:-1:1
        ...
        b2_offset = (i_seq_l - 1) * d
        ln2_offset = (i_seq_l - 1) * d
        JACC.@parallel_for range = div(1 - n_d, -1) + 1 jacc_kernel_transformer_b_13!(i_seq_l, n_d, x, x_nextb, x_stack, xb)
        JACC.@parallel_for range = div(1 - n, -1) + 1 jacc_kernel_transformer_b_14!(d, denom_stack, dff, diff_stack, dk, eps, epsb, h, i_seq_l, ln2_biasb, ln2_gain, ln2_gainb, ln2_offset, n, n_d, n_dff, n_layers, resid2, resid2b, row_mean_stack, row_var_stack, s2_stack, s_stack, x_nextb)
        JACC.@parallel_for range = div(1 - n_d, -1) + 1 jacc_kernel_transformer_b_15!(ff_outb, i_seq_l, n_d, normed1b, resid2, resid2_stack, resid2b)
        JACC.@parallel_for range = div(1 - n_d, -1) + 1 jacc_kernel_transformer_b_16!(b2_offset, b2b, d, dff, dk, ff_hidden, ff_hiddenb, ff_outb, h, i_seq_l, n, n_d, n_dff, n_layers, s_stack, w2, w2_offset, w2b)
        JACC.@parallel_for range = div(1 - n_dff, -1) + 1 jacc_kernel_transformer_b_17!(b1, b1_offset, b1b, d, dff, dk, ff_hidden, ff_hidden_stack, ff_hiddenb, h, i_seq_l, n, n_d, n_dff, n_layers, normed1, normed1b, s_stack, w1, w1_offset, w1b)
        JACC.@parallel_for range = div(1 - n, -1) + 1 jacc_kernel_transformer_b_18!(d, denom_stack, diff_stack, dk, eps, epsb, h, i_seq_l, ln1_biasb, ln1_gain, ln1_gainb, ln_offset, n, n_d, n_layers, normed1, normed1_stack, normed1b, resid1, resid1b, row_mean_stack, row_var_stack, s2_stack, s_stack)
        JACC.@parallel_for range = div(1 - n_d, -1) + 1 jacc_kernel_transformer_b_19!(attn_outb, i_seq_l, n_d, resid1, resid1_stack, resid1b, xb)
        JACC.@parallel_for range = div(1 - n_d, -1) + 1 jacc_kernel_transformer_b_20!(attn_outb, b_offset, bob, ctx, ctxb, d, dk, h, i_seq_l, n, n_d, n_layers, s_stack, w_offset, wo, wob)
        JACC.@parallel_for range = div(1 - h, -1) + 1 jacc_kernel_transformer_b_21!(ctx, ctx_stack, ctxb, d, dk, h, i_seq_l, inv_sqrt_dk, k, kb, n, n_d, n_layers, probs, probs_stack, probsb, q, qb, row_max_stack, row_sum_stack, s_stack, scores, scores_stack, scoresb, v, vb)
        JACC.@parallel_for range = div(1 - n_d, -1) + 1 jacc_kernel_transformer_b_22!(b_offset, bvb, d, i_seq_l, n_d, n_layers, s_stack, v, v_stack, vb, w_offset, wv, wvb, x, xb)
        JACC.@parallel_for range = div(1 - n_d, -1) + 1 jacc_kernel_transformer_b_23!(b_offset, bkb, d, i_seq_l, k, k_stack, kb, n_d, n_layers, s_stack, w_offset, wk, wkb, x, xb)
        JACC.@parallel_for range = div(1 - n_d, -1) + 1 jacc_kernel_transformer_b_24!(b_offset, bqb, d, i_seq_l, n_d, n_layers, q, q_stack, qb, s_stack, w_offset, wq, wqb, x, xb)
    end
    inv_sqrt_dkb = 0.0
    epsb = (JACC.to_host(epsb))[1]
    return epsb
end

function transformer_jacc(x, wq, bq, wk, bk, wv, bv, wo, bo, ln1_gain, ln1_bias, w1, b1, w2, b2, ln2_gain, ln2_bias, q, k, v, scores, probs, ctx, attn_out, resid1, normed1, ff_hidden, ff_out, resid2, x_next, n, d, dk, h, dff, n_layers, eps)
    inv_sqrt_dk = 1.0 / sqrt(dk)
    n_d = n * d
    n_dff = n * dff
    for i_seq_l = 1:n_layers
        ...
        JACC.@parallel_for range = div(h - 1, 1) + 1 jacc_kernel_transformer_4!(ctx, d, dk, h, inv_sqrt_dk, k, n, probs, q, scores, v)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_5!(attn_out, b_offset, bo, ctx, d, n_d, w_offset, wo)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_6!(attn_out, n_d, resid1, x)
        JACC.@parallel_for range = div(n - 1, 1) + 1 jacc_kernel_transformer_7!(d, eps, ln1_bias, ln1_gain, ln_offset, n, normed1, resid1)
        JACC.@parallel_for range = div(n_dff - 1, 1) + 1 jacc_kernel_transformer_8!(b1, b1_offset, d, dff, ff_hidden, n_dff, normed1, w1, w1_offset)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_9!(b2, b2_offset, d, dff, ff_hidden, ff_out, n_d, w2, w2_offset)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_10!(ff_out, n_d, normed1, resid2)
        JACC.@parallel_for range = div(n - 1, 1) + 1 jacc_kernel_transformer_11!(d, eps, ln2_bias, ln2_gain, ln2_offset, n, resid2, x_next)
        JACC.@parallel_for range = div(n_d - 1, 1) + 1 jacc_kernel_transformer_12!(n_d, x, x_next)
    end
    return nothing
end
