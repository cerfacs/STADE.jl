...
function cuda_kernel_transformer_b_1!(b_offset, bq, d, i_seq_l, n_d, n_layers, q, q_stack, s_stack, w_offset, wq, x)
...
function cuda_kernel_transformer_b_2!(b_offset, bk, d, i_seq_l, k, k_stack, n_d, n_layers, s_stack, w_offset, wk, x)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(n_d - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_3!(b_offset, bv, d, i_seq_l, n_d, n_layers, s_stack, v, v_stack, w_offset, wv, x)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(n_d - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_4!(ctx, ctx_stack, d, dk, h, i_seq_l, inv_sqrt_dk, k, n, n_d, n_layers, probs, probs_stack, q, row_max_stack, row_sum_stack, s_stack, scores, scores_stack, v)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_transformer_b_5!(attn_out, b_offset, bo, ctx, d, dk, h, i_seq_l, n, n_d, n_layers, s_stack, w_offset, wo)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(n_d - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_6!(attn_out, i_seq_l, n_d, resid1, resid1_stack, x)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(n_d - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_7!(d, denom_stack, diff_stack, dk, eps, h, i_seq_l, ln1_bias, ln1_gain, ln_offset, n, n_d, n_layers, normed1, normed1_stack, resid1, row_mean_stack, row_var_stack, s2_stack, s_stack)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_transformer_b_8!(b1, b1_offset, d, dff, dk, ff_hidden, ff_hidden_stack, h, i_seq_l, n, n_d, n_dff, n_layers, normed1, s_stack, w1, w1_offset)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_transformer_b_9!(b2, b2_offset, d, dff, dk, ff_hidden, ff_out, h, i_seq_l, n, n_d, n_dff, n_layers, s_stack, w2, w2_offset)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(n_d - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_10!(ff_out, i_seq_l, n_d, normed1, resid2, resid2_stack)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(n_d - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_11!(d, denom_stack, dff, diff_stack, dk, eps, h, i_seq_l, ln2_bias, ln2_gain, ln2_offset, n, n_d, n_dff, n_layers, resid2, row_mean_stack, row_var_stack, s2_stack, s_stack, x_next)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(n - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_12!(i_seq_l, n_d, x, x_next, x_stack)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(n_d - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_13!(i_seq_l, n_d, x, x_nextb, x_stack, xb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_transformer_b_14!(d, denom_stack, dff, diff_stack, dk, eps, epsb, h, i_seq_l, ln2_biasb, ln2_gain, ln2_gainb, ln2_offset, n, n_d, n_dff, n_layers, resid2, resid2b, row_mean_stack, row_var_stack, s2_stack, s_stack, x_nextb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_transformer_b_15!(ff_outb, i_seq_l, n_d, normed1b, resid2, resid2_stack, resid2b)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(1 - n_d, -1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_16!(b2_offset, b2b, d, dff, dk, ff_hidden, ff_hiddenb, ff_outb, h, i_seq_l, n, n_d, n_dff, n_layers, s_stack, w2, w2_offset, w2b)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(1 - n_d, -1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_17!(b1, b1_offset, b1b, d, dff, dk, ff_hidden, ff_hidden_stack, ff_hiddenb, h, i_seq_l, n, n_d, n_dff, n_layers, normed1, normed1b, s_stack, w1, w1_offset, w1b)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_transformer_b_18!(d, denom_stack, diff_stack, dk, eps, epsb, h, i_seq_l, ln1_biasb, ln1_gain, ln1_gainb, ln_offset, n, n_d, n_layers, normed1, normed1_stack, normed1b, resid1, resid1b, row_mean_stack, row_var_stack, s2_stack, s_stack)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(1 - n, -1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_19!(attn_outb, i_seq_l, n_d, resid1, resid1_stack, resid1b, xb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(1 - n_d, -1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_20!(attn_outb, b_offset, bob, ctx, ctxb, d, dk, h, i_seq_l, n, n_d, n_layers, s_stack, w_offset, wo, wob)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(1 - n_d, -1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_21!(ctx, ctx_stack, ctxb, d, dk, h, i_seq_l, inv_sqrt_dk, k, kb, n, n_d, n_layers, probs, probs_stack, probsb, q, qb, row_max_stack, row_sum_stack, s_stack, scores, scores_stack, scoresb, v, vb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_transformer_b_22!(b_offset, bvb, d, i_seq_l, n_d, n_layers, s_stack, v, v_stack, vb, w_offset, wv, wvb, x, xb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(1 - n_d, -1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_23!(b_offset, bkb, d, i_seq_l, k, k_stack, kb, n_d, n_layers, s_stack, w_offset, wk, wkb, x, xb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(1 - n_d, -1) + 1
        return nothing
    end
...
function cuda_kernel_transformer_b_24!(b_offset, bqb, d, i_seq_l, n_d, n_layers, q, q_stack, qb, s_stack, w_offset, wq, wqb, x, xb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(1 - n_d, -1) + 1
        return nothing
    end
...
function transformer_b_cuda(x, xb, wq, wqb, bq, bqb, wk, wkb, bk, bkb, wv, wvb, bv, bvb, wo, wob, bo, bob, ln1_gain, ln1_gainb, ln1_bias, ln1_biasb, w1, w1b, b1, b1b, w2, w2b, b2, b2b, ln2_gain, ln2_gainb, ln2_bias, ln2_biasb, q, qb, k, kb, v, vb, scores, scoresb, probs, probsb, ctx, ctxb, attn_out, attn_outb, resid1, resid1b, normed1, normed1b, ff_hidden, ff_hiddenb, ff_out, ff_outb, resid2, resid2b, x_next, x_nextb, n, d, dk, h, dff, n_layers, eps, epsb, s_stack, q_stack, k_stack, v_stack, scores_stack, row_max_stack, probs_stack, row_sum_stack, ctx_stack, resid1_stack, row_mean_stack, s2_stack, diff_stack, row_var_stack, denom_stack, normed1_stack, ff_hidden_stack, resid2_stack, x_stack)
    ...
    for i_seq_l = 1:n_layers
        ...
        @cuda threads = nthread_per_block blocks = cld(div(n_d - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_1!(b_offset, bq, d, i_seq_l, n_d, n_layers, q, q_stack, s_stack, w_offset, wq, x)
        @cuda threads = nthread_per_block blocks = cld(div(n_d - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_2!(b_offset, bk, d, i_seq_l, k, k_stack, n_d, n_layers, s_stack, w_offset, wk, x)
        @cuda threads = nthread_per_block blocks = cld(div(n_d - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_3!(b_offset, bv, d, i_seq_l, n_d, n_layers, s_stack, v, v_stack, w_offset, wv, x)
        @cuda threads = nthread_per_block blocks = cld(div(h - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_4!(ctx, ctx_stack, d, dk, h, i_seq_l, inv_sqrt_dk, k, n, n_d, n_layers, probs, probs_stack, q, row_max_stack, row_sum_stack, s_stack, scores, scores_stack, v)
        @cuda threads = nthread_per_block blocks = cld(div(n_d - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_5!(attn_out, b_offset, bo, ctx, d, dk, h, i_seq_l, n, n_d, n_layers, s_stack, w_offset, wo)
        @cuda threads = nthread_per_block blocks = cld(div(n_d - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_6!(attn_out, i_seq_l, n_d, resid1, resid1_stack, x)
        @cuda threads = nthread_per_block blocks = cld(div(n - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_7!(d, denom_stack, diff_stack, dk, eps, h, i_seq_l, ln1_bias, ln1_gain, ln_offset, n, n_d, n_layers, normed1, normed1_stack, resid1, row_mean_stack, row_var_stack, s2_stack, s_stack)
        @cuda threads = nthread_per_block blocks = cld(div(n_dff - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_8!(b1, b1_offset, d, dff, dk, ff_hidden, ff_hidden_stack, h, i_seq_l, n, n_d, n_dff, n_layers, normed1, s_stack, w1, w1_offset)
        @cuda threads = nthread_per_block blocks = cld(div(n_d - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_9!(b2, b2_offset, d, dff, dk, ff_hidden, ff_out, h, i_seq_l, n, n_d, n_dff, n_layers, s_stack, w2, w2_offset)
        @cuda threads = nthread_per_block blocks = cld(div(n_d - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_10!(ff_out, i_seq_l, n_d, normed1, resid2, resid2_stack)
        @cuda threads = nthread_per_block blocks = cld(div(n - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_11!(d, denom_stack, dff, diff_stack, dk, eps, h, i_seq_l, ln2_bias, ln2_gain, ln2_offset, n, n_d, n_dff, n_layers, resid2, row_mean_stack, row_var_stack, s2_stack, s_stack, x_next)
        @cuda threads = nthread_per_block blocks = cld(div(n_d - 1, 1) + 1, nthread_per_block) cuda_kernel_transformer_b_12!(i_seq_l, n_d, x, x_next, x_stack)
    ...
    for i_seq_l = n_layers:-1:1
        ...
        b2_offset = (i_seq_l - 1) * d
        ln2_offset = (i_seq_l - 1) * d
        @cuda threads = nthread_per_block blocks = cld(div(1 - n_d, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_13!(i_seq_l, n_d, x, x_nextb, x_stack, xb)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_14!(d, denom_stack, dff, diff_stack, dk, eps, epsb, h, i_seq_l, ln2_biasb, ln2_gain, ln2_gainb, ln2_offset, n, n_d, n_dff, n_layers, resid2, resid2b, row_mean_stack, row_var_stack, s2_stack, s_stack, x_nextb)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n_d, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_15!(ff_outb, i_seq_l, n_d, normed1b, resid2, resid2_stack, resid2b)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n_d, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_16!(b2_offset, b2b, d, dff, dk, ff_hidden, ff_hiddenb, ff_outb, h, i_seq_l, n, n_d, n_dff, n_layers, s_stack, w2, w2_offset, w2b)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n_dff, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_17!(b1, b1_offset, b1b, d, dff, dk, ff_hidden, ff_hidden_stack, ff_hiddenb, h, i_seq_l, n, n_d, n_dff, n_layers, normed1, normed1b, s_stack, w1, w1_offset, w1b)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_18!(d, denom_stack, diff_stack, dk, eps, epsb, h, i_seq_l, ln1_biasb, ln1_gain, ln1_gainb, ln_offset, n, n_d, n_layers, normed1, normed1_stack, normed1b, resid1, resid1b, row_mean_stack, row_var_stack, s2_stack, s_stack)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n_d, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_19!(attn_outb, i_seq_l, n_d, resid1, resid1_stack, resid1b, xb)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n_d, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_20!(attn_outb, b_offset, bob, ctx, ctxb, d, dk, h, i_seq_l, n, n_d, n_layers, s_stack, w_offset, wo, wob)
        @cuda threads = nthread_per_block blocks = cld(div(1 - h, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_21!(ctx, ctx_stack, ctxb, d, dk, h, i_seq_l, inv_sqrt_dk, k, kb, n, n_d, n_layers, probs, probs_stack, probsb, q, qb, row_max_stack, row_sum_stack, s_stack, scores, scores_stack, scoresb, v, vb)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n_d, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_22!(b_offset, bvb, d, i_seq_l, n_d, n_layers, s_stack, v, v_stack, vb, w_offset, wv, wvb, x, xb)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n_d, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_23!(b_offset, bkb, d, i_seq_l, k, k_stack, kb, n_d, n_layers, s_stack, w_offset, wk, wkb, x, xb)
        @cuda threads = nthread_per_block blocks = cld(div(1 - n_d, -1) + 1, nthread_per_block) cuda_kernel_transformer_b_24!(b_offset, bqb, d, i_seq_l, n_d, n_layers, q, q_stack, qb, s_stack, w_offset, wq, wqb, x, xb)
    end
    inv_sqrt_dkb = 0.0
    epsb = (Array(epsb))[1]
    return epsb
