...
function jacc_kernel_stencil_loss_b_2!(__jacc_i, i_n, lossb, w, wb)
    i_seq_x = (i_n - 1) + (__jacc_i - 1) * -1
    wb[i_seq_x] = wb[i_seq_x] + (2 * w[i_seq_x]) * lossb[1]
    return nothing
end

function jacc_kernel_stencil_loss_b_3!(__jacc_i, i_n, ub, wb)
    i_x = 2 + (__jacc_i - 1)
...
function stencil_loss_b_jacc(loss, lossb, u, ub, w, wb, i_n)
    ...
    JACC.@parallel_for range = div(2 - (i_n - 1), -1) + 1 jacc_kernel_stencil_loss_b_2!(i_n, lossb, w, wb)
    JACC.@parallel_for range = div((i_n - 1) - 2, 1) + 1 jacc_kernel_stencil_loss_b_3!(i_n, ub, wb)
    return nothing
end
