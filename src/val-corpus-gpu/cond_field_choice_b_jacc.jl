...
function jacc_kernel_cond_field_choice_b_3!(__jacc_i, i_n, lossb, wb)
    i_seq_x = i_n + (__jacc_i - 1) * -1
    wb[i_seq_x] = wb[i_seq_x] + lossb[1]
    return nothing
end

function jacc_kernel_cond_field_choice_b_4!(__jacc_i, i_n, u, ub, wb)
    i_x = 1 + (__jacc_i - 1)
...
function jacc_kernel_cond_field_choice_b_5!(__jacc_i, i_n, v, vb, wb)
    i_x = 1 + (__jacc_i - 1)
...
function cond_field_choice_b_jacc(loss, lossb, u, ub, v, vb, w, wb, i_branch, i_n, branch_stack)
    ...
    JACC.@parallel_for range = div(1 - i_n, -1) + 1 jacc_kernel_cond_field_choice_b_3!(i_n, lossb, wb)
    __branch = branch_stack[1]
    if __branch == 1
        JACC.@parallel_for range = div(i_n - 1, 1) + 1 jacc_kernel_cond_field_choice_b_4!(i_n, u, ub, wb)
    else
        JACC.@parallel_for range = div(i_n - 1, 1) + 1 jacc_kernel_cond_field_choice_b_5!(i_n, v, vb, wb)
    end
    return nothing
end
