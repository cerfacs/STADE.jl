...
function jacc_kernel_ttgc_b_1!(__jacc_i, aeresk_stack, aerex_stack, aerey_stack, aerez_stack, beta, c, cavgx_stack, cavgy_stack, cavgz_stack, cell_vol, dt, factor_stack, gamma, i_cell_to_node, i_ncell, re_stack, res, res2, skx, sky, skz, u, vere_stack)
    i_cell = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_b_2!(__jacc_i, i_node_perio, npernode_half, res, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    resperio[k, 1] = res[i1]
...
function jacc_kernel_ttgc_b_3!(__jacc_i, i_node_perio, npernode_half, res, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_b_4!(__jacc_i, i_nnode, node_vol, res, up)
    i_node = 1 + (__jacc_i - 1)
    up[i_node] = res[i_node] / node_vol[i_node]
    return nothing
end

function jacc_kernel_ttgc_b_5!(__jacc_i, i_nnode, i_seq_, mup, mup_stack)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_b_6!(__jacc_i, auxu_stack, cell_vol, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, up)
    i_cell = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_b_7!(__jacc_i, i_node_perio, mup, npernode_half, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    resperio[k, 1] = mup[i1]
...
function jacc_kernel_ttgc_b_8!(__jacc_i, i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, npernode_half, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_b_9!(__jacc_i, i_nnode, i_seq_, mup, node_vol, res, up, up_stack)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_b_10!(__jacc_i, c, dt, i_cell_to_node, i_ncell, res2, skx, sky, u, up, vere_stack)
    i_cell = 1 + (__jacc_i - 1)
    for i_loc = 1:4
        i_node = i_cell_to_node[i_loc, i_cell]
...
function jacc_kernel_ttgc_b_11!(__jacc_i, i_node_perio, npernode_half, res2, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_b_12!(__jacc_i, i_node_perio, npernode_half, res2, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_b_13!(__jacc_i, i_njac, i_nnode, node_vol, res2, up, up_stack)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_b_14!(__jacc_i, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, npernode_half)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_b_15!(__jacc_i, auxu_stack, cell_vol, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, npernode_half, up)
    i_cell = 1 + (__jacc_i - 1)
    i_node1 = i_cell_to_node[1, i_cell]
    i_node2 = i_cell_to_node[2, i_cell]
    i_node3 = i_cell_to_node[3, i_cell]
...
function jacc_kernel_ttgc_b_16!(__jacc_i, i_node_perio, mup, npernode_half, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    resperio[k, 1] = mup[i1]
...
function jacc_kernel_ttgc_b_17!(__jacc_i, i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, npernode_half, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_b_18!(__jacc_i, i_njac, i_nnode, i_seq_, mup, node_vol, res2, up, up_stack)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_b_19!(__jacc_i, loss, __jgen_redval)
...
function ttgc_b_jacc(u, ub, uref, urefb, i_cell_to_node, cell_vol, cell_volb, node_vol, node_volb, skx, skxb, sky, skyb, skz, skzb, i_ncell, i_nnode, c, cb, dt, dtb, beta, betab, gamma, gammab, i_njac, res, resb, res2, res2b, up, upb, mup, mupb, npernode_half, resperio, resperiob, i_node_perio, loss, lossb, cavgx_stack, cavgy_stack, cavgz_stack, vere_stack, re_stack, aerex_stack, aerey_stack, aerez_stack, aeresk_stack, factor_stack, mup_stack, auxu_stack, up_stack)
    ...
    resib = 0.0
    vereb = 0.0
    JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_b_1!(aeresk_stack, aerex_stack, aerey_stack, aerez_stack, beta, c, cavgx_stack, cavgy_stack, cavgz_stack, cell_vol, dt, factor_stack, gamma, i_cell_to_node, i_ncell, re_stack, res, res2, skx, sky, skz, u, vere_stack)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_b_2!(i_node_perio, npernode_half, res, resperio)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_b_3!(i_node_perio, npernode_half, res, resperio)
    JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_b_4!(i_nnode, node_vol, res, up)
    for i_seq_ = 1:i_njac
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_b_5!(i_nnode, i_seq_, mup, mup_stack)
        JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_b_6!(auxu_stack, cell_vol, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, up)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_b_7!(i_node_perio, mup, npernode_half, resperio)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_b_8!(i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, npernode_half, resperio)
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_b_9!(i_nnode, i_seq_, mup, node_vol, res, up, up_stack)
        auxu_stack[(div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1) + ((i_seq_ - 1) + 1)] = auxu
    end
    JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_b_10!(c, dt, i_cell_to_node, i_ncell, res2, skx, sky, u, up, vere_stack)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_b_11!(i_node_perio, npernode_half, res2, resperio)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_b_12!(i_node_perio, npernode_half, res2, resperio)
    JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_b_13!(i_njac, i_nnode, node_vol, res2, up, up_stack)
    for i_seq_ = 1:i_njac
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_b_14!(i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, npernode_half)
        JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_b_15!(auxu_stack, cell_vol, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, npernode_half, up)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_b_16!(i_node_perio, mup, npernode_half, resperio)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_b_17!(i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, npernode_half, resperio)
    ...
    JACC.@parallel_for range = 1 jacc_kernel_ttgc_b_19!(loss, __jgen_redval_19)
