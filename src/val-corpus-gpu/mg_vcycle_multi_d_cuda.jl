...
function cuda_kernel_mg_vcycle_multi_d_1!(f, fd, hl2, hl2d, i_seq_level, n, r, rd, u, ud)
...
function cuda_kernel_mg_vcycle_multi_d_2!(f, fd, i_seq_level, nc, r, rd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_mg_vcycle_multi_d_3!(i_seq_level, nc, u, ud)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_mg_vcycle_multi_d_4!(i_seq_level, nc, u, ud)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_mg_vcycle_multi_d_5!(i_seq_level, nc, u, ud)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_mg_vcycle_multi_1!(f, hl2, i_seq_level, n, r, u)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(n - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_mg_vcycle_multi_2!(f, i_seq_level, nc, r)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_mg_vcycle_multi_3!(i_seq_level, nc, u)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_mg_vcycle_multi_4!(i_seq_level, nc, u)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(nc - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_mg_vcycle_multi_5!(i_seq_level, nc, u)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div((nc + 1) - 1, 1) + 1
        return nothing
    end
...
function mg_vcycle_multi_d_cuda(u, ud, f, fd, r, rd, nfine, num_levels, h1, h1d, nu1, nu2, n)
    ...
    for i_seq_level = 1:num_levels - 1
        ...
        @cuda threads = nthread_per_block blocks = cld(div(n - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_d_1!(f, fd, hl2, hl2d, i_seq_level, n, r, rd, u, ud)
        ncgd = 0.0
        ncg = div(nl, 2)
        ncd = 0.0
        nc = ncg - 1
        @cuda threads = nthread_per_block blocks = cld(div(nc - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_d_2!(f, fd, i_seq_level, nc, r, rd)
        @cuda threads = nthread_per_block blocks = cld(div(nc - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_d_3!(i_seq_level, nc, u, ud)
        nld = 0.0
        nl = ncg
        hld = 2.0hld
        hl = hl * 2.0
    ...
    for i_seq_level = num_levels - 1:-1:1
        ...
        hl2d = hl * hld + hl * hld
        hl2 = hl * hl
        @cuda threads = nthread_per_block blocks = cld(div(nc - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_d_4!(i_seq_level, nc, u, ud)
        @cuda threads = nthread_per_block blocks = cld(div((nc + 1) - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_d_5!(i_seq_level, nc, u, ud)
...
function mg_vcycle_multi_cuda(u, f, r, nfine, num_levels, h1, nu1, nu2, n)
    nthread_per_block = 256
    n = n * 2
    nl = nfine
    hl = h1
    for i_seq_level = 1:num_levels - 1
        ...
        @cuda threads = nthread_per_block blocks = cld(div(n - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_1!(f, hl2, i_seq_level, n, r, u)
        ncg = div(nl, 2)
        nc = ncg - 1
        @cuda threads = nthread_per_block blocks = cld(div(nc - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_2!(f, i_seq_level, nc, r)
        @cuda threads = nthread_per_block blocks = cld(div(nc - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_3!(i_seq_level, nc, u)
        nl = ncg
        hl = hl * 2.0
    end
    CUDA.@allowscalar begin
    ...
    for i_seq_level = num_levels - 1:-1:1
        ...
        nc = ncg - 1
        hl2 = hl * hl
        @cuda threads = nthread_per_block blocks = cld(div(nc - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_4!(i_seq_level, nc, u)
        @cuda threads = nthread_per_block blocks = cld(div((nc + 1) - 1, 1) + 1, nthread_per_block) cuda_kernel_mg_vcycle_multi_5!(i_seq_level, nc, u)
        for i_seq_k_mg_relax_c2 = 1:nu2
            for i_seq_j_mg_relax_c2 = 1:n
                left_mg_relax_c2 = 0.0
                if i_seq_j_mg_relax_c2 > 1
