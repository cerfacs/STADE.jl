...
function jacc_kernel_advection_b_1!(__jacc_i, du, du_stack, i_nnode, i_seq_, u)
    i_x = 2 + (__jacc_i - 1)
...
function jacc_kernel_advection_b_4!(__jacc_i, du, du_stack, dub, i_nnode, i_seq_, ub)
...
function advection_b_jacc(u, ub, du, dub, c, cb, dx, dxb, dt, dtb, i_nstep, i_nnode, du_stack)
    ...
    for i_seq_ = 1:i_nstep
        JACC.@parallel_for range = div(i_nnode - 2, 1) + 1 jacc_kernel_advection_b_1!(du, du_stack, i_nnode, i_seq_, u)
        JACC.@parallel_for range = div(i_nnode - 2, 1) + 1 jacc_kernel_advection_b_2!(c, dt, du, dx, i_nnode, u)
    end
    for i_seq_ = i_nstep:-1:1
        JACC.@parallel_for range = div(i_nnode - 2, 1) + 1 jacc_kernel_advection_b_3!(c, cb, dt, dtb, du, dub, dx, dxb, i_nnode, ub)
        JACC.@parallel_for range = div(2 - i_nnode, -1) + 1 jacc_kernel_advection_b_4!(du, du_stack, dub, i_nnode, i_seq_, ub)
    end
