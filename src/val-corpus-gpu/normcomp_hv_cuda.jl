...
function cuda_kernel_normcomp_hv_2!(i_n, loss, lossd, w, wd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_normcomp_hv_3!(i_n, lossb, lossbd, w, wb, wbd, wd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_normcomp_hv_4!(i_n, ub, ubd, vb, vbd, wb, wbd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function normcomp_hv_cuda(loss, lossb, u, ub, v, vb, w, wb, i_n, lossd, lossbd, ud, ubd, vd, vbd, wd, wbd)
    ...
    @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_normcomp_hv_2!(i_n, loss, lossd, w, wd)
    @cuda threads = nthread_per_block blocks = cld(div(1 - i_n, -1) + 1, nthread_per_block) cuda_kernel_normcomp_hv_3!(i_n, lossb, lossbd, w, wb, wbd, wd)
    @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_normcomp_hv_4!(i_n, ub, ubd, vb, vbd, wb, wbd)
    return nothing
end
