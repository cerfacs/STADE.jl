...
function cuda_kernel_relu_field_b_2!(i_n, lossb, vb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_relu_field_b_3!(branch_stack, i_n, u, ub, vb)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(1 - i_n, -1) + 1
        return nothing
    end
...
function relu_field_b_cuda(loss, lossb, u, ub, v, vb, i_n, branch_stack)
    ...
    CUDA.@allowscalar begin
            loss[1] = loss[1] + mapreduce(((__mr_1,)->__mr_1), +, v)
        end
    @cuda threads = nthread_per_block blocks = cld(div(1 - i_n, -1) + 1, nthread_per_block) cuda_kernel_relu_field_b_2!(i_n, lossb, vb)
    @cuda threads = nthread_per_block blocks = cld(div(1 - i_n, -1) + 1, nthread_per_block) cuda_kernel_relu_field_b_3!(branch_stack, i_n, u, ub, vb)
    return nothing
end
