...
function mg_vcycle_d_jacc(u, ud, f, fd, r, rd, nfine, num_levels, h1, h1d, nu1, nu2, n)
    ...
    for i_seq_level = 1:num_levels - 1
        ...
        JACC.@parallel_for range = div(n - 1, 1) + 1 jacc_kernel_mg_vcycle_d_1!(f, fd, hl2, hl2d, i_seq_level, n, r, rd, u, ud)
        ncgd = 0.0
        ncg = div(nl, 2)
        ncd = 0.0
        nc = ncg - 1
        JACC.@parallel_for range = div(nc - 1, 1) + 1 jacc_kernel_mg_vcycle_d_2!(f, fd, i_seq_level, nc, r, rd)
        JACC.@parallel_for range = div(nc - 1, 1) + 1 jacc_kernel_mg_vcycle_d_3!(i_seq_level, nc, u, ud)
        nld = 0.0
        nl = ncg
        hld = 2.0hld
        hl = hl * 2.0
    ...
    for i_seq_level = num_levels - 1:-1:1
        ...
        hl2d = hl * hld + hl * hld
        hl2 = hl * hl
        JACC.@parallel_for range = div(nc - 1, 1) + 1 jacc_kernel_mg_vcycle_d_4!(i_seq_level, nc, u, ud)
        JACC.@parallel_for range = div((nc + 1) - 1, 1) + 1 jacc_kernel_mg_vcycle_d_5!(i_seq_level, nc, u, ud)
        for i_seq_k = 1:nu2
            for i_seq_j = 1:n
                leftd = 0.0
                left = 0.0
...
function mg_vcycle_jacc(u, f, r, nfine, num_levels, h1, nu1, nu2, n)
    ...
    for i_seq_level = 1:num_levels - 1
        ...
        JACC.@parallel_for range = div(n - 1, 1) + 1 jacc_kernel_mg_vcycle_1!(f, hl2, i_seq_level, n, r, u)
        ncg = div(nl, 2)
        nc = ncg - 1
        JACC.@parallel_for range = div(nc - 1, 1) + 1 jacc_kernel_mg_vcycle_2!(f, i_seq_level, nc, r)
        JACC.@parallel_for range = div(nc - 1, 1) + 1 jacc_kernel_mg_vcycle_3!(i_seq_level, nc, u)
        nl = ncg
        hl = hl * 2.0
    end
    hl2 = hl * hl
    ...
    for i_seq_level = num_levels - 1:-1:1
        ...
        nc = ncg - 1
        hl2 = hl * hl
        JACC.@parallel_for range = div(nc - 1, 1) + 1 jacc_kernel_mg_vcycle_4!(i_seq_level, nc, u)
        JACC.@parallel_for range = div((nc + 1) - 1, 1) + 1 jacc_kernel_mg_vcycle_5!(i_seq_level, nc, u)
        for i_seq_k = 1:nu2
            for i_seq_j = 1:n
                left = 0.0
                if i_seq_j > 1
