...
function jacc_kernel_ttgc_hv_1!(__jacc_i, aeresk_stack, aeresk_stack_d, aerex_stack, aerex_stack_d, aerey_stack, aerey_stack_d, aerez_stack, aerez_stack_d, beta, betad, c, cavgx_stack, cavgx_stack_d, cavgy_stack, cavgy_stack_d, cavgz_stack, cavgz_stack_d, cd, cell_vol, cell_vold, dt, dtd, factor_stack, factor_stack_d, gamma, gammad, i_cell_to_node, i_ncell, re_stack, re_stack_d, res, res2, res2d, resd, skx, skxd, sky, skyd, skz, skzd, u, ud, vere_stack, vere_stack_d)
    ...
    for i_loc = 1:4
        ...
        for i_k = 1:4
            ...
            Atomix.@atomic resd[i_k_node] += auxresd
            Atomix.@atomic res[i_k_node] += auxres
            Atomix.@atomic res2d[i_k_node] += auxres2d
            Atomix.@atomic res2[i_k_node] += auxres2
        end
...
function jacc_kernel_ttgc_hv_2!(__jacc_i, i_node_perio, npernode_half, res, resd, resperio, resperiod)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_hv_3!(__jacc_i, i_node_perio, npernode_half, res, resd, resperio, resperiod)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic resd[i1] += resperiod[k, 2]
    Atomix.@atomic res[i1] += resperio[k, 2]
    Atomix.@atomic resd[i2] += resperiod[k, 1]
    Atomix.@atomic res[i2] += resperio[k, 1]
    return nothing
end

function jacc_kernel_ttgc_hv_4!(__jacc_i, i_nnode, node_vol, node_vold, res, resd, up, upd)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_hv_5!(__jacc_i, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupd)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_hv_6!(__jacc_i, auxu_stack, auxu_stack_d, cell_vol, cell_vold, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupd, up, upd)
    i_cell = 1 + (__jacc_i - 1)
    ...
    Atomix.@atomic mupd[i_node1] += 0.05 * (cell_vol[i_cell] * (auxud + upd[i_node1]) + (auxu + up[i_node1]) * cell_vold[i_cell])
    Atomix.@atomic mup[i_node1] += ((auxu + up[i_node1]) * cell_vol[i_cell]) / 20
    ...
    Atomix.@atomic mupd[i_node2] += 0.05 * (cell_vol[i_cell] * (auxud + upd[i_node2]) + (auxu + up[i_node2]) * cell_vold[i_cell])
    Atomix.@atomic mup[i_node2] += ((auxu + up[i_node2]) * cell_vol[i_cell]) / 20
    ...
    Atomix.@atomic mupd[i_node3] += 0.05 * (cell_vol[i_cell] * (auxud + upd[i_node3]) + (auxu + up[i_node3]) * cell_vold[i_cell])
    Atomix.@atomic mup[i_node3] += ((auxu + up[i_node3]) * cell_vol[i_cell]) / 20
    ...
    Atomix.@atomic mupd[i_node4] += 0.05 * (cell_vol[i_cell] * (auxud + upd[i_node4]) + (auxu + up[i_node4]) * cell_vold[i_cell])
    Atomix.@atomic mup[i_node4] += ((auxu + up[i_node4]) * cell_vol[i_cell]) / 20
    return nothing
end

function jacc_kernel_ttgc_hv_7!(__jacc_i, i_node_perio, mup, mupd, npernode_half, resperio, resperiod)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_hv_8!(__jacc_i, i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, mup_stack_d, mupd, npernode_half, resperio, resperiod)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    ...
    Atomix.@atomic mupd[i1] += resperiod[k, 2]
    Atomix.@atomic mup[i1] += resperio[k, 2]
    ...
    Atomix.@atomic mupd[i2] += resperiod[k, 1]
    Atomix.@atomic mup[i2] += resperio[k, 1]
    return nothing
end

function jacc_kernel_ttgc_hv_9!(__jacc_i, i_nnode, i_seq_, mup, mupd, node_vol, node_vold, res, resd, up, up_stack, up_stack_d, upd)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_hv_10!(__jacc_i, c, cd, dt, dtd, i_cell_to_node, i_ncell, res2, res2d, skx, skxd, sky, skyd, u, ud, up, upd, vere_stack, vere_stack_d)
    i_cell = 1 + (__jacc_i - 1)
    for i_loc = 1:4
        i_node = i_cell_to_node[i_loc, i_cell]
        ...
        for i_k = 1:4
            i_k_node = i_cell_to_node[i_k, i_cell]
            Atomix.@atomic res2d[i_k_node] += resid
            Atomix.@atomic res2[i_k_node] += resi
        end
    end
...
function jacc_kernel_ttgc_hv_11!(__jacc_i, i_node_perio, npernode_half, res2, res2d, resperio, resperiod)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_hv_12!(__jacc_i, i_node_perio, npernode_half, res2, res2d, resperio, resperiod)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic res2d[i1] += resperiod[k, 2]
    Atomix.@atomic res2[i1] += resperio[k, 2]
    Atomix.@atomic res2d[i2] += resperiod[k, 1]
    Atomix.@atomic res2[i2] += resperio[k, 1]
    return nothing
end

function jacc_kernel_ttgc_hv_13!(__jacc_i, i_njac, i_nnode, node_vol, node_vold, res2, res2d, up, up_stack, up_stack_d, upd)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_hv_14!(__jacc_i, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupd, npernode_half)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_hv_15!(__jacc_i, auxu_stack, auxu_stack_d, cell_vol, cell_vold, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupd, npernode_half, up, upd)
    i_cell = 1 + (__jacc_i - 1)
    i_node1 = i_cell_to_node[1, i_cell]
    i_node2 = i_cell_to_node[2, i_cell]
    i_node3 = i_cell_to_node[3, i_cell]
...
function jacc_kernel_ttgc_hv_16!(__jacc_i, i_node_perio, mup, mupd, npernode_half, resperio, resperiod)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    resperiod[k, 1] = mupd[i1]
...
function jacc_kernel_ttgc_hv_17!(__jacc_i, i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, mup_stack_d, mupd, npernode_half, resperio, resperiod)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_hv_18!(__jacc_i, i_njac, i_nnode, i_seq_, mup, mupd, node_vol, node_vold, res2, res2d, up, up_stack, up_stack_d, upd)
    i_node = 1 + (__jacc_i - 1)
...
function jacc_kernel_ttgc_hv_19!(__jacc_i, i_nnode, loss, lossd, u, ud, up, upd, uref, urefd)
    i_seq_node = 1 + (__jacc_i - 1)
    Atomix.@atomic lossd[1] += (2 * ((u[i_seq_node] + up[i_seq_node]) - uref[i_seq_node])) * ((ud[i_seq_node] + upd[i_seq_node]) + -(urefd[i_seq_node]))
    Atomix.@atomic loss[1] += ((u[i_seq_node] + up[i_seq_node]) - uref[i_seq_node]) ^ 2
    return nothing
end

function jacc_kernel_ttgc_hv_20!(__jacc_i, i_nnode, lossb, lossbd, u, ub, ubd, ud, up, upb, upbd, upd, uref, urefb, urefbd, urefd)
...
function jacc_kernel_ttgc_hv_21!(__jacc_i, i_njac, i_nnode, i_seq_, mup, mupb, mupbd, mupd, node_vol, node_volb, node_volbd, node_vold, res2, res2b, res2bd, res2d, up, up_stack, up_stack_d, upb, upbd, upd)
...
function jacc_kernel_ttgc_hv_22!(__jacc_i, i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, npernode_half, resperiob, resperiobd)
...
function jacc_kernel_ttgc_hv_23!(__jacc_i, i_node_perio, mupb, mupbd, npernode_half, resperiob, resperiobd)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic mupbd[i2] += resperiobd[k, 2]
    Atomix.@atomic mupb[i2] += resperiob[k, 2]
    resperiobd[k, 2] = 0.0
    resperiob[k, 2] = 0.0
    Atomix.@atomic mupbd[i1] += resperiobd[k, 1]
    Atomix.@atomic mupb[i1] += resperiob[k, 1]
    resperiobd[k, 1] = 0.0
    resperiob[k, 1] = 0.0
    return nothing
end

function jacc_kernel_ttgc_hv_24!(__jacc_i, auxu_stack, auxu_stack_d, cell_vol, cell_volb, cell_volbd, cell_vold, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, npernode_half, up, upb, upbd, upd)
    ...
    Atomix.@atomic upbd[i_node4] += (0.05 * mupb[i_node4]) * cell_vold[i_cell] + cell_vol[i_cell] * (0.05 * mupbd[i_node4])
    Atomix.@atomic upb[i_node4] += cell_vol[i_cell] * (0.05 * mupb[i_node4])
    ...
    Atomix.@atomic upbd[i_node3] += (0.05 * mupb[i_node3]) * cell_vold[i_cell] + cell_vol[i_cell] * (0.05 * mupbd[i_node3])
    Atomix.@atomic upb[i_node3] += cell_vol[i_cell] * (0.05 * mupb[i_node3])
    ...
    Atomix.@atomic upbd[i_node2] += (0.05 * mupb[i_node2]) * cell_vold[i_cell] + cell_vol[i_cell] * (0.05 * mupbd[i_node2])
    Atomix.@atomic upb[i_node2] += cell_vol[i_cell] * (0.05 * mupb[i_node2])
    ...
    Atomix.@atomic upbd[i_node1] += (0.05 * mupb[i_node1]) * cell_vold[i_cell] + cell_vol[i_cell] * (0.05 * mupbd[i_node1])
    Atomix.@atomic upb[i_node1] += cell_vol[i_cell] * (0.05 * mupb[i_node1])
    ...
    Atomix.@atomic upbd[i_node1] += auxubd
    Atomix.@atomic upb[i_node1] += auxub
    Atomix.@atomic upbd[i_node2] += auxubd
    Atomix.@atomic upb[i_node2] += auxub
    Atomix.@atomic upbd[i_node3] += auxubd
    Atomix.@atomic upb[i_node3] += auxub
    Atomix.@atomic upbd[i_node4] += auxubd
    Atomix.@atomic upb[i_node4] += auxub
    auxubd = 0.0
    auxub = 0.0
    return nothing
end

function jacc_kernel_ttgc_hv_25!(__jacc_i, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, npernode_half)
    i_node = i_nnode + (__jacc_i - 1) * -1
...
function jacc_kernel_ttgc_hv_26!(__jacc_i, i_njac, i_nnode, node_vol, node_volb, node_volbd, node_vold, res2, res2b, res2bd, res2d, up, up_stack, up_stack_d, upb, upbd, upd)
    i_node = i_nnode + (__jacc_i - 1) * -1
...
function jacc_kernel_ttgc_hv_27!(__jacc_i, i_node_perio, npernode_half, res2b, res2bd, resperiob, resperiobd)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_hv_28!(__jacc_i, i_node_perio, npernode_half, res2b, res2bd, resperiob, resperiobd)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic res2bd[i2] += resperiobd[k, 2]
    Atomix.@atomic res2b[i2] += resperiob[k, 2]
    resperiobd[k, 2] = 0.0
    resperiob[k, 2] = 0.0
    Atomix.@atomic res2bd[i1] += resperiobd[k, 1]
    Atomix.@atomic res2b[i1] += resperiob[k, 1]
    resperiobd[k, 1] = 0.0
    resperiob[k, 1] = 0.0
    return nothing
end

function jacc_kernel_ttgc_hv_29!(__jacc_i, c, cb, cbd, cd, dt, dtb, dtbd, dtd, i_cell_to_node, i_ncell, res2b, res2bd, skx, skxb, skxbd, skxd, sky, skyb, skybd, skyd, u, ub, ubd, ud, up, upb, upbd, upd, vere_stack, vere_stack_d)
    i_cell = i_ncell + (__jacc_i - 1) * -1
    ...
    for i_loc = 4:-1:1
        ...
        Atomix.@atomic dtbd[1] += 0.25 * -((resib * vered + vere * resibd))
        Atomix.@atomic dtb[1] += 0.25 * -(vere * resib)
        ...
        Atomix.@atomic ubd[i_node] += vereb * (-(1.0 / 3.0) * ((skx[i_loc, i_cell] * cd[1, i_node] + c[1, i_node] * skxd[i_loc, i_cell]) + (sky[i_loc, i_cell] * cd[2, i_node] + c[2, i_node] * skyd[i_loc, i_cell]))) + (-(1.0 / 3.0) * (c[1, i_node] * skx[i_loc, i_cell] + c[2, i_node] * sky[i_loc, i_cell])) * verebd
        Atomix.@atomic ub[i_node] += (-(1.0 / 3.0) * (c[1, i_node] * skx[i_loc, i_cell] + c[2, i_node] * sky[i_loc, i_cell])) * vereb
        Atomix.@atomic upbd[i_node] += vereb * (-(1.0 / 3.0) * ((skx[i_loc, i_cell] * cd[1, i_node] + c[1, i_node] * skxd[i_loc, i_cell]) + (sky[i_loc, i_cell] * cd[2, i_node] + c[2, i_node] * skyd[i_loc, i_cell]))) + (-(1.0 / 3.0) * (c[1, i_node] * skx[i_loc, i_cell] + c[2, i_node] * sky[i_loc, i_cell])) * verebd
        Atomix.@atomic upb[i_node] += (-(1.0 / 3.0) * (c[1, i_node] * skx[i_loc, i_cell] + c[2, i_node] * sky[i_loc, i_cell])) * vereb
        Atomix.@atomic cbd[1, i_node] += ((-(1.0 / 3.0) * (u[i_node] + up[i_node])) * vereb) * skxd[i_loc, i_cell] + skx[i_loc, i_cell] * (vereb * (-(1.0 / 3.0) * (ud[i_node] + upd[i_node])) + (-(1.0 / 3.0) * (u[i_node] + up[i_node])) * verebd)
        Atomix.@atomic cb[1, i_node] += skx[i_loc, i_cell] * ((-(1.0 / 3.0) * (u[i_node] + up[i_node])) * vereb)
        ...
        Atomix.@atomic cbd[2, i_node] += ((-(1.0 / 3.0) * (u[i_node] + up[i_node])) * vereb) * skyd[i_loc, i_cell] + sky[i_loc, i_cell] * (vereb * (-(1.0 / 3.0) * (ud[i_node] + upd[i_node])) + (-(1.0 / 3.0) * (u[i_node] + up[i_node])) * verebd)
        Atomix.@atomic cb[2, i_node] += sky[i_loc, i_cell] * ((-(1.0 / 3.0) * (u[i_node] + up[i_node])) * vereb)
...
function jacc_kernel_ttgc_hv_30!(__jacc_i, i_nnode, i_seq_, mup, mupb, mupbd, mupd, node_vol, node_volb, node_volbd, node_vold, res, resb, resbd, resd, up, up_stack, up_stack_d, upb, upbd, upd)
    i_node = i_nnode + (__jacc_i - 1) * -1
...
function jacc_kernel_ttgc_hv_31!(__jacc_i, i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, npernode_half, resperiob, resperiobd)
    k = npernode_half + (__jacc_i - 1) * -1
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_hv_32!(__jacc_i, i_node_perio, mupb, mupbd, npernode_half, resperiob, resperiobd)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic mupbd[i2] += resperiobd[k, 2]
...
function jacc_kernel_ttgc_hv_33!(__jacc_i, auxu_stack, auxu_stack_d, cell_vol, cell_volb, cell_volbd, cell_vold, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, up, upb, upbd, upd)
    i_cell = i_ncell + (__jacc_i - 1) * -1
    i_node1 = i_cell_to_node[1, i_cell]
    i_node2 = i_cell_to_node[2, i_cell]
    i_node3 = i_cell_to_node[3, i_cell]
...
function jacc_kernel_ttgc_hv_34!(__jacc_i, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd)
    i_node = i_nnode + (__jacc_i - 1) * -1
...
function jacc_kernel_ttgc_hv_35!(__jacc_i, i_nnode, node_vol, node_volb, node_volbd, node_vold, res, resb, resbd, resd, upb, upbd)
    i_node = 1 + (__jacc_i - 1)
    resbd[i_node] = resbd[i_node] + (upb[i_node] * (-(1.0 / node_vol[i_node] ^ 2) * node_vold[i_node]) + (1.0 / node_vol[i_node]) * upbd[i_node])
    resb[i_node] = resb[i_node] + (1.0 / node_vol[i_node]) * upb[i_node]
...
function jacc_kernel_ttgc_hv_36!(__jacc_i, i_node_perio, npernode_half, resb, resbd, resperiob, resperiobd)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
...
function jacc_kernel_ttgc_hv_37!(__jacc_i, i_node_perio, npernode_half, resb, resbd, resperiob, resperiobd)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic resbd[i2] += resperiobd[k, 2]
    Atomix.@atomic resb[i2] += resperiob[k, 2]
    resperiobd[k, 2] = 0.0
    resperiob[k, 2] = 0.0
    Atomix.@atomic resbd[i1] += resperiobd[k, 1]
    Atomix.@atomic resb[i1] += resperiob[k, 1]
    resperiobd[k, 1] = 0.0
    resperiob[k, 1] = 0.0
    return nothing
end

function jacc_kernel_ttgc_hv_38!(__jacc_i, aeresk_stack, aeresk_stack_d, aerex_stack, aerex_stack_d, aerey_stack, aerey_stack_d, aerez_stack, aerez_stack_d, beta, betab, betabd, betad, c, cavgx_stack, cavgx_stack_d, cavgy_stack, cavgy_stack_d, cavgz_stack, cavgz_stack_d, cb, cbd, cd, cell_vol, cell_volb, cell_volbd, cell_vold, dt, dtb, dtbd, dtd, factor_stack, factor_stack_d, gamma, gammab, gammabd, gammad, i_cell_to_node, i_ncell, re_stack, re_stack_d, res2b, res2bd, resb, resbd, skx, skxb, skxbd, skxd, sky, skyb, skybd, skyd, skz, skzb, skzbd, skzd, u, ub, ubd, ud, vere_stack, vere_stack_d)
    i_cell = i_ncell + (__jacc_i - 1) * -1
    ...
    for i_loc = 4:-1:1
        ...
        for i_k = 4:-1:1
            ...
            Atomix.@atomic dtbd[1] += (0.3333333333333333 * (aeresk * factorb)) * (2dtd) + (2dt) * (0.3333333333333333 * (factorb * aereskd + aeresk * factorbd))
            Atomix.@atomic dtb[1] += (2dt) * (0.3333333333333333 * (aeresk * factorb))
        ...
        Atomix.@atomic dtbd[1] += 0.25 * -((resib * (vere * -(gammad[i_cell]) + (0.5 - gamma[i_cell]) * vered) + ((0.5 - gamma[i_cell]) * vere) * resibd))
        Atomix.@atomic dtb[1] += 0.25 * -(((0.5 - gamma[i_cell]) * vere) * resib)
        ...
        Atomix.@atomic ubd[i_node] += vereb * (-(1.0 / 3.0) * (((skx[i_loc, i_cell] * cd[1, i_node] + c[1, i_node] * skxd[i_loc, i_cell]) + (sky[i_loc, i_cell] * cd[2, i_node] + c[2, i_node] * skyd[i_loc, i_cell])) + (skz[i_loc, i_cell] * cd[3, i_node] + c[3, i_node] * skzd[i_loc, i_cell]))) + (-(1.0 / 3.0) * (c[1, i_node] * skx[i_loc, i_cell] + c[2, i_node] * sky[i_loc, i_cell] + c[3, i_node] * skz[i_loc, i_cell])) * verebd
        Atomix.@atomic ub[i_node] += (-(1.0 / 3.0) * (c[1, i_node] * skx[i_loc, i_cell] + c[2, i_node] * sky[i_loc, i_cell] + c[3, i_node] * skz[i_loc, i_cell])) * vereb
        Atomix.@atomic cbd[1, i_node] += ((-(1.0 / 3.0) * u[i_node]) * vereb) * skxd[i_loc, i_cell] + skx[i_loc, i_cell] * (vereb * (-(1.0 / 3.0) * ud[i_node]) + (-(1.0 / 3.0) * u[i_node]) * verebd)
        Atomix.@atomic cb[1, i_node] += skx[i_loc, i_cell] * ((-(1.0 / 3.0) * u[i_node]) * vereb)
        ...
        Atomix.@atomic cbd[2, i_node] += ((-(1.0 / 3.0) * u[i_node]) * vereb) * skyd[i_loc, i_cell] + sky[i_loc, i_cell] * (vereb * (-(1.0 / 3.0) * ud[i_node]) + (-(1.0 / 3.0) * u[i_node]) * verebd)
        Atomix.@atomic cb[2, i_node] += sky[i_loc, i_cell] * ((-(1.0 / 3.0) * u[i_node]) * vereb)
        ...
        Atomix.@atomic cbd[3, i_node] += ((-(1.0 / 3.0) * u[i_node]) * vereb) * skzd[i_loc, i_cell] + skz[i_loc, i_cell] * (vereb * (-(1.0 / 3.0) * ud[i_node]) + (-(1.0 / 3.0) * u[i_node]) * verebd)
        Atomix.@atomic cb[3, i_node] += skz[i_loc, i_cell] * ((-(1.0 / 3.0) * u[i_node]) * vereb)
    ...
    for i_loc = 4:-1:1
        ...
        Atomix.@atomic cbd[3, i_node] += 0.25cavgzbd
        Atomix.@atomic cb[3, i_node] += 0.25cavgzb
        ...
        Atomix.@atomic cbd[2, i_node] += 0.25cavgybd
        Atomix.@atomic cb[2, i_node] += 0.25cavgyb
        ...
        Atomix.@atomic cbd[1, i_node] += 0.25cavgxbd
        Atomix.@atomic cb[1, i_node] += 0.25cavgxb
    end
...
function jacc_kernel_ttgc_1!(__jacc_i, beta, c, cell_vol, dt, gamma, i_cell_to_node, i_ncell, res, res2, skx, sky, skz, u)
    i_cell = 1 + (__jacc_i - 1)
    cavgx = 0.0
    cavgy = 0.0
    cavgz = 0.0
...
function jacc_kernel_ttgc_2!(__jacc_i, i_node_perio, npernode_half, res, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    resperio[k, 1] = res[i1]
...
function jacc_kernel_ttgc_3!(__jacc_i, i_node_perio, npernode_half, res, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic res[i1] += resperio[k, 2]
...
function jacc_kernel_ttgc_4!(__jacc_i, i_nnode, node_vol, res, up)
    i_node = 1 + (__jacc_i - 1)
    up[i_node] = res[i_node] / node_vol[i_node]
    return nothing
end

function jacc_kernel_ttgc_5!(__jacc_i, i_nnode, mup)
    i_node = 1 + (__jacc_i - 1)
    mup[i_node] = 0.0
    return nothing
end

function jacc_kernel_ttgc_6!(__jacc_i, cell_vol, i_cell_to_node, i_ncell, mup, up)
    i_cell = 1 + (__jacc_i - 1)
    i_node1 = i_cell_to_node[1, i_cell]
    i_node2 = i_cell_to_node[2, i_cell]
    i_node3 = i_cell_to_node[3, i_cell]
...
function jacc_kernel_ttgc_7!(__jacc_i, i_node_perio, mup, npernode_half, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    resperio[k, 1] = mup[i1]
...
function jacc_kernel_ttgc_8!(__jacc_i, i_node_perio, mup, npernode_half, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic mup[i1] += resperio[k, 2]
...
function jacc_kernel_ttgc_9!(__jacc_i, i_nnode, mup, node_vol, res, up)
    i_node = 1 + (__jacc_i - 1)
    up[i_node] = up[i_node] + (res[i_node] - mup[i_node]) / node_vol[i_node]
    return nothing
end

function jacc_kernel_ttgc_10!(__jacc_i, c, dt, i_cell_to_node, i_ncell, res2, skx, sky, u, up)
    i_cell = 1 + (__jacc_i - 1)
    for i_loc = 1:4
        i_node = i_cell_to_node[i_loc, i_cell]
        vere = -(1.0 / 3.0) * (u[i_node] + up[i_node]) * (c[1, i_node] * skx[i_loc, i_cell] + c[2, i_node] * sky[i_loc, i_cell])
...
function jacc_kernel_ttgc_11!(__jacc_i, i_node_perio, npernode_half, res2, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    resperio[k, 1] = res2[i1]
...
function jacc_kernel_ttgc_12!(__jacc_i, i_node_perio, npernode_half, res2, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic res2[i1] += resperio[k, 2]
...
function jacc_kernel_ttgc_13!(__jacc_i, i_nnode, node_vol, res2, up)
    i_node = 1 + (__jacc_i - 1)
    up[i_node] = res2[i_node] / node_vol[i_node]
    return nothing
end

function jacc_kernel_ttgc_14!(__jacc_i, i_nnode, mup)
    i_node = 1 + (__jacc_i - 1)
    mup[i_node] = 0.0
    return nothing
end

function jacc_kernel_ttgc_15!(__jacc_i, cell_vol, i_cell_to_node, i_ncell, mup, up)
    i_cell = 1 + (__jacc_i - 1)
    i_node1 = i_cell_to_node[1, i_cell]
    i_node2 = i_cell_to_node[2, i_cell]
    i_node3 = i_cell_to_node[3, i_cell]
...
function jacc_kernel_ttgc_16!(__jacc_i, i_node_perio, mup, npernode_half, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    resperio[k, 1] = mup[i1]
...
function jacc_kernel_ttgc_17!(__jacc_i, i_node_perio, mup, npernode_half, resperio)
    k = 1 + (__jacc_i - 1)
    i1 = i_node_perio[k, 1]
    i2 = i_node_perio[k, 2]
    Atomix.@atomic mup[i1] += resperio[k, 2]
...
function jacc_kernel_ttgc_18!(__jacc_i, i_nnode, mup, node_vol, res2, up)
    i_node = 1 + (__jacc_i - 1)
    up[i_node] = up[i_node] + (res2[i_node] - mup[i_node]) / node_vol[i_node]
    return nothing
end
...
function initstacks_ttgc_b_jacc(i_ncell, i_njac, i_nnode, npernode_half)
    ...
    cavgz_stack = JACC.zeros(Float64, (((div(i_ncell - 1, 1) + 1) + (div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1)) + (div(i_ncell - 1, 1) + 1)) + 1)
    vere_stack = JACC.zeros(Float64, ((((div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1) + (div(i_ncell - 1, 1) + 1)) + (div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1)) + (div(i_ncell - 1, 1) + 1)) + 1)
    re_stack = JACC.zeros(Float64, ((div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1) + (div(i_ncell - 1, 1) + 1)) + 1)
    aerex_stack = JACC.zeros(Float64, ((div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1) + (div(i_ncell - 1, 1) + 1)) + 1)
    aerey_stack = JACC.zeros(Float64, ((div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1) + (div(i_ncell - 1, 1) + 1)) + 1)
    aerez_stack = JACC.zeros(Float64, ((div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1) + (div(i_ncell - 1, 1) + 1)) + 1)
    aeresk_stack = JACC.zeros(Float64, (((div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1) * (div(4 - 1, 1) + 1) + (div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1)) + (div(i_ncell - 1, 1) + 1)) + 1)
    factor_stack = JACC.zeros(Float64, (((div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1) * (div(4 - 1, 1) + 1) + (div(i_ncell - 1, 1) + 1) * (div(4 - 1, 1) + 1)) + (div(i_ncell - 1, 1) + 1)) + 1)
    mup_stack = JACC.zeros(Float64, (((((((((((((div(i_njac - 1, 1) + 1) * (div(i_nnode - 1, 1) + 1) + (div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(npernode_half - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(npernode_half - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_nnode - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(npernode_half - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(npernode_half - 1, 1) + 1))
    auxu_stack = JACC.zeros(Float64, ((((div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1) + (div(i_njac - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_ncell - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1)) + 1)
    up_stack = JACC.zeros(Float64, ((div(i_njac - 1, 1) + 1) * (div(i_nnode - 1, 1) + 1) + (div(i_nnode - 1, 1) + 1)) + (div(i_njac - 1, 1) + 1) * (div(i_nnode - 1, 1) + 1))
...
function ttgc_hv_jacc(u, ub, uref, urefb, i_cell_to_node, cell_vol, cell_volb, node_vol, node_volb, skx, skxb, sky, skyb, skz, skzb, i_ncell, i_nnode, c, cb, dt, dtb, beta, betab, gamma, gammab, i_njac, res, resb, res2, res2b, up, upb, mup, mupb, npernode_half, resperio, resperiob, i_node_perio, loss, lossb, ud, ubd, urefd, urefbd, cell_vold, cell_volbd, node_vold, node_volbd, skxd, skxbd, skyd, skybd, skzd, skzbd, cd, cbd, dtd, dtbd, betad, betabd, gammad, gammabd, resd, resbd, res2d, res2bd, upd, upbd, mupd, mupbd, resperiod, resperiobd, lossd, lossbd, cavgx_stack, cavgy_stack, cavgz_stack, vere_stack, re_stack, aerex_stack, aerey_stack, aerez_stack, aeresk_stack, factor_stack, mup_stack, auxu_stack, up_stack)
    ...
    vered = 0.0
    verebd = 0.0
    JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_hv_1!(aeresk_stack, aeresk_stack_d, aerex_stack, aerex_stack_d, aerey_stack, aerey_stack_d, aerez_stack, aerez_stack_d, beta, betad, c, cavgx_stack, cavgx_stack_d, cavgy_stack, cavgy_stack_d, cavgz_stack, cavgz_stack_d, cd, cell_vol, cell_vold, dt, dtd, factor_stack, factor_stack_d, gamma, gammad, i_cell_to_node, i_ncell, re_stack, re_stack_d, res, res2, res2d, resd, skx, skxd, sky, skyd, skz, skzd, u, ud, vere_stack, vere_stack_d)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_2!(i_node_perio, npernode_half, res, resd, resperio, resperiod)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_3!(i_node_perio, npernode_half, res, resd, resperio, resperiod)
    JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_hv_4!(i_nnode, node_vol, node_vold, res, resd, up, upd)
    for i_seq_ = 1:i_njac
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_hv_5!(i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupd)
        JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_hv_6!(auxu_stack, auxu_stack_d, cell_vol, cell_vold, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupd, up, upd)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_7!(i_node_perio, mup, mupd, npernode_half, resperio, resperiod)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_8!(i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, mup_stack_d, mupd, npernode_half, resperio, resperiod)
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_hv_9!(i_nnode, i_seq_, mup, mupd, node_vol, node_vold, res, resd, up, up_stack, up_stack_d, upd)
    ...
    JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_hv_10!(c, cd, dt, dtd, i_cell_to_node, i_ncell, res2, res2d, skx, skxd, sky, skyd, u, ud, up, upd, vere_stack, vere_stack_d)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_11!(i_node_perio, npernode_half, res2, res2d, resperio, resperiod)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_12!(i_node_perio, npernode_half, res2, res2d, resperio, resperiod)
    JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_hv_13!(i_njac, i_nnode, node_vol, node_vold, res2, res2d, up, up_stack, up_stack_d, upd)
    for i_seq_ = 1:i_njac
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_hv_14!(i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupd, npernode_half)
        JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_hv_15!(auxu_stack, auxu_stack_d, cell_vol, cell_vold, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupd, npernode_half, up, upd)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_16!(i_node_perio, mup, mupd, npernode_half, resperio, resperiod)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_17!(i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, mup_stack_d, mupd, npernode_half, resperio, resperiod)
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_hv_18!(i_njac, i_nnode, i_seq_, mup, mupd, node_vol, node_vold, res2, res2d, up, up_stack, up_stack_d, upd)
    ...
    JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_hv_19!(i_nnode, loss, lossd, u, ud, up, upd, uref, urefd)
    ...
    JACC.@parallel_for range = div(1 - i_nnode, -1) + 1 jacc_kernel_ttgc_hv_20!(i_nnode, lossb, lossbd, u, ub, ubd, ud, up, upb, upbd, upd, uref, urefb, urefbd, urefd)
    for i_seq_ = i_njac:-1:1
        ...
        JACC.@parallel_for range = div(1 - i_nnode, -1) + 1 jacc_kernel_ttgc_hv_21!(i_njac, i_nnode, i_seq_, mup, mupb, mupbd, mupd, node_vol, node_volb, node_volbd, node_vold, res2, res2b, res2bd, res2d, up, up_stack, up_stack_d, upb, upbd, upd)
        JACC.@parallel_for range = div(1 - npernode_half, -1) + 1 jacc_kernel_ttgc_hv_22!(i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, npernode_half, resperiob, resperiobd)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_23!(i_node_perio, mupb, mupbd, npernode_half, resperiob, resperiobd)
        JACC.@parallel_for range = div(1 - i_ncell, -1) + 1 jacc_kernel_ttgc_hv_24!(auxu_stack, auxu_stack_d, cell_vol, cell_volb, cell_volbd, cell_vold, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, npernode_half, up, upb, upbd, upd)
        JACC.@parallel_for range = div(1 - i_nnode, -1) + 1 jacc_kernel_ttgc_hv_25!(i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, npernode_half)
    end
    JACC.@parallel_for range = div(1 - i_nnode, -1) + 1 jacc_kernel_ttgc_hv_26!(i_njac, i_nnode, node_vol, node_volb, node_volbd, node_vold, res2, res2b, res2bd, res2d, up, up_stack, up_stack_d, upb, upbd, upd)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_27!(i_node_perio, npernode_half, res2b, res2bd, resperiob, resperiobd)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_28!(i_node_perio, npernode_half, res2b, res2bd, resperiob, resperiobd)
    JACC.@parallel_for range = div(1 - i_ncell, -1) + 1 jacc_kernel_ttgc_hv_29!(c, cb, cbd, cd, dt, dtb, dtbd, dtd, i_cell_to_node, i_ncell, res2b, res2bd, skx, skxb, skxbd, skxd, sky, skyb, skybd, skyd, u, ub, ubd, ud, up, upb, upbd, upd, vere_stack, vere_stack_d)
    for i_seq_ = i_njac:-1:1
        ...
        JACC.@parallel_for range = div(1 - i_nnode, -1) + 1 jacc_kernel_ttgc_hv_30!(i_nnode, i_seq_, mup, mupb, mupbd, mupd, node_vol, node_volb, node_volbd, node_vold, res, resb, resbd, resd, up, up_stack, up_stack_d, upb, upbd, upd)
        JACC.@parallel_for range = div(1 - npernode_half, -1) + 1 jacc_kernel_ttgc_hv_31!(i_ncell, i_njac, i_nnode, i_node_perio, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, npernode_half, resperiob, resperiobd)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_32!(i_node_perio, mupb, mupbd, npernode_half, resperiob, resperiobd)
        JACC.@parallel_for range = div(1 - i_ncell, -1) + 1 jacc_kernel_ttgc_hv_33!(auxu_stack, auxu_stack_d, cell_vol, cell_volb, cell_volbd, cell_vold, i_cell_to_node, i_ncell, i_njac, i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd, up, upb, upbd, upd)
        JACC.@parallel_for range = div(1 - i_nnode, -1) + 1 jacc_kernel_ttgc_hv_34!(i_nnode, i_seq_, mup, mup_stack, mup_stack_d, mupb, mupbd, mupd)
    end
    JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_hv_35!(i_nnode, node_vol, node_volb, node_volbd, node_vold, res, resb, resbd, resd, upb, upbd)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_36!(i_node_perio, npernode_half, resb, resbd, resperiob, resperiobd)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_hv_37!(i_node_perio, npernode_half, resb, resbd, resperiob, resperiobd)
    JACC.@parallel_for range = div(1 - i_ncell, -1) + 1 jacc_kernel_ttgc_hv_38!(aeresk_stack, aeresk_stack_d, aerex_stack, aerex_stack_d, aerey_stack, aerey_stack_d, aerez_stack, aerez_stack_d, beta, betab, betabd, betad, c, cavgx_stack, cavgx_stack_d, cavgy_stack, cavgy_stack_d, cavgz_stack, cavgz_stack_d, cb, cbd, cd, cell_vol, cell_volb, cell_volbd, cell_vold, dt, dtb, dtbd, dtd, factor_stack, factor_stack_d, gamma, gammab, gammabd, gammad, i_cell_to_node, i_ncell, re_stack, re_stack_d, res2b, res2bd, resb, resbd, skx, skxb, skxbd, skxd, sky, skyb, skybd, skyd, skz, skzb, skzbd, skzd, u, ub, ubd, ud, vere_stack, vere_stack_d)
    dtb = (JACC.to_host(dtb))[1]
    dtbd = (JACC.to_host(dtbd))[1]
    return (dtb, dtbd)
end

function ttgc_jacc(u, uref, i_cell_to_node, cell_vol, node_vol, skx, sky, skz, i_ncell, i_nnode, c, dt, beta, gamma, i_njac, res, res2, up, mup, npernode_half, resperio, i_node_perio, loss)
    JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_1!(beta, c, cell_vol, dt, gamma, i_cell_to_node, i_ncell, res, res2, skx, sky, skz, u)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_2!(i_node_perio, npernode_half, res, resperio)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_3!(i_node_perio, npernode_half, res, resperio)
    JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_4!(i_nnode, node_vol, res, up)
    for i_seq_ = 1:i_njac
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_5!(i_nnode, mup)
        JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_6!(cell_vol, i_cell_to_node, i_ncell, mup, up)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_7!(i_node_perio, mup, npernode_half, resperio)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_8!(i_node_perio, mup, npernode_half, resperio)
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_9!(i_nnode, mup, node_vol, res, up)
    end
    JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_10!(c, dt, i_cell_to_node, i_ncell, res2, skx, sky, u, up)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_11!(i_node_perio, npernode_half, res2, resperio)
    JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_12!(i_node_perio, npernode_half, res2, resperio)
    JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_13!(i_nnode, node_vol, res2, up)
    for i_seq_ = 1:i_njac
        JACC.@parallel_for range = div(i_nnode - 1, 1) + 1 jacc_kernel_ttgc_14!(i_nnode, mup)
        JACC.@parallel_for range = div(i_ncell - 1, 1) + 1 jacc_kernel_ttgc_15!(cell_vol, i_cell_to_node, i_ncell, mup, up)
        JACC.@parallel_for range = div(npernode_half - 1, 1) + 1 jacc_kernel_ttgc_16!(i_node_perio, mup, npernode_half, resperio)
