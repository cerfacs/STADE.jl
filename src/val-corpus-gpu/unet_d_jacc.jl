...
function jacc_kernel_unet_d_1!(__jacc_i, n_xpad0, xpad0, xpad0d, zero_val)
    i = 1 + (__jacc_i - 1)
    xpad0d[i] = 0.0
    xpad0[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_2!(__jacc_i, c_in, hp1, hw, pad, w, wp1, x, xd, xpad0, xpad0d)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_3!(__jacc_i, b_e1a, b_e1ad, c1, c_in, hp1, hw, kh, khkw, kw, t_e1, t_e1d, w, w_e1a, w_e1ad, wp1, xpad0, xpad0d)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_4!(__jacc_i, n_e1_mid, t_e1, t_e1d, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_5!(__jacc_i, n_e1_midpad, t_e1pad, t_e1padd, zero_val)
    i = 1 + (__jacc_i - 1)
    t_e1padd[i] = 0.0
    t_e1pad[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_6!(__jacc_i, c1, hp1, hw, pad, t_e1, t_e1d, t_e1pad, t_e1padd, w, wp1)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_7!(__jacc_i, b_e1b, b_e1bd, c1, hp1, hw, kh, khkw, kw, skip1, skip1d, t_e1pad, t_e1padd, w, w_e1b, w_e1bd, wp1)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_8!(__jacc_i, n_e1_out, skip1, skip1d, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_9!(__jacc_i, c1, hw, hw2, p1, p1d, skip1, skip1d, w, w2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_10!(__jacc_i, n_p1pad, p1pad, p1padd, zero_val)
    i = 1 + (__jacc_i - 1)
    p1padd[i] = 0.0
    p1pad[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_11!(__jacc_i, c1, hp2, hw2, p1, p1d, p1pad, p1padd, pad, w2, wp2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_12!(__jacc_i, b_e2a, b_e2ad, c1, c2, hp2, hw2, kh, khkw, kw, p1pad, p1padd, t_e2, t_e2d, w2, w_e2a, w_e2ad, wp2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_13!(__jacc_i, n_e2_mid, t_e2, t_e2d, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_14!(__jacc_i, n_e2_midpad, t_e2pad, t_e2padd, zero_val)
    i = 1 + (__jacc_i - 1)
    t_e2padd[i] = 0.0
    t_e2pad[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_15!(__jacc_i, c2, hp2, hw2, pad, t_e2, t_e2d, t_e2pad, t_e2padd, w2, wp2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_16!(__jacc_i, b_e2b, b_e2bd, c2, hp2, hw2, kh, khkw, kw, skip2, skip2d, t_e2pad, t_e2padd, w2, w_e2b, w_e2bd, wp2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_17!(__jacc_i, n_e2_out, skip2, skip2d, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_18!(__jacc_i, c2, hw2, hw4, p2, p2d, skip2, skip2d, w2, w4)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_19!(__jacc_i, n_p2pad, p2pad, p2padd, zero_val)
    i = 1 + (__jacc_i - 1)
    p2padd[i] = 0.0
    p2pad[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_20!(__jacc_i, c2, hp4, hw4, p2, p2d, p2pad, p2padd, pad, w4, wp4)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_21!(__jacc_i, b_ba, b_bad, c2, c3, hp4, hw4, kh, khkw, kw, p2pad, p2padd, t_b, t_bd, w4, w_ba, w_bad, wp4)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_22!(__jacc_i, n_b_mid, t_b, t_bd, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_23!(__jacc_i, n_b_midpad, t_bpad, t_bpadd, zero_val)
    i = 1 + (__jacc_i - 1)
    t_bpadd[i] = 0.0
    t_bpad[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_24!(__jacc_i, c3, hp4, hw4, pad, t_b, t_bd, t_bpad, t_bpadd, w4, wp4)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_25!(__jacc_i, b_bb, b_bbd, bott, bottd, c3, hp4, hw4, kh, khkw, kw, t_bpad, t_bpadd, w4, w_bb, w_bbd, wp4)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_26!(__jacc_i, bott, bottd, n_b_out, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_27!(__jacc_i, bott, bottd, c3, hw2, hw4, scale, u2, u2d, w2, w4)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_28!(__jacc_i, c3, cat2, cat2d, hw2, u2, u2d)
    idx = 1 + (__jacc_i - 1)
    cat2d[idx] = u2d[idx]
    cat2[idx] = u2[idx]
    return nothing
...
function jacc_kernel_unet_d_29!(__jacc_i, c2, c3, cat2, cat2d, hw2, skip2, skip2d)
    idx = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_30!(__jacc_i, cat2pad, cat2padd, n_cat2pad, zero_val)
    i = 1 + (__jacc_i - 1)
    cat2padd[i] = 0.0
    cat2pad[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_31!(__jacc_i, c32, cat2, cat2d, cat2pad, cat2padd, hp2, hw2, pad, w2, wp2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_32!(__jacc_i, b_d2a, b_d2ad, c2, c32, cat2pad, cat2padd, hp2, hw2, kh, khkw, kw, t_d2, t_d2d, w2, w_d2a, w_d2ad, wp2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_33!(__jacc_i, n_d2_mid, t_d2, t_d2d, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_34!(__jacc_i, n_d2_midpad, t_d2pad, t_d2padd, zero_val)
    i = 1 + (__jacc_i - 1)
    t_d2padd[i] = 0.0
    t_d2pad[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_35!(__jacc_i, c2, hp2, hw2, pad, t_d2, t_d2d, t_d2pad, t_d2padd, w2, wp2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_36!(__jacc_i, b_d2b, b_d2bd, c2, dec2out, dec2outd, hp2, hw2, kh, khkw, kw, t_d2pad, t_d2padd, w2, w_d2b, w_d2bd, wp2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_37!(__jacc_i, dec2out, dec2outd, n_d2_out, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_38!(__jacc_i, c2, dec2out, dec2outd, hw, hw2, scale, u1, u1d, w, w2)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_39!(__jacc_i, c2, cat1, cat1d, hw, u1, u1d)
    idx = 1 + (__jacc_i - 1)
    cat1d[idx] = u1d[idx]
    cat1[idx] = u1[idx]
    return nothing
...
function jacc_kernel_unet_d_40!(__jacc_i, c1, c2, cat1, cat1d, hw, skip1, skip1d)
    idx = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_41!(__jacc_i, cat1pad, cat1padd, n_cat1pad, zero_val)
    i = 1 + (__jacc_i - 1)
    cat1padd[i] = 0.0
    cat1pad[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_42!(__jacc_i, c21, cat1, cat1d, cat1pad, cat1padd, hp1, hw, pad, w, wp1)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_43!(__jacc_i, b_d1a, b_d1ad, c1, c21, cat1pad, cat1padd, hp1, hw, kh, khkw, kw, t_d1, t_d1d, w, w_d1a, w_d1ad, wp1)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_44!(__jacc_i, n_d1_mid, t_d1, t_d1d, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_45!(__jacc_i, n_d1_midpad, t_d1pad, t_d1padd, zero_val)
    i = 1 + (__jacc_i - 1)
    t_d1padd[i] = 0.0
    t_d1pad[i] = zero_val
    return nothing
...
function jacc_kernel_unet_d_46!(__jacc_i, c1, hp1, hw, pad, t_d1, t_d1d, t_d1pad, t_d1padd, w, wp1)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cid = 0.0
...
function jacc_kernel_unet_d_47!(__jacc_i, b_d1b, b_d1bd, c1, dec1out, dec1outd, hp1, hw, kh, khkw, kw, t_d1pad, t_d1padd, w, w_d1b, w_d1bd, wp1)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function jacc_kernel_unet_d_48!(__jacc_i, dec1out, dec1outd, n_d1_out, zero_val)
    i = 1 + (__jacc_i - 1)
...
function jacc_kernel_unet_d_49!(__jacc_i, b_out, b_outd, c1, c_out, dec1out, dec1outd, hw, kh_out, khkw_out, kw_out, w, w_out, w_outd, y, yd)
    idx = 1 + (__jacc_i - 1)
    idxm1d = 0.0
    idxm1 = idx - 1
    cod = 0.0
...
function unet_d_jacc(x, xd, h, w, c_in, c1, c2, c3, c_out, w_e1a, w_e1ad, b_e1a, b_e1ad, w_e1b, w_e1bd, b_e1b, b_e1bd, w_e2a, w_e2ad, b_e2a, b_e2ad, w_e2b, w_e2bd, b_e2b, b_e2bd, w_ba, w_bad, b_ba, b_bad, w_bb, w_bbd, b_bb, b_bbd, w_d2a, w_d2ad, b_d2a, b_d2ad, w_d2b, w_d2bd, b_d2b, b_d2bd, w_d1a, w_d1ad, b_d1a, b_d1ad, w_d1b, w_d1bd, b_d1b, b_d1bd, w_out, w_outd, b_out, b_outd, xpad0, xpad0d, t_e1, t_e1d, t_e1pad, t_e1padd, skip1, skip1d, p1, p1d, p1pad, p1padd, t_e2, t_e2d, t_e2pad, t_e2padd, skip2, skip2d, p2, p2d, p2pad, p2padd, t_b, t_bd, t_bpad, t_bpadd, bott, bottd, u2, u2d, cat2, cat2d, cat2pad, cat2padd, t_d2, t_d2d, t_d2pad, t_d2padd, dec2out, dec2outd, u1, u1d, cat1, cat1d, cat1pad, cat1padd, t_d1, t_d1d, t_d1pad, t_d1padd, dec1out, dec1outd, y, yd)
    ...
    n_d1_outd = 0.0
    n_d1_out = c1 * hw
    JACC.@parallel_for range = div(n_xpad0 - 1, 1) + 1 jacc_kernel_unet_d_1!(n_xpad0, xpad0, xpad0d, zero_val)
    JACC.@parallel_for range = div(c_in * hw - 1, 1) + 1 jacc_kernel_unet_d_2!(c_in, hp1, hw, pad, w, wp1, x, xd, xpad0, xpad0d)
    JACC.@parallel_for range = div(c1 * hw - 1, 1) + 1 jacc_kernel_unet_d_3!(b_e1a, b_e1ad, c1, c_in, hp1, hw, kh, khkw, kw, t_e1, t_e1d, w, w_e1a, w_e1ad, wp1, xpad0, xpad0d)
    JACC.@parallel_for range = div(n_e1_mid - 1, 1) + 1 jacc_kernel_unet_d_4!(n_e1_mid, t_e1, t_e1d, zero_val)
    JACC.@parallel_for range = div(n_e1_midpad - 1, 1) + 1 jacc_kernel_unet_d_5!(n_e1_midpad, t_e1pad, t_e1padd, zero_val)
    JACC.@parallel_for range = div(c1 * hw - 1, 1) + 1 jacc_kernel_unet_d_6!(c1, hp1, hw, pad, t_e1, t_e1d, t_e1pad, t_e1padd, w, wp1)
    JACC.@parallel_for range = div(c1 * hw - 1, 1) + 1 jacc_kernel_unet_d_7!(b_e1b, b_e1bd, c1, hp1, hw, kh, khkw, kw, skip1, skip1d, t_e1pad, t_e1padd, w, w_e1b, w_e1bd, wp1)
    JACC.@parallel_for range = div(n_e1_out - 1, 1) + 1 jacc_kernel_unet_d_8!(n_e1_out, skip1, skip1d, zero_val)
    JACC.@parallel_for range = div(c1 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_9!(c1, hw, hw2, p1, p1d, skip1, skip1d, w, w2)
    JACC.@parallel_for range = div(n_p1pad - 1, 1) + 1 jacc_kernel_unet_d_10!(n_p1pad, p1pad, p1padd, zero_val)
    JACC.@parallel_for range = div(c1 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_11!(c1, hp2, hw2, p1, p1d, p1pad, p1padd, pad, w2, wp2)
    JACC.@parallel_for range = div(c2 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_12!(b_e2a, b_e2ad, c1, c2, hp2, hw2, kh, khkw, kw, p1pad, p1padd, t_e2, t_e2d, w2, w_e2a, w_e2ad, wp2)
    JACC.@parallel_for range = div(n_e2_mid - 1, 1) + 1 jacc_kernel_unet_d_13!(n_e2_mid, t_e2, t_e2d, zero_val)
    JACC.@parallel_for range = div(n_e2_midpad - 1, 1) + 1 jacc_kernel_unet_d_14!(n_e2_midpad, t_e2pad, t_e2padd, zero_val)
    JACC.@parallel_for range = div(c2 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_15!(c2, hp2, hw2, pad, t_e2, t_e2d, t_e2pad, t_e2padd, w2, wp2)
    JACC.@parallel_for range = div(c2 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_16!(b_e2b, b_e2bd, c2, hp2, hw2, kh, khkw, kw, skip2, skip2d, t_e2pad, t_e2padd, w2, w_e2b, w_e2bd, wp2)
    JACC.@parallel_for range = div(n_e2_out - 1, 1) + 1 jacc_kernel_unet_d_17!(n_e2_out, skip2, skip2d, zero_val)
    JACC.@parallel_for range = div(c2 * hw4 - 1, 1) + 1 jacc_kernel_unet_d_18!(c2, hw2, hw4, p2, p2d, skip2, skip2d, w2, w4)
    JACC.@parallel_for range = div(n_p2pad - 1, 1) + 1 jacc_kernel_unet_d_19!(n_p2pad, p2pad, p2padd, zero_val)
    JACC.@parallel_for range = div(c2 * hw4 - 1, 1) + 1 jacc_kernel_unet_d_20!(c2, hp4, hw4, p2, p2d, p2pad, p2padd, pad, w4, wp4)
    JACC.@parallel_for range = div(c3 * hw4 - 1, 1) + 1 jacc_kernel_unet_d_21!(b_ba, b_bad, c2, c3, hp4, hw4, kh, khkw, kw, p2pad, p2padd, t_b, t_bd, w4, w_ba, w_bad, wp4)
    JACC.@parallel_for range = div(n_b_mid - 1, 1) + 1 jacc_kernel_unet_d_22!(n_b_mid, t_b, t_bd, zero_val)
    JACC.@parallel_for range = div(n_b_midpad - 1, 1) + 1 jacc_kernel_unet_d_23!(n_b_midpad, t_bpad, t_bpadd, zero_val)
    JACC.@parallel_for range = div(c3 * hw4 - 1, 1) + 1 jacc_kernel_unet_d_24!(c3, hp4, hw4, pad, t_b, t_bd, t_bpad, t_bpadd, w4, wp4)
    JACC.@parallel_for range = div(c3 * hw4 - 1, 1) + 1 jacc_kernel_unet_d_25!(b_bb, b_bbd, bott, bottd, c3, hp4, hw4, kh, khkw, kw, t_bpad, t_bpadd, w4, w_bb, w_bbd, wp4)
    JACC.@parallel_for range = div(n_b_out - 1, 1) + 1 jacc_kernel_unet_d_26!(bott, bottd, n_b_out, zero_val)
    JACC.@parallel_for range = div(c3 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_27!(bott, bottd, c3, hw2, hw4, scale, u2, u2d, w2, w4)
    JACC.@parallel_for range = div(c3 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_28!(c3, cat2, cat2d, hw2, u2, u2d)
    JACC.@parallel_for range = div(c2 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_29!(c2, c3, cat2, cat2d, hw2, skip2, skip2d)
    JACC.@parallel_for range = div(n_cat2pad - 1, 1) + 1 jacc_kernel_unet_d_30!(cat2pad, cat2padd, n_cat2pad, zero_val)
    JACC.@parallel_for range = div(c32 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_31!(c32, cat2, cat2d, cat2pad, cat2padd, hp2, hw2, pad, w2, wp2)
    JACC.@parallel_for range = div(c2 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_32!(b_d2a, b_d2ad, c2, c32, cat2pad, cat2padd, hp2, hw2, kh, khkw, kw, t_d2, t_d2d, w2, w_d2a, w_d2ad, wp2)
    JACC.@parallel_for range = div(n_d2_mid - 1, 1) + 1 jacc_kernel_unet_d_33!(n_d2_mid, t_d2, t_d2d, zero_val)
    JACC.@parallel_for range = div(n_d2_midpad - 1, 1) + 1 jacc_kernel_unet_d_34!(n_d2_midpad, t_d2pad, t_d2padd, zero_val)
    JACC.@parallel_for range = div(c2 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_35!(c2, hp2, hw2, pad, t_d2, t_d2d, t_d2pad, t_d2padd, w2, wp2)
    JACC.@parallel_for range = div(c2 * hw2 - 1, 1) + 1 jacc_kernel_unet_d_36!(b_d2b, b_d2bd, c2, dec2out, dec2outd, hp2, hw2, kh, khkw, kw, t_d2pad, t_d2padd, w2, w_d2b, w_d2bd, wp2)
    JACC.@parallel_for range = div(n_d2_out - 1, 1) + 1 jacc_kernel_unet_d_37!(dec2out, dec2outd, n_d2_out, zero_val)
    JACC.@parallel_for range = div(c2 * hw - 1, 1) + 1 jacc_kernel_unet_d_38!(c2, dec2out, dec2outd, hw, hw2, scale, u1, u1d, w, w2)
    JACC.@parallel_for range = div(c2 * hw - 1, 1) + 1 jacc_kernel_unet_d_39!(c2, cat1, cat1d, hw, u1, u1d)
    JACC.@parallel_for range = div(c1 * hw - 1, 1) + 1 jacc_kernel_unet_d_40!(c1, c2, cat1, cat1d, hw, skip1, skip1d)
    JACC.@parallel_for range = div(n_cat1pad - 1, 1) + 1 jacc_kernel_unet_d_41!(cat1pad, cat1padd, n_cat1pad, zero_val)
    JACC.@parallel_for range = div(c21 * hw - 1, 1) + 1 jacc_kernel_unet_d_42!(c21, cat1, cat1d, cat1pad, cat1padd, hp1, hw, pad, w, wp1)
    JACC.@parallel_for range = div(c1 * hw - 1, 1) + 1 jacc_kernel_unet_d_43!(b_d1a, b_d1ad, c1, c21, cat1pad, cat1padd, hp1, hw, kh, khkw, kw, t_d1, t_d1d, w, w_d1a, w_d1ad, wp1)
    JACC.@parallel_for range = div(n_d1_mid - 1, 1) + 1 jacc_kernel_unet_d_44!(n_d1_mid, t_d1, t_d1d, zero_val)
    JACC.@parallel_for range = div(n_d1_midpad - 1, 1) + 1 jacc_kernel_unet_d_45!(n_d1_midpad, t_d1pad, t_d1padd, zero_val)
    JACC.@parallel_for range = div(c1 * hw - 1, 1) + 1 jacc_kernel_unet_d_46!(c1, hp1, hw, pad, t_d1, t_d1d, t_d1pad, t_d1padd, w, wp1)
    JACC.@parallel_for range = div(c1 * hw - 1, 1) + 1 jacc_kernel_unet_d_47!(b_d1b, b_d1bd, c1, dec1out, dec1outd, hp1, hw, kh, khkw, kw, t_d1pad, t_d1padd, w, w_d1b, w_d1bd, wp1)
    JACC.@parallel_for range = div(n_d1_out - 1, 1) + 1 jacc_kernel_unet_d_48!(dec1out, dec1outd, n_d1_out, zero_val)
    JACC.@parallel_for range = div(c_out * hw - 1, 1) + 1 jacc_kernel_unet_d_49!(b_out, b_outd, c1, c_out, dec1out, dec1outd, hw, kh_out, khkw_out, kw_out, w, w_out, w_outd, y, yd)
    return nothing
end
