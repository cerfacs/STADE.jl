...
function cuda_kernel_two_field_loss_hv_3!(i_n, loss, lossd, p, pd, q, qd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_two_field_loss_hv_4!(i_n, lossb, lossbd, pb, pbd, qb, qbd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
...
function cuda_kernel_two_field_loss_hv_5!(i_n, qb, qbd, v, vb, vbd, vd)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function cuda_kernel_two_field_loss_hv_6!(i_n, pb, pbd, u, ub, ubd, ud)
    __tid = ((blockIdx()).x - 1) * (blockDim()).x + (threadIdx()).x
    if __tid > div(i_n - 1, 1) + 1
        return nothing
    end
...
function two_field_loss_hv_cuda(loss, lossb, u, ub, v, vb, p, pb, q, qb, i_n, lossd, lossbd, ud, ubd, vd, vbd, pd, pbd, qd, qbd)
    ...
    @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_two_field_loss_hv_3!(i_n, loss, lossd, p, pd, q, qd)
    @cuda threads = nthread_per_block blocks = cld(div(1 - i_n, -1) + 1, nthread_per_block) cuda_kernel_two_field_loss_hv_4!(i_n, lossb, lossbd, pb, pbd, qb, qbd)
    @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_two_field_loss_hv_5!(i_n, qb, qbd, v, vb, vbd, vd)
    @cuda threads = nthread_per_block blocks = cld(div(i_n - 1, 1) + 1, nthread_per_block) cuda_kernel_two_field_loss_hv_6!(i_n, pb, pbd, u, ub, ubd, ud)
    return nothing
end
